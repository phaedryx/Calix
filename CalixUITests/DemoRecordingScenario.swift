...
// records real Claude Code agents running in Calix panes (including a
...
//
// Skipped by default (see the CALIX_DEMO_RECORDING guard, first line of
// the test method) so it never runs as part of a normal CalixUITests
// pass -- only `scripts/record-demo.sh` sets that env var (propagated to
// the test runner process via project.yml's own
// `schemes.CalixUITests.test.environmentVariables` entry, the same
// propagation `CALIX_SESSION_BIN` already needs there: a plain shell
// prefix on `xcodebuild test` does not otherwise reach the runner).
//
// LAUNCH CONFIGURATION, and why this does NOT use CalixUITestCase's own
...
// method, after the CALIX_DEMO_RECORDING skip guard, so `setUp()`/
...
// argument (`-calix.cockpit.agentHookApprovalEnabled YES`) rather than a
// runner-side `UserDefaults(suiteName:).set(...)` write: the
// CalixUITests runner is itself App-Sandboxed (see
...
// per-run UUID (CalixUITestCase.setUp()'s own convention), cleaned up in
// `tearDown()` below.
//
...
final class DemoRecordingScenario: CalixUITestCase {
    ...
    /// Per-run UUID defaults suite (CalixUITestCase.setUp()'s own
    ...
    override func tearDown() {
        ...
        // super.tearDown(): CalixUITestCase.tearDown() force-unwraps
        ...
        // CALIX_DEMO_RECORDING guard skips before `app` is ever assigned,
        ...
        if let defaultsSuiteName {
            ...
            // CalixUITestCase.tearDown()'s own identical cleanup.
    ...
    func test_demoRecordingScenario() throws {
        guard ProcessInfo.processInfo.environment["CALIX_DEMO_RECORDING"] == "1" else {
            throw XCTSkip("Set CALIX_DEMO_RECORDING=1 to run the demo recording scenario")
        }

        // MARK: - Launch configuration

        // Per-run UUID suite name (CalixUITestCase.setUp()'s own
        ...
        let suiteName = "com.calix.tests.e2e.DemoRecordingScenario-\(UUID().uuidString)"
        defaultsSuiteName = suiteName

        let tempDir = NSTemporaryDirectory() + "CalixDemoRecording-\(UUID().uuidString)"
        ...
        app.launchArguments = [
            ...
            // (Calix/Features/ApprovalInbox/CockpitSettings.swift; this
            // target has no @testable import Calix linkage). See this
            // file's header for why this must be an NSArgumentDomain
            // argument, not a suite pre-seed from the test runner.
            "-calix.cockpit.agentHookApprovalEnabled", "YES"
        ]
        ...
        clickQuadrant(1)
        panePasteAndReturn(
            "cd /tmp/calix-demo-workspace && claude --model sonnet " +
        ...
        clickQuadrant(4)
        panePasteAndReturn("cd /tmp/calix-demo-workspace && clear")
        ...
        let banner = app.descendants(matching: .any)
            .matching(identifier: "calix.approvalBanner.container")
            .firstMatch
        awaitOrContinue(banner, timeout: 60)
        Thread.sleep(forTimeInterval: 2)

        let allowButton = app.buttons["calix.approvalBanner.allowButton"]
        ...
        let agentModeButton = app.descendants(matching: .any)
            .matching(identifier: "calix.sidebar.agentModeButton")
            .firstMatch
        ...
        let firstAgentRow = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "calix.sidebar.agentRow."))
            .firstMatch
        ...
        // ("touch /tmp/calix-demo-done") is a completion SENTINEL: the
        ...
        panePasteAndReturn(
            "Respond only in English. Using the calix-ipc tools, find the pane that is running a " +
            "shell command (not one of the agent panes), wait for that command to finish, then tell " +
            "me how many tests passed. When you have answered, run exactly: touch /tmp/calix-demo-done"
        )

        // Hold for pane 4 to actually run the tests and pane 1 to
        // summarize the result -- a KEEPER loop that ends on the agent's
        // OWN completion sentinel (/tmp/calix-demo-done, touched by its
        ...
        // the same "sandboxed CalixUITests runner reading a plain /tmp
        ...
        for _ in 0..<90 {
            Thread.sleep(forTimeInterval: 2)
            approvePendingIfAny()
            if FileManager.default.fileExists(atPath: "/tmp/calix-demo-done") {
                break
            }
        }
    ...
    /// near-duplicate rather than promoted onto CalixUITestCase itself,
    ...
    private func clickAllowAllPending() {
        let crossActionsMenu = app.descendants(matching: .any)
            .matching(identifier: "calix.approvalBanner.crossActionsMenu")
            .firstMatch
        ...
        let itemByID = app.menuItems["calix.approvalBanner.allowAllPendingItem"]
    ...
    /// EVERY agent tool call banners (CalixMCPServer's
