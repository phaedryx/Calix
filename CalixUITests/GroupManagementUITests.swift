...
final class GroupManagementUITests: CalixUITestCase {
    ...
    func test_renameGroupByDoubleClick() {
        ...
        let groups = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "calix.sidebar.group."))
        ...
        let renameField = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "calix.sidebar.groupNameTextField."))
            .firstMatch
        XCTAssertTrue(
    ...
    /// Sidebar group header elements (identifier `calix.sidebar.group.<UUID>`,
    ...
    private func sidebarGroupsByPosition() -> [XCUIElement] {
        let query = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH 'calix.sidebar.group.' AND NOT identifier CONTAINS '.closeAllButton' AND NOT identifier CONTAINS '.collapseButton'"))
    ...
    func test_collapseGroupHidesTabs() {
        ...
        let initialTabCount = countElements(matching: "calix.sidebar.tab.")
        ...
        let collapseButton = app.descendants(matching: .any)
            .matching(NSPredicate(format: "identifier BEGINSWITH %@", "calix.sidebar.groupCollapseButton."))
            .firstMatch
        XCTAssertTrue(
        ...
        let tabCountAfterCollapse = countElements(matching: "calix.sidebar.tab.")
        XCTAssertEqual(
    ...
    func test_expandGroupShowsTabs() {
        ...
        let tabCountCollapsed = countElements(matching: "calix.sidebar.tab.")
        XCTAssertEqual(
            tabCountCollapsed, 0,
            "All tabs should be hidden after collapsing the group"
        )
        ...
        let tabCountExpanded = countElements(matching: "calix.sidebar.tab.")
        XCTAssertGreaterThanOrEqual(
            tabCountExpanded, 1,
    ...
    func test_consecutiveGroupCloseButtons() {
        ...
        let initialGroupCount = countElements(matching: "calix.sidebar.group.", excludingSuffix: "Button")
        ...
        XCTAssertEqual(
            countElements(matching: "calix.sidebar.group.", excludingSuffix: "Button"),
            2,
