...
// (`Calix/Features/SessionBrowser/SessionBrowserWindowController.swift`,
...
// `SUN_LEN`, `CALIX_UITEST_SESSION_DIR` for window/tab persistence,
// `CALIX_SESSION_BIN` pinning a specific built `calix-session` binary)
...
// (Calix/Features/SessionBrowser/SessionBrowserView.swift) each carry
// per-row `.accessibilityIdentifier`s from `AccessibilityID.SessionBrowser`
// (Calix/Helpers/AccessibilityID.swift), the same pattern
...
// up by its row-scoped identifier (`calix.sessionBrowser.row.<id>
// .attachButton` / `.killButton`) rather than by label.

import XCTest

final class SessionBrowserAttachKillE2ETests: CalixUITestCase {
    ...
    /// itself -- not some Calix-internal setting -- is what redirects
    ...
    private func launchApp() {
        ...
        app.launchEnvironment["CALIX_SESSION_BIN"] = CalixUITestCase.builtSessionBinaryPath
        app.launch()
    }
    ...
    /// suite's `homeDir` launches Calix.app with), so it would silently
    ...
    /// the count of actual Calix main (tab-hosting) windows, the thing
    ...
    /// calix-session CLI, routed through A's own pane -- see
    /// `PaneCLIExec.paneExec`'s header for why a sandboxed XCUITest
    /// runner cannot spawn `calix-session` itself). Opens the browser,
    ...
    func test_sessionBrowser_attachDetachedSession_thenKillAnotherSession() {
        ...
        // header for why a bare `calix-session`/`$CALIX_SESSION_BIN`
        // inside a pane silently resolves against the real HOME instead
        // of this test's isolated one). If `CALIX_SESSION_BIN` isn't set
        ...
        let sessionBinaryPath = CalixUITestCase.builtSessionBinaryPath
        XCTAssertFalse(
            sessionBinaryPath.isEmpty,
            "CALIX_SESSION_BIN is empty in this test runner's own process environment, " +
            "so every calix-session command this suite pastes into a pane would be " +
            ...
            "header). Set CALIX_SESSION_BIN as a build setting/env var when invoking " +
            "xcodebuild test, e.g. `xcodebuild test ... CALIX_SESSION_BIN=/path/to/" +
            "calix-session`."
        )
        ...
        // Session B: created Detached (via `calix-session new`, which
        // never attaches -- see calix-session/crates/cli/src/commands/
        ...
        // REAL `~/.calix`, not this test's isolated homeDir.
        //
        ...
        // process context `calix-session new`'s spawned child actually
        ...
        // resolved runner-side above (never a `$CALIX_SESSION_BIN`
        ...
        let sessionBID = paneExec(
            "'\(sessionBinaryPath)' \(calixSessionRootFlags(homeDir: homeDir)) new --name sessionB --cwd \(sessionBCwd)",
            counter: &execCounter
        )
        XCTAssertFalse(
            sessionBID.isEmpty || sessionBID == "(no output)",
            "`calix-session new` produced no output when run through session A's pane, " +
            "meaning the CLI call routed through the pane never reached the daemon (or " +
            "CALIX_SESSION_BIN isn't inherited into the pane's environment -- see " +
        ...
        XCTAssertFalse(
            ...
            "`calix-session new --cwd \(sessionBCwd)` returned an error instead of a " +
        ...
        guard let sessionBRow = sessionBPoll.value else {
            XCTFail(
                "Session B (id \(sessionBID), printed by `calix-session new`) never " +
        ...
        XCTAssertEqual(
            ...
            "Session B should have 0 attached_clients right after `calix-session new` " +
        ...
        // Calix itself, not this suite), so it renders its raw id; B
        ...
        // anywhere in this Calix process) but not A (isAttachedHere,
        ...
        let attachButtonForB = app.buttons["calix.sessionBrowser.row.\(sessionBID).attachButton"]
        ...
        XCTAssertEqual(
            ...
            "The number of Calix main windows changed after attaching session B " +
        ...
        let killButtonForA = app.buttons["calix.sessionBrowser.row.\(sessionAID).killButton"]
