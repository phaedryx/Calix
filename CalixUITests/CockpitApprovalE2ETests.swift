...
// pane's shell does NOT inherit Calix.app's own HOME override at all --
...
// scripts read `~/Library/Application Support/Calix/agent-endpoint.json`
...
// WORSE mismatch. `CALIX_UITEST_SESSION_DIR` / `CALIX_UITEST_DEFAULTS_SUITE`
...
// `uiTestSuite`, keyed by `CALIX_UITEST_DEFAULTS_SUITE`, never
// `.standard`).
//
...
// own header on why: the CalixUITests runner is itself App-Sandboxed and
...
final class CockpitApprovalE2ETests: CalixUITestCase {

    // MARK: - Accessibility identifiers

    /// Literal mirrors of `AccessibilityID.ApprovalBanner.*`
    /// (Calix/Helpers/AccessibilityID.swift) -- the `CalixUITests`
    /// target drives the app-under-test as a separate OS process via
    /// `XCUIApplication`, with no `@testable import Calix` linkage, so
    ...
    /// own `"calix.commandPalette.searchField"`) already hardcodes the
    ...
    private static let approvalBannerAllowButtonID = "calix.approvalBanner.allowButton"
    private static let approvalBannerDenyButtonID = "calix.approvalBanner.denyButton"
    private static let approvalBannerPayloadID = "calix.approvalBanner.payload"
    private static let approvalBannerNextButtonID = "calix.approvalBanner.nextButton"
    private static let approvalBannerPreviousButtonID = "calix.approvalBanner.previousButton"
    private static let approvalBannerPositionLabelID = "calix.approvalBanner.positionLabel"
    ...
    func test_cockpitTools_endToEnd() throws {
        ...
        let surfaceID = paneExec("echo $CALIX_SURFACE_ID", counter: &counter)
        XCTAssertNotEqual(surfaceID, "(no output)", "could not read $CALIX_SURFACE_ID from the pane")
        XCTAssertFalse(surfaceID.isEmpty, "$CALIX_SURFACE_ID must be set for every ghostty-spawned pane")
        ...
        let allowOutFile = "/tmp/calix-e2e-cockpit-run-allow-\(ProcessInfo.processInfo.processIdentifier).json"
        ...
        let denyOutFile = "/tmp/calix-e2e-cockpit-run-deny-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
        ...
        let navAOutFile = "/tmp/calix-e2e-cockpit-run-nav-a-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
        ...
        let navBOutFile = "/tmp/calix-e2e-cockpit-run-nav-b-\(ProcessInfo.processInfo.processIdentifier).json"
        toolCallBackgrounded(
            name: "pane_run",
    ...
    private func enableAIAgentIPCViaCommandPalette() {
        ...
        XCTAssertTrue(alert.waitForExistence(timeout: 10),
                     "the IPC enable/error alert (CalixWindowController.showIPCAlert) did not appear")
    ...
    private func toolCallSync(name: String, argumentsJSON: String, counter: inout Int, timeoutAttempts: Int = 20) -> [String: Any] {
        ...
        counter += 1
        let scriptPath = "/tmp/calix-e2e-cockpit-sync-\(counter).py"
    ...
    private func toolCallBackgrounded(name: String, argumentsJSON: String, outFile: String, counter: inout Int) {
        ...
        let encoded = Data(script.utf8).base64EncodedString()
        counter += 1
        let scriptPath = "/tmp/calix-e2e-cockpit-bg-\(counter).py"
