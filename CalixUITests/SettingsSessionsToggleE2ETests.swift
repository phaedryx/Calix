...
//
// WHY NO "-calix.session.persistentSessionsEnabled" LAUNCH ARGUMENT
...
//
// WHY A PER-TEST `CALIX_UITEST_DEFAULTS_SUITE`, NOT THE REAL
// `com.calix.terminal.e2e` DOMAIN (investigated, not assumed -- an
...
// developer's REAL `com.calix.terminal.e2e` defaults domain regardless
// of `homeDir`). `SessionSettings` now has its own dedicated seam for
// this (`CALIX_UITEST_DEFAULTS_SUITE`, mirroring
// `SessionPersistenceActor`'s existing `CALIX_UITEST_SESSION_DIR`
// convention): this suite launches Calix with that variable set to a
...
// isolate everything else this journey touches (`CALIX_UITEST_SESSION_DIR`
...
final class SettingsSessionsToggleE2ETests: CalixUITestCase {

    private var homeDir: String!
    private var sessionDir: String!
    /// Per-test-unique `UserDefaults` suite name (see this file's header,
    /// "WHY A PER-TEST `CALIX_UITEST_DEFAULTS_SUITE`"): the app-under-test
    /// reads/writes `SessionSettings.persistentSessionsEnabled` here
    /// instead of the developer's real `com.calix.terminal.e2e` defaults
    ...
    override func setUp() {
        ...
        defaultsSuiteName = "com.calix.tests.e2e.SettingsSessionsToggleE2ETests-\(UUID().uuidString)"
    ...
    override func tearDown() {
        ...
        if let defaultsSuiteName {
            // Best-effort only (mirrors CalixUITestCase.saveScreenshot's
            ...
            // `com.calix.terminal.e2e` domain, which this suite never
    ...
    /// file's header): no `-calix.session.*` overrides at all.
    ...
    /// `AccessibilityID` enum type itself: CalixUITests never imports
    /// the Calix module (this whole target only `import XCTest`,
    ...
    /// (Calix/Helpers/AccessibilityID.swift, a new case this cycle's
    /// fix proposes) once it's wired onto the real switch via
    /// `.setAccessibilityIdentifier(...)` in SettingsWindowController.swift.
    private static let persistentSessionsSwitchIdentifier = "calix.settings.sessions.persistentSessionsSwitch"
    ...
    func test_togglingPersistentSessionsSwitch_actuallyPersistsAcrossRelaunch() {
        ...
        quitAppViaMenu()
        XCTAssertTrue(app.wait(for: .notRunning, timeout: 10), "Calix did not fully quit after \"Quit Calix\".")
