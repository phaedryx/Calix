...
final class MenuShortcutsUITests: CalixUITestCase {
    ...
    /// switch groups using the existing `calix.sidebar.group.<UUID>` identifier
    /// scheme.
    ...
    func test_findViaMenu_opensSearchBar() {
        ...
        XCTAssertTrue(
            ...
            "Edit > Find > Find… should reveal the in-terminal search bar (identifier: calix.search.searchField)"
        )
    }
    ...
    func test_splitRight_isDisabled_inBrowserTab() {
        ...
        let toolbar = app.descendants(matching: .any)
            .matching(identifier: "calix.browser.toolbar")
            .firstMatch
        XCTAssertTrue(
