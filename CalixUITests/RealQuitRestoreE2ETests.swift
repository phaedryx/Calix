...
// `menuAction("Calix", item: "Quit Calix")` while launched with
...
// `CalixWindowController.windowShouldClose`/`windowWillClose` at all
...
// `CalixWindowController.windowWillClose` used to save the snapshot on
...
// (`CalixWindowController.swift`) instead of the per-window
// `isClosingForShutdown` flag. The existing unit test for this
// (`CalixTests/Views/CalixWindowControllerLastWindowCloseSaveTests
...
// `CalixWindowController.windowShouldClose`'s OWN independent call to
// `appDelegate.confirmQuitIfNeeded()` (CalixWindowController.swift:
...
// anywhere in Calix). `cursorIsAtPrompt()`
...
// its 2026-07-04 update) says Calix never sets `GHOSTTY_RESOURCES_DIR`,
...
// shell integration into calix-session-managed shells -- exactly what
...
final class RealQuitRestoreE2ETests: CalixUITestCase {
    ...
    /// `CALIX_UITEST_SESSION_DIR`, `CALIX_SESSION_BIN`).
    override func setUp() {
    ...
    func test_realQuit_closingLastWindow_showsConfirmDialog_andSnapshotSurvivesForReattach() {
        ...
        // termination flow) and NOT the "Quit Calix" menu item (calls
        ...
        XCTAssertTrue(
            confirmDialog.staticTexts["Quit Calix?"].exists,
            "The confirm-quit dialog appeared but its title text did not match " +
            "confirmQuitIfNeeded()'s literal \"Quit Calix?\" messageText."
        )
        ...
        XCTAssertEqual(
            ...
            "The number of Calix main windows changed after the real-quit/relaunch round " +
