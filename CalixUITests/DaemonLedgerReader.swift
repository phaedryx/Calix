...
// unix-domain-socket connection to the daemon, so `calix-session ls`
...
/// Reads `$HOME/.calix/state/sessions.json` (see
/// `calix-session/crates/daemon/src/ledger.rs`) directly from disk and
...
struct DaemonLedgerReader {
    ...
    /// serde derive in `calix-session/crates/proto/src/control.rs`).
    ...
    /// change, so a `calix-session meta set` reaches this file without
