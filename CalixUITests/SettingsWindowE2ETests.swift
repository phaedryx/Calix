...
// switcher (Calix/Features/Settings/SettingsWindowController.swift,
...
// is scoped to CalixUITests/ only, so:
...
// re-focuses the main Calix window afterward, so it stays the frontmost/
// key window (and XCUITest's `windows` query orders front-to-back) for
// this test's whole duration.

import XCTest

final class SettingsWindowE2ETests: CalixUITestCase {
    ...
    /// to `CalixUITestCase.uiShotDir` for manual review -- including of
    ...
    func test_settingsWindow_toolbarTabsHaveImagesAndSwitchContent() {
        ...
        XCTAssertTrue(
            ...
            "No window appeared after invoking Preferences… from the Calix application menu."
        )
