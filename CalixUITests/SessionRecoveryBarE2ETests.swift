...
// CalixTests/RecoveryBarModelTests.swift for the view-model-level RED
...
// existence check against `$CALIX_UITEST_SESSION_DIR/sessions.recovery.json`
...
// calix-session daemon-backed persistent session or an ordinary
...
// that method's own doc comment) -- no calix-session binary, no daemon
// socket, no `CALIX_SESSION_BIN` involved at all. So instead of
// standing up an isolated daemon (short-HOME trick, CALIX_SESSION_BIN,
...
// `sessionRefs` key) directly into a fresh `CALIX_UITEST_SESSION_DIR`
...
// (CalixTests/, root) -- this suite adds the missing "the user can
...
//
// Isolation: `CALIX_UITEST_SESSION_DIR` only (mirrors the base
// CalixUITestCase.setUp()'s own key) -- no HOME override, no
// CALIX_SESSION_BIN, no calix-session daemon involved at all (see
...
/// the preserved file".
final class SessionRecoveryBarE2ETests: CalixUITestCase {
    ...
    override func setUp() {
        ...
        sessionDir = NSTemporaryDirectory() + "CalixUITests-recoverybar-\(UUID().uuidString)"
    ...
    /// to under a `CALIX_UITEST_SESSION_DIR` override (see
    /// SessionPersistenceActor.swift's init): the directory itself IS
    /// `calixDir` in that branch (unlike a real launch, which appends
    /// `.calix` under `HOME` -- the test override skips that extra
    /// path component), so the recovery file lives directly at
    /// `<sessionDir>/sessions.recovery.json`, not
    /// `<sessionDir>/.calix/sessions.recovery.json`.
    ...
    /// shell with zero calix-session/daemon involvement (see file
    ...
    //
    // Literal `"calix.recoveryBar.*"` strings, not `AccessibilityID.RecoveryBar.*`:
    // CalixUITests runs the app-under-test as a SEPARATE process (no
    // `@testable import Calix` anywhere in this target -- confirmed by
    ...
    // `app.buttons["calix.sessionBrowser.row.\(id).attachButton"]`
    // already does, cross-referenced back to
    // Calix/Helpers/AccessibilityID.swift by name in each comment.

    /// AccessibilityID.RecoveryBar.restoreButton
    private var restoreButton: XCUIElement {
        app.buttons["calix.recoveryBar.restoreButton"]
    }

    /// AccessibilityID.RecoveryBar.dismissButton
    private var dismissButton: XCUIElement {
        app.buttons["calix.recoveryBar.dismissButton"]
    }
    ...
    private func pollForRecoveryBarContainer(exists: Bool, timeout: TimeInterval = 10) -> Bool {
        ...
        repeat {
            let found = app.descendants(matching: .any)
                .matching(identifier: "calix.recoveryBar.container")
                .firstMatch.exists
            if found == exists {
                return true
            }
    ...
    /// `CalixWindowController.setupCommandRegistry()`). Unlike the bar,
    ...
    func test_recoveryBar_paletteRecoverPreviousSession_restoresWindowClearsFileAndDisablesCommand() {
        ...
        let resultsTable = app.descendants(matching: .any)
            .matching(identifier: "calix.commandPalette.resultsTable")
            .firstMatch
        XCTAssertEqual(
...
/// -- the base `CalixUITestCase.setUp()`'s own default session dir is
...
final class SessionRecoveryBarAbsentE2ETests: CalixUITestCase {
    ...
    func test_recoveryBar_doesNotAppear_onNormalLaunchWithNoPreservedSnapshot() {
        ...
        let staysAbsent = XCTNSPredicateExpectation(
            ...
            object: app.otherElements["calix.recoveryBar.container"]
        )
