...
// restart (calix-session's daemon keeps the child process running
// independent of Calix.app's lifetime), and the pane restored after
...
// `calix-session ls --all --json` as an out-of-process `Process` from
// inside the test runner. That approach is unusable here: the
// `CalixUITests` runner is itself App-Sandboxed (`codesign -d
...
// `calix-session ls` from the runner therefore always returns empty
...
// `$HOME/.calix/state/sessions.json` (see
// `calix-session/crates/daemon/src/ledger.rs`), the same file the
// daemon writes atomically (temp file + rename) on every registry
// change, and the same array `calix-session ls` would otherwise print,
...
// same child process alive across Calix.app's termination, and that
...
//
// Isolation:
// - Window/tab session persistence: `CalixUITestCase`'s existing
//   `CALIX_UITEST_SESSION_DIR` (per-test temp dir) — reused here via
//   its own launchEnvironment key, unchanged.
// - calix-session daemon state (`$HOME/.calix/run`,
//   `$HOME/.calix/state` — see
//   `calix-session/crates/cli/src/commands/mod.rs`'s
//   `resolve_runtime_dir`): the daemon consults `$HOME` directly and
//   has no independent `CALIX_SESSION_*` runtime-dir override wired
//   through the bundled `Calix.app` launch path, so `HOME` itself is
//   overridden to a per-test temp directory instead.
// - `CALIX_SESSION_BIN` points `SessionBinaryResolver` at a
//   specific, already-built `calix-session` binary rather than the
...
// not run yet): `Calix.app`'s ghostty-spawned `calix-session attach`
// child process inherits its environment from `Calix.app`'s own
// process, which is what makes overriding `HOME` on `app
// .launchEnvironment` (rather than some Calix-internal setting) enough
...
final class SessionPersistenceE2ETests: CalixUITestCase {
    ...
    override func setUp() {
        ...
        // $HOME/.calix/run/sessiond.sock (see
        // `calix-session/crates/cli/src/commands/mod.rs`'s
        ...
        // past that limit once `.calix/run/sessiond.sock` is appended.
        ...
        // "/.calix/run/sessiond.sock" (25 bytes) = 46 bytes total,
    ...
    /// Path to a pre-built `calix-session` binary this test points
    /// `CALIX_SESSION_BIN` at. RED-phase placeholder: the actual
    ...
    /// simulating the user quitting and reopening Calix, not a fresh
    ...
    private func launchApp() {
        ...
        app.launchEnvironment["CALIX_UITEST_SESSION_DIR"] = sessionDir
        app.launchEnvironment["HOME"] = homeDir
        app.launchEnvironment["CALIX_SESSION_BIN"] = Self.builtSessionBinaryPath()
        app.launch()
    }
    ...
    /// Quits the app through its own application menu ("Calix" > "Quit
    /// Calix"), instead of `app.terminate()`. `app.terminate()` sends
    ...
    private func quitAppViaMenu() {
        menuAction("Calix", item: "Quit Calix")
    }
    ...
    // `calix-session ls` process, because the sandboxed test runner
    // cannot open the unix domain socket that would require (see file
    // header).

    /// Reads and parses the daemon's on-disk session ledger at
    /// `$HOME/.calix/state/sessions.json` (see
    /// `calix-session/crates/daemon/src/ledger.rs`), the same array
    /// `calix-session ls` would otherwise print. Each entry mirrors
    /// `SessionInfo` (`calix-session/crates/proto/src/control.rs`):
    ...
    /// `calix-session/crates/daemon/src/conn.rs`), and `"state"`,
    ...
    private func daemonSessions() -> (sessions: [[String: Any]], raw: String) {
        let ledgerURL = URL(fileURLWithPath: "\(homeDir!)/.calix/state/sessions.json")
    ...
    func test_persistentSession_survivesRestart_sameShellStaysAlive() {
        ...
        // `calix-session attach --create`, which registers a session
        ...
        guard sawRunningSession.value else {
            XCTFail(
                ...
                "`calix-session attach --create` ever established a live daemon session. " +
        ...
        for (id, snap) in preRestart {
            XCTAssertGreaterThanOrEqual(
                ...
                "tab appearing, meaning the pane's `calix-session attach --create` " +
        ...
        XCTAssertEqual(
            ...
            "The number of Calix main windows changed after restarting (before: " +
        ...
        for (id, before) in preRestart {
            guard let after = postRestart[id] else {
                XCTFail(
                    ...
                    "keep it alive across Calix.app's termination. Last ledger contents: " +
                    "\(postRestartPoll.raw)"
                )
                continue
            }
            XCTAssertTrue(
                ...
                "the daemon-held child process died instead of surviving Calix.app's " +
                "termination."
            )
            XCTAssertEqual(
