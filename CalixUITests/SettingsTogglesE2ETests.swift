...
// identifiers, still prefixed `calix.settings.sessions.*`) plus
...
// `SessionSettings.store` (resolves `CALIX_UITEST_DEFAULTS_SUITE` before
...
final class SettingsTogglesE2ETests: CalixUITestCase {

    /// The real, standing UserDefaults domain every `--uitesting` launch
    /// of Calix.app resolves `UserDefaults.standard` to (see this file's
    /// header). Read directly from the TEST process to save/restore the
    /// three raw keys that bypass `CALIX_UITEST_DEFAULTS_SUITE`.
    ...
    private nonisolated(unsafe) static let e2eStandardDefaults = UserDefaults(suiteName: "com.calix.terminal.e2e")!

    private static let smoothScrollKey = "smoothScrollEnabled"
    private static let lspAutoInstallKey = "calix.lsp.autoInstallEnabled"
    private static let lspRequireConfirmationKey = "calix.lsp.requireInstallConfirmation"
    ...
    override func setUp() {
        ...
        defaultsSuiteName = "com.calix.tests.e2e.SettingsTogglesE2ETests-\(UUID().uuidString)"
    ...
    override func tearDown() {
        ...
        // Restore the three raw `com.calix.terminal.e2e` keys to exactly
    ...
    /// SettingsSessionsToggleE2ETests exactly: no `-calix.session.*` /
    /// `-calix.lsp.*` NSArgumentDomain overrides, which would shadow
    ...
    private func launchApp() {
        ...
        app.launchEnvironment["CALIX_UITEST_DEFAULTS_SUITE"] = defaultsSuiteName
    ...
    private func waitForMenuBarAndWindow() {
        ...
        XCTAssertTrue(
            ...
            "Calix's menu bar never appeared within ~20s of its window showing up."
        )
    }
    ...
    private func openSettingsPane(_ pane: String) {
        ...
        menuAction("Calix", item: "Settings…")
    ...
    private func assertTogglePersistsAcrossRelaunch(
        ...
        XCTAssertTrue(
            ...
            "Calix did not fully quit after \"Quit Calix\".",
            file: file, line: line
        )
    ...
    func test_historyPersistenceSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "calix.settings.sessions.historyPersistenceSwitch",
            pane: "Sessions"
        )
    }
    ...
    func test_agentResumeSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "calix.settings.sessions.agentResumeSwitch",
            pane: "Agents"
        )
    }
    ...
    func test_agentResumeAutoExecuteSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "calix.settings.sessions.agentResumeAutoExecuteSwitch",
            pane: "Agents"
        )
    }
    ...
    func test_smoothScrollingSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "calix.settings.appearance.smoothScrollingSwitch",
            pane: "Appearance"
        )
    }
    ...
    func test_lspAutoInstallSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "calix.settings.lsp.lspAutoInstallSwitch",
            pane: "LSP"
        )
    }
    ...
    func test_lspRequireConfirmationSwitch_persistsAcrossRelaunch() {
        assertTogglePersistsAcrossRelaunch(
            identifier: "calix.settings.lsp.lspRequireConfirmationSwitch",
            pane: "LSP"
        )
    }
    ...
    func test_agentHookApprovalSwitch_isReachableAndTogglesOnAgentsPane() {
        ...
        let toggleElement = toggle(identifier: "calix.settings.sessions.agentHookApprovalSwitch")
