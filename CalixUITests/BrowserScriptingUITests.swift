...
final class BrowserScriptingUITests: CalixUITestCase {
    ...
    private func paletteRun(_ query: String, buttonTitle: String = "OK") {
        ...
        let sf = app.descendants(matching: .any)
            .matching(identifier: "calix.commandPalette.searchField").firstMatch
    ...
    private func terminalExec(_ command: String) -> String {
        cmdCounter += 1
        let outFile = "/tmp/calix-e2e-\(cmdCounter).txt"
    ...
    func test_mcpToolsWorkEndToEnd() {
        ...
        let toolbar = app.descendants(matching: .any)
            .matching(identifier: "calix.browser.toolbar").firstMatch
        ...
        // 5. calix browser list — get tab_id
        let list = terminalExec("calix browser list")
        ...
        // 6. calix browser get-text h1 --tab-id <id>
        let getText = terminalExec("calix browser get-text h1 --tab-id \(tabId)")
        XCTAssertTrue(getText.contains("Example Domain"), "get-text should contain Example Domain, got: \(getText)")

        // 7. calix browser snapshot --tab-id <id>
        let snap = terminalExec("calix browser snapshot --tab-id \(tabId)")
        XCTAssertFalse(snap.isEmpty, "snapshot should not be empty")

        // 8. calix browser get-html h1 --tab-id <id>
        let getHTML = terminalExec("calix browser get-html h1 --tab-id \(tabId)")
        XCTAssertTrue(getHTML.contains("<h1"), "get-html should contain h1 tag, got: \(getHTML.prefix(200))")

        // 9. calix browser eval (before any navigation changes the page)
        let eval = terminalExec("calix browser eval \"document.title\" --tab-id \(tabId)")
        XCTAssertTrue(eval.contains("Example Domain"), "eval should return 'Example Domain', got: [\(eval)]")

        // 10. calix browser click a --tab-id <id>
        let click = terminalExec("calix browser click a --tab-id \(tabId)")
        XCTAssertTrue(click.contains("clicked"), "click should return 'clicked', got: \(click)")

        // 11. calix browser navigate --tab-id <id> (go back to example.com after click navigated away)
        let nav = terminalExec("calix browser navigate https://example.com --tab-id \(tabId)")
        XCTAssertTrue(nav.contains("Navigated"), "navigate should return 'Navigated', got: \(nav)")
        Thread.sleep(forTimeInterval: 3)

        // 12. calix browser back --tab-id <id>
        let back = terminalExec("calix browser back --tab-id \(tabId)")
        XCTAssertTrue(back.contains("back"), "back should return 'back', got: \(back)")

        // 13. calix browser forward --tab-id <id>
        let forward = terminalExec("calix browser forward --tab-id \(tabId)")
        XCTAssertTrue(forward.contains("forward"), "forward should return 'forward', got: \(forward)")

        // 14. calix browser reload --tab-id <id>
        let reload = terminalExec("calix browser reload --tab-id \(tabId)")
        XCTAssertTrue(reload.contains("Reloaded"), "reload should return 'Reloaded', got: \(reload)")
        Thread.sleep(forTimeInterval: 3)

        // 15. calix browser screenshot --tab-id <id>
        let screenshot = terminalExec("calix browser screenshot --tab-id \(tabId)")
        XCTAssertTrue(screenshot.contains("/tmp/") || screenshot.contains("path"), "screenshot should return file path, got: \(screenshot)")

        // 16. calix browser wait --selector h1 --tab-id <id>
        let wait = terminalExec("calix browser wait --selector h1 --tab-id \(tabId)")
        ...
        let _ = terminalExec("calix browser navigate https://httpbin.org/forms/post --tab-id \(tabId)")
        Thread.sleep(forTimeInterval: 3)

        // 18. fill (httpbin form has input[name=custname])
        let fill = terminalExec("calix browser fill input --value hello --tab-id \(tabId)")
        XCTAssertTrue(fill.contains("filled"), "fill should return 'filled', got: \(fill)")

        // 19. type
        let typeCmd = terminalExec("calix browser type world --tab-id \(tabId)")
        XCTAssertTrue(typeCmd.contains("typed"), "type should return 'typed', got: \(typeCmd)")

        // 20. press
        let press = terminalExec("calix browser press Tab --tab-id \(tabId)")
        XCTAssertTrue(press.contains("pressed"), "press should return 'pressed', got: \(press)")

        // 21. check (httpbin form has checkboxes)
        let check = terminalExec("calix browser check 'input[type=checkbox]' --tab-id \(tabId)")
        XCTAssertTrue(check.contains("checked"), "check should return 'checked', got: \(check)")

        // 22. uncheck
        let uncheck = terminalExec("calix browser uncheck 'input[type=checkbox]' --tab-id \(tabId)")
        XCTAssertTrue(uncheck.contains("unchecked"), "uncheck should return 'unchecked', got: \(uncheck)")

        // 24. calix browser open (opens new tab)
        let open = terminalExec("calix browser open https://example.com")
    ...
    func test_newBrowserCommands() {
        ...
        let attr2 = terminalExec("calix browser get-attribute '#test-input' name --tab-id \(tabId)")
        XCTAssertTrue(attr2.contains("email"), "get-attribute name should contain email, got: \(attr2)")

        // 7. get-links
        let links = terminalExec("calix browser get-links --tab-id \(tabId)")
        ...
        // 8. get-inputs
        let inputs = terminalExec("calix browser get-inputs --tab-id \(tabId)")
        ...
        let scroll2 = terminalExec("calix browser scroll up --amount 100 --tab-id \(tabId)")
