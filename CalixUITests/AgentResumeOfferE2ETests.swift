...
// (`Calix/App/AppDelegate.swift:1928-1952`) calls
...
// file (`$HOME/.calix/state/history/<id>.raw`, see
...
//      `create_session` (calix-session/crates/daemon/src/conn.rs:466-
...
// (Calix.app is quit and relaunched, but the daemon-held session
...
// `calix-session history on` (via a pane, see `PaneCLIExec`'s header
...
final class AgentResumeOfferE2ETests: CalixUITestCase {
    ...
    private func launchApp(autoExecute: Bool) {
        ...
        app.launchArguments = [
            ...
            "-calix.session.agentResumeEnabled", "YES",
            "-calix.session.historyPersistenceEnabled", "YES",
            "-calix.session.agentResumeAutoExecute", autoExecute ? "YES" : "NO",
        ]
    ...
    private func setUpResumableSessionAcrossRestart(autoExecute: Bool) -> String {
        ...
        // header) are REQUIRED on every pane-executed calix-session
        ...
        let historyOnOutput = paneExec(
            "\(CalixUITestCase.builtSessionBinaryPath) \(calixSessionRootFlags(homeDir: homeDir)) history on",
            counter: &execCounter
        )
        XCTAssertEqual(
            historyOnOutput, "on",
            "`calix-session history on` did not reply \"on\" (got \(historyOnOutput.debugDescription)); " +
        ...
        _ = paneExec(
            "\(CalixUITestCase.builtSessionBinaryPath) \(calixSessionRootFlags(homeDir: homeDir)) " +
        ...
        XCTAssertEqual(
            ...
            "within ~10s of `calix-session meta set`. Last ledger contents: \(metaPoll.raw)"
        )
    ...
    func test_agentResumeOffer_autoExecutesResumeCommand_whenAutoExecuteSettingIsOn() {
        ...
        panePasteAndReturn(
            "\(CalixUITestCase.builtSessionBinaryPath) \(calixSessionRootFlags(homeDir: homeDir)) kill \(sessionID)"
        )
        _ = ledger.poll(
            timeoutAttempts: 10, sleepInterval: 1,
            transform: { sessions in self.ledger.session(withID: sessionID, in: sessions) },
