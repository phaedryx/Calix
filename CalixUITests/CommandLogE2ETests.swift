...
//  (ShellIntegrationInstaller/CalixShellIntegrationEnvironment,
...
//  (`PaneCLIExec.calixSessionRootFlags`) -- there is no equivalent
//  escape hatch here: `calix.zsh`/the fish integration both read
//  `"$HOME/Library/Application Support/Calix/agent-endpoint.json"` as a
...
//  verified) that a pane's shell does NOT inherit Calix.app's own HOME
...
//  `~/Library/Application Support/Calix/` tree. This is acceptable
...
//  removal of anything pre-existing): `CALIX_UITEST_SESSION_DIR` /
//  `CALIX_UITEST_DEFAULTS_SUITE` (the base class's own isolation) still
...
//  otherwise `CalixWindowController.enableIPC` stops the MCP server
...
//  `SessionPersistenceE2ETests`'s own header on why: the `CalixUITests`
...
final class CommandLogE2ETests: CalixUITestCase {
    ...
    func test_trackedCommands_areQueryableViaTerminalMCPTools() throws {
        ...
        panePasteAndReturn("echo CALIX_CMDLOG_MARKER_A1; false")
        panePasteAndReturn("echo done")

        let encodedScript = Data(Self.queryScript.utf8).base64EncodedString()
        let queryCommand = "printf '%s' '\(encodedScript)' | base64 -d > /tmp/calix-e2e-cmdlog-query.py && " +
            "python3 /tmp/calix-e2e-cmdlog-query.py"
        // Generous timeoutAttempts: the script itself retries
        // terminal_list_commands for up to ~20s internally (the curl
        // POSTs calix.zsh's hooks fire are backgrounded+disowned, an
        ...
        let markerIndex = commands.firstIndex { ($0["command"] as? String)?.contains("CALIX_CMDLOG_MARKER_A1") == true }
        let doneIndex = commands.firstIndex { ($0["command"] as? String)?.contains("echo done") == true }
        let markerCommandIndex = try XCTUnwrap(markerIndex, "no tracked command's text contains CALIX_CMDLOG_MARKER_A1")
        ...
        XCTAssertEqual(markerCommand["exit_code"] as? Int, 1,
                       "`echo CALIX_CMDLOG_MARKER_A1; false` must report exit_code 1 -- the LAST command in the " +
        ...
        XCTAssertTrue(outputText.contains("CALIX_CMDLOG_MARKER_A1"),
    ...
    /// real `CalixWindowController.enableIPC()` -- starts a real
    /// `CalixMCPServer`, writes a real `agent-endpoint.json`, installs
    ...
    def main():
        ...
        surface_id = os.environ.get("CALIX_SURFACE_ID", "")
        if not surface_id:
            surface_id = os.environ.get("CALIX_SESSION_ID", "")
        if not surface_id:
            print(json.dumps({"error": "CALIX_SURFACE_ID and CALIX_SESSION_ID both unset in the pane"}))
            return
        ...
        for _ in range(20):
            ...
            marker_command = next(
                (c for c in list_result.get("commands", []) if "CALIX_CMDLOG_MARKER_A1" in c.get("command", "")),
                None,
            )
