...
- **Named limitation (discovered during implementation verification): cursor-agent requires a one-time manual approval.** Unlike Claude Code, Codex, and OpenCode, cursor-agent gates every MCP server behind a local approve/deny list — `cursor-agent mcp list` reports a freshly-written `calix-ipc` entry as `not loaded (needs approval)` until the user runs `cursor-agent mcp enable calix-ipc` (or approves it interactively) at least once. Verified directly: a project-scoped `.cursor/mcp.json` with our exact generated entry (no `type`, no surface-ID header) was accepted and parsed correctly by `cursor-agent mcp list` — confirming the JSON shape itself is valid — and after running `cursor-agent mcp enable calix-ipc`, `mcp list` moved to `Error: Connection failed` (expected, since nothing was listening on the test port), proving cursor-agent does attempt a real connection to our URL once approved. Calix cannot suppress or bypass this approval gate from the config file alone. This is a one-time, per-machine step for the user, not a bug in `CursorAgentConfigManager` — worth a mention in user-facing docs/release notes for this feature, but out of scope to work around in this pass.
...
- **Correction to the initial research**: no manual `project.pbxproj` editing is needed. This project uses `xcodegen` (`project.yml`, confirmed installed at `/opt/homebrew/bin/xcodegen`) — `Calix.xcodeproj/` is entirely gitignored (`.gitignore:4`) and regenerated from `project.yml`'s directory-glob `sources:` entries. A new file under `Calix/Features/IPC/` or `CalixTests/IPC/` is picked up automatically; just run `xcodegen generate` after adding it. (A prior plan in this repo, `docs/superpowers/plans/2026-08-05-tab-scoped-git-changes-panel.md`, already documents this exact correction.)
