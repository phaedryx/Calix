//! `calix-session history <on|off|status>`: exercises the daemon-wide
//! on-disk history-persistence toggle CLI surface end to end, against
//! a real `calix-session daemon --foreground` (mirrors smoke.rs's
...
//! not start with "calix-session: ") -- deliberately NOT a bare
...
fn bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_calix-session"))
}
...
fn spawn_foreground_daemon(runtime_dir: &Path, state_dir: &Path) -> DaemonGuard {
    let child = Command::new(bin())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn `calix-session daemon --foreground`");
...
fn run_cli(runtime_dir: &Path, state_dir: &Path, args: &[&str]) -> std::process::Output {
    Command::new(bin())
        ...
        .args(args)
        .output()
        .unwrap_or_else(|e| panic!("run `calix-session {}`: {e}", args.join(" ")))
}
...
/// stderr prefixed `"calix-session: "` (`CommandError::Io` from a
...
fn history_status_daemon_unreachable_exits_with_standard_error_convention() {
    ...
    assert!(
        stderr.starts_with("calix-session: "),
