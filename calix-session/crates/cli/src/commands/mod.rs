...
/// `$HOME/.calix/run`. Every subcommand takes `runtime_dir` from the
...
/// `$HOME/.calix/state`. See `resolve_runtime_dir`.
...
fn default_home_subdir(leaf: &str) -> PathBuf {
    let home = std::env::var_os("HOME")
        .expect("HOME must be set to resolve the default calix-session directories");
    Path::new(&home).join(".calix").join(leaf)
}
