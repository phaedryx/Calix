...
//! `detectShell`). A persistent pane's command is `calix-session attach
...
pub(crate) fn resolve_shell_integration_env(
    ...
    // the attach client's own env already has ZDOTDIR pointed at Calix's
    // own installed zsh dir (CalixShellIntegrationEnvironment.apply,
    ...
    // "ghostty統合が無効な環境ではzshがCalixの.zshenvを直接読み、同じコード
    // が単独で成立する" design -- Calix's own chain must stand alone
    ...
    // Calix's own zsh command-log shell integration
    // (ShellIntegrationInstaller.swift / CalixShellIntegrationEnvironment.swift)
    ...
    // Calix.app's own process env, set by CalixShellIntegrationEnvironment
    // .apply). Without this, a persistent-session daemon shell has nothing
    // to read CALIX_ZSH_ZDOTDIR from once execution reaches Calix's own
    ...
    // as `shell_integration_env`'s own gate) -- CALIX_ZSH_ZDOTDIR is a
    // zsh-specific concept, a stray env var in a bash/fish shell.
    if zsh {
        if let Some(calix_zsh_zdotdir) = get_env("CALIX_ZSH_ZDOTDIR") {
            env.push(("CALIX_ZSH_ZDOTDIR".to_string(), calix_zsh_zdotdir));
        }
    }

    // XDG_DATA_DIRS: fish's own vendor_conf.d discovery mechanism
    // (CalixShellIntegrationEnvironment.apply also appends Calix's own
    ...
    // this, a persistent-session daemon's fish shell never sees Calix's
...
mod tests {
    ...
    // ==================== P4: CALIX_ZSH_ZDOTDIR forwarding ====================
    // Calix's own zsh command-log shell integration installs its own
    // .zshenv-based restore chain (ShellIntegrationInstaller.swift /
    // CalixShellIntegrationEnvironment.swift on the Swift side), entirely
    ...
    // forwards CALIX_ZSH_ZDOTDIR into the daemon-hosted shell's own env,
    // so Calix's own restore chain (see the P4 plan's zsh ZDOTDIR relay
    ...
    #[test]
    fn resolve_shell_integration_env_forwards_calix_zsh_zdotdir_verbatim_when_present() {
        let fake_env: BTreeMap<&str, &str> = [
            ("GHOSTTY_RESOURCES_DIR", "/opt/ghostty-resources"),
            ("SHELL", "/bin/zsh"),
            ("ZDOTDIR", "/Users/alice/.config/zsh"),
            ("CALIX_ZSH_ZDOTDIR", "/Users/alice/.config/calix-zsh"),
        ]
        .into_iter()
        .collect();
        ...
        let expected: BTreeMap<String, String> = [
            ...
            (
                "CALIX_ZSH_ZDOTDIR".to_string(),
                "/Users/alice/.config/calix-zsh".to_string(),
            ),
        ]
        .into_iter()
        .collect();
        ...
        assert_eq!(
            as_map(result),
            expected,
            "CALIX_ZSH_ZDOTDIR must be forwarded verbatim alongside the existing ghostty zsh env, so \
             Calix's own zsh integration restore chain has somewhere to read from in a persistent-session \
             daemon shell -- today it is dropped entirely"
        );
    }

    #[test]
    fn resolve_shell_integration_env_does_not_forward_calix_zsh_zdotdir_for_non_zsh_shell() {
        let fake_env: BTreeMap<&str, &str> = [
            ("SHELL", "/usr/bin/fish"),
            ("CALIX_ZSH_ZDOTDIR", "/Users/alice/.config/calix-zsh"),
        ]
        ...
        assert!(
            as_map(result).get("CALIX_ZSH_ZDOTDIR").is_none(),
            "CALIX_ZSH_ZDOTDIR is a zsh-specific concept; a non-zsh $SHELL (fish here) must never \
    ...
    // attach client's own env already has ZDOTDIR pointed at Calix's own
    // installed zsh dir (CalixShellIntegrationEnvironment.apply, which
    // runs independently of ghostty), that value never reached the
    // persistent-session daemon's spawned shell. Calix's own chain is
    ...
    fn resolve_shell_integration_env_relays_client_zdotdir_verbatim_when_no_resources_dir_for_zsh() {
        let fake_env: BTreeMap<&str, &str> = [
            ("SHELL", "/bin/zsh"),
            ("ZDOTDIR", "/Users/alice/Library/Application Support/Calix/shell-integration/zsh"),
        ]
        .into_iter()
        .collect();
        ...
        assert_eq!(
            ...
            Some("/Users/alice/Library/Application Support/Calix/shell-integration/zsh"),
            "with no ghostty resources dir, Calix's own chain must still stand alone: the client's \
             own ZDOTDIR (already pointed at Calix's installed zsh dir) must relay verbatim into a \
    ...
    fn resolve_shell_integration_env_forwards_xdg_data_dirs_verbatim_alongside_zsh_env() {
        let fake_env: BTreeMap<&str, &str> = [
            ("GHOSTTY_RESOURCES_DIR", "/opt/ghostty-resources"),
            ("SHELL", "/bin/zsh"),
            ("XDG_DATA_DIRS", "/opt/calix/shell-integration:/usr/local/share:/usr/share"),
        ]
        .into_iter()
        .collect();
        ...
        assert_eq!(
            ...
            Some("/opt/calix/shell-integration:/usr/local/share:/usr/share"),
    ...
    fn resolve_shell_integration_env_forwards_xdg_data_dirs_for_fish_persistent_panes_without_resources_dir() {
        ...
        // discover Calix's vendor_conf.d entry -- was never forwarded,
        ...
        assert_eq!(
            ...
            "XDG_DATA_DIRS must forward for a fish persistent-session pane even with no ghostty \
             resources dir at all -- fixes fish persistent panes never picking up Calix's own \
             vendor_conf.d entry"
        );
    }
}
