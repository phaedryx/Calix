//! calix-session daemon: a per-user background process that owns PTY
//! sessions and lets any number of `calix-session attach` clients
...
//!   `TIOCSCTTY` + `execvp`), with `CALIX_SESSION_ID=<id>` injected into
