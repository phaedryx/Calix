...
#[test]
fn calix_session_id_env_var_cannot_be_overridden_by_spec_env() {
    ...
    let s = SessionSpec {
        ...
        argv: Some(vec![
            "/bin/sh".to_string(),
            "-c".to_string(),
            "echo \"ID=$CALIX_SESSION_ID\"".to_string(),
        ]),
        env: vec![("CALIX_SESSION_ID".to_string(), "bogus".to_string())],
        cols: 80,
        rows: 24,
    };
    ...
    assert!(
        ...
        "CALIX_SESSION_ID inside the session should be the real session id, got {output:?}"
    );
    assert!(
        !output.contains("ID=bogus"),
        "spec.env should not be able to override CALIX_SESSION_ID, got {output:?}"
    );
}
