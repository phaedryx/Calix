...
//  surface under `Calix/Features/LSP/LSPSession.swift`.
//
//  Spec entry points:
...
final class LSPSessionServerHandlersTests: XCTestCase {
    ...
    private let testWorkspaceRoot = URL(fileURLWithPath: "/tmp/calix-lsp-session-server-handlers-test")
    ...
    func test_workspaceWorkspaceFolders_returnsCurrentRoot() async throws {
        let customRoot = URL(fileURLWithPath: "/tmp/calix-lsp-session-server-handlers-test/sub-root")
