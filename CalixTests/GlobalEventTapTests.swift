...
//  CalixTests
//
//  Regression tests for the global CGEvent tap's interaction with Calix's
...
final class GlobalEventTapTests: XCTestCase {
    ...
    /// key equivalents as matching when Calix is not the active app.
    func test_shouldPassEventToMainMenu_inactiveApp_doesNotMatch() {
        ...
        XCTAssertFalse(
            matched,
            "The event should not be routed to the main menu when Calix is inactive"
        )
        XCTAssertFalse(
            target.actionInvoked,
