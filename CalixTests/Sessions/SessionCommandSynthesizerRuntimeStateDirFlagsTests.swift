...
//  the exec'd calix-session binary. That stamp existed only to make the
...
//  `--runtime-dir`/`--state-dir` flags (calix-session/crates/cli/src/cli.rs:15-20,
//  `global = true`, so every subcommand accepts them, not just `daemon`)
//  that `attach` both accepts and, when it auto-spawns the daemon,
//  forwards verbatim (calix-session/crates/cli/src/commands/attach.rs's
...
//  first word is simply the absolute calix-session binary path again,
...
//  `.shell`-variant command Calix always produces); its own single
...
//  substituting a small stub script (in place of the real calix-session
...
//  `--runtime-dir`/`--state-dir` values equal `<real $HOME>/.calix/{run,state}`
//  -- identical to the Rust CLI's own fallback
//  (calix-session/crates/cli/src/commands/mod.rs:74's
...
//  - both prepend `--runtime-dir <root>/.calix/run --state-dir
//    <root>/.calix/state` immediately before the `attach` subcommand,
...
//    `<real $HOME>/.calix/{run,state}` (regression guard against the
...
final class SessionCommandSynthesizerRuntimeStateDirFlagsTests: XCTestCase {
    ...
    func test_reattachCommand_containsNoEnvWrapperOrHomeStamp() throws {
        let binaryPath = uniqueTempPath("calix-session-dumper")
    ...
    func test_attachCommand_containsNoEnvWrapperOrHomeStamp() {
        let command = SessionCommandSynthesizer.attachCommand(
            binaryPath: "/usr/local/bin/calix-session",
            sessionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV",
            cwd: "/Users/dev/repo",
            rootResolver: FakeRootResolver(root: "/opt/calix-fixture/custom-home")
        )
    ...
    func test_reattachCommand_prependsRuntimeAndStateDirFlagsBeforeAttach_ambientHOMEIrrelevant() throws {
        let binaryPath = uniqueTempPath("calix-session-dumper")
        let outputPath = uniqueTempPath("calix-home-argv-capture")
        defer {
        ...
        try runShCEmulatingGhosttyWrapping(command, ambientHome: "/tmp/calix-wrong-ambient-home")
        let lines = readCapturedLines(at: outputPath)

        XCTAssertEqual(lines.first, "HOME=/tmp/calix-wrong-ambient-home",
                       "The exec'd calix-session process's own $HOME must be left completely untouched -- " +
        ...
        XCTAssertEqual(Array(lines.dropFirst()),
                       ["--runtime-dir", "/opt/calix-fixture/custom-home/.calix/run",
                        "--state-dir", "/opt/calix-fixture/custom-home/.calix/state",
                       ...
                       "before-subcommand order the Rust CLI itself uses (calix-session/crates/cli/src/commands/attach.rs's " +
    ...
    func test_attachCommand_prependsRuntimeAndStateDirFlagsBeforeAttach_ambientHOMEIrrelevant() throws {
        ...
        let command = SessionCommandSynthesizer.attachCommand(
            ...
            sessionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV",
            cwd: "/Users/dev/repo",
            rootResolver: FakeRootResolver(root: "/opt/calix-fixture/spawn-home")
        )
        ...
        XCTAssertEqual(Array(lines.dropFirst()),
                       ["--runtime-dir", "/opt/calix-fixture/spawn-home/.calix/run",
                        "--state-dir", "/opt/calix-fixture/spawn-home/.calix/state",
                        "attach", "01ARZ3NDEKTSV4RRFFQ69G5FAV", "--create", "--cwd", "/Users/dev/repo"],
    ...
    func test_attachCommand_defaultRootResolver_composesRealHOMECalixRunAndStateDirs() throws {
        let binaryPath = uniqueTempPath("calix-session-dumper")
        let outputPath = uniqueTempPath("calix-argv-capture")
        defer {
            try? FileManager.default.removeItem(atPath: binaryPath)
            try? FileManager.default.removeItem(atPath: outputPath)
        }
        ...
        XCTAssertEqual(Array(lines.dropFirst()),
                       ["--runtime-dir", expectedRoot + "/.calix/run",
                        "--state-dir", expectedRoot + "/.calix/state",
                        "attach", "01ARZ3NDEKTSV4RRFFQ69G5FAV", "--create", "--cwd", "/Users/dev/repo"],
                       "With no rootResolver override (real production use), the composed --runtime-dir/" +
                       "--state-dir paths must equal <real $HOME>/.calix/{run,state} -- identical to the " +
                       "Rust CLI's own default_home_subdir fallback (calix-session/crates/cli/src/commands/mod.rs:74), " +
