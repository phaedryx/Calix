...
//  `CalixWindowController.performReconnect` calls
...
//  relies on `CalixWindowController
...
//  immediate `.unreachable`, depending on whether a `calix-session`
...
//  swap, which `CalixWindowController._sessionReconnectCoordinatorForTesting`
...
final class SessionReconnectAttemptResetTimingTests: XCTestCase {
    ...
    func test_performReconnect_doesNotResetAttemptCountImmediately_uponSwappingInReplacementSurface() {
        ...
        // CALIX_SESSION_BIN env override (SessionBinaryResolver's own
        ...
        // would bail out ("No calix-session binary resolvable") before
