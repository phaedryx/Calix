...
//  binary as a single-quoted '~/.calix/bin/calix-session'. That is WRONG:
...
//  instead leaves the literal text $HOME/.calix/bin/calix-session
...
//  remote calix-session binary without needing to be shell-safe in any
//  other sense.
//
...
//  - the remote command word references $HOME/.calix/bin/calix-session
...
//    fixture directory containing a stub calix-session, actually
...
final class SessionCommandSynthesizerRemoteAttachTests: XCTestCase {
    ...
    /// `<fixtureHome>/.calix/bin/calix-session` works directly) that
    ...
    /// containing a stub calix-session (LAYER 2), and returns what that
    ...
    private func roundTripRemoteAttachCommand(
        ...
        let fixtureHome = uniqueTempPath("calix-fixture-remote-home")
        let remoteOutputPath = uniqueTempPath("calix-session-remote-argv-capture")
        defer {
        ...
        let remoteCalixSessionPath = fixtureHome + "/.calix/bin/calix-session"
        try makeSelfPathAndArgvDumperScript(at: remoteCalixSessionPath, outputPath: remoteOutputPath)
        ...
        guard let remoteSelfPath = remoteLines.first else {
            throw RoundTripCaptureFailure(description: "remote calix-session stub was never invoked " +
    ...
    func test_remoteAttachCommand_remoteCommandReferencesHomeCalixBinCalixSessionUnquoted() {
        let command = SessionCommandSynthesizer.remoteAttachCommand(
            host: "build-box.example.com", sessionID: "01ARZ3NDEKTSV4RRFFQ69G5FAV", cwd: "/home/dev/repo"
        )
        XCTAssertTrue(command.contains("$HOME/.calix/bin/calix-session"),
                       "The remote binary must be referenced as the literal text $HOME (a REMOTE-shell " +
                       "parameter expansion), never as a single-quoted '~/.calix/bin/calix-session' -- " +
        ...
        XCTAssertFalse(command.contains("'~/.calix/bin/calix-session'"),
    ...
    func test_remoteAttachCommand_execution_fullRoundTrip_remoteShellExpandsHomeAndArgvSurvivesBothLayers() throws {
        ...
        XCTAssertTrue(result.remoteSelfPath.hasSuffix("/.calix/bin/calix-session"),
    ...
    func test_remoteAttachCommand_execution_sessionIDWithSemicolonCannotInjectASecondRemoteShellStatement() throws {
        let marker = uniqueTempPath("calix-remote-injection-marker")
        ...
        XCTAssertEqual(result.remoteArgv,
                       ...
                       "sessionID containing ; must survive as ONE intact argument to calix-session on " +
    ...
    func test_remoteAttachCommand_execution_cwdWithDollarParenAndBackticksSurviveBothLayersUnexpanded() throws {
        let markerA = uniqueTempPath("calix-remote-injection-marker-a")
        let markerB = uniqueTempPath("calix-remote-injection-marker-b")
        defer {
