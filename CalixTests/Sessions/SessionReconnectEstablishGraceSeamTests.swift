...
//  comment for the bug itself): `CalixWindowController.performReconnect`
...
//  2000ms). `CalixWindowController`'s own
...
//  comment, itself citing `CalixWindowControllerFullScreenTests`), this
...
//  from the build (e.g. temporarily moved out of `CalixTests/`) while
//  running the rest of the round's RED suite, since a compile failure
//  anywhere in the `CalixTests` target fails the WHOLE target (Swift
...
//  `CalixTests/ReconnectFixture.swift` (round-18 cleanup).
//
//  Coverage:
...
final class SessionReconnectEstablishGraceSeamTests: XCTestCase {
    ...
    /// `CalixWindowControllerReconnectGraceOverrides` does not exist
