//
//  CalixWindowControllerRemoteReconnectCommandTests.swift
//  CalixTests
//
//  P5 (remote sessions) RED phase, contract R3 (COMPILE-RED, HELD-OUT
//  FILE): CalixWindowController.performReconnect(oldSurfaceID:sessionID:)
//  (CalixWindowController.swift near :2378) always synthesizes its
...
//
//    #if DEBUG
//    extension CalixWindowController {
...
//  _performReconnectSurfaceCreationHookForTesting, the CALIX_SESSION_BIN/
//  CALIX_SSH_BIN env sentinel convention from
...
@MainActor
final class CalixWindowControllerRemoteReconnectCommandTests: XCTestCase {
    ...
    private func withBinarySentinels(
        ...
        let originalLocal = ProcessInfo.processInfo.environment["CALIX_SESSION_BIN"]
        let originalSSH = ProcessInfo.processInfo.environment["CALIX_SSH_BIN"]
        setenv("CALIX_SESSION_BIN", local, 1)
        setenv("CALIX_SSH_BIN", ssh, 1)
        defer {
            if let originalLocal { setenv("CALIX_SESSION_BIN", originalLocal, 1) } else { unsetenv("CALIX_SESSION_BIN") }
            if let originalSSH { setenv("CALIX_SSH_BIN", originalSSH, 1) } else { unsetenv("CALIX_SSH_BIN") }
        }
        try body()
    }

    func test_performReconnect_localSession_synthesizesReattachCommand_notSSH() throws {
        let localBin = "/tmp/calix-session-sentinel-\(UUID().uuidString)"
    ...
    func test_performReconnect_remoteSession_synthesizesSSHCommand_notLocalReattach() throws {
        ...
        try withBinarySentinels(local: localBin, ssh: sshBin) {
            ...
            XCTAssertTrue(command.hasPrefix("'\(sshBin)"),
                          ...
                          "whose first word is the ssh binary, never the local calix-session binary")
            XCTAssertFalse(command.contains(localBin), "A remote reconnect must never invoke the local calix-session binary directly")
