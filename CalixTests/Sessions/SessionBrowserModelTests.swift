...
//  `calix-session` process, matching this codebase's direct-query test
...
/// no real `calix-session` binary involved.
...
final class SessionBrowserModelTests: XCTestCase {
    ...
    /// `calix-session/crates/daemon/src/ledger.rs`'s retention GC), so a
