//
//  CalixWindowControllerRemoteKillRoutingTests.swift
//  CalixTests
//
//  P5 (remote sessions) RED phase, BUG 1 (five-angle convergence review
//  finding), contract 1b (controller level): CalixWindowController
...
//  the LOCAL-only operation, silently orphaning the remote calix-session
//  forever.
//
//  WHERE HOST LIVES AT KILL TIME: exactly like performReconnect's own
//  identical read (see CalixWindowControllerRemoteReconnectCommandTests'
...
@MainActor
final class CalixWindowControllerRemoteKillRoutingTests: XCTestCase {
    ...
    func test_sessionKill_remoteSessionRef_routesObserverWithGivenHost() throws {
        ...
        XCTAssertEqual(result.host, "devbox.example.com",
                       ...
                       "orphans the remote calix-session today")
    }
