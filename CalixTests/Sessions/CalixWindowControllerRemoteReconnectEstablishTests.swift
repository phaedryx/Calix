//
//  CalixWindowControllerRemoteReconnectEstablishTests.swift
//  CalixTests
//
...
//  SessionDaemonClient.shared -- the LOCAL calix-session daemon)
...
//  driveReconnect shape (CalixWindowControllerReconnectGraceOverrides
...
//  the CALIX_SESSION_BIN env sentinel, pumpRunLoop), plus
...
@MainActor
final class CalixWindowControllerRemoteReconnectEstablishTests: XCTestCase {
    ...
    func test_performReconnect_remoteSession_establishesFromSurfaceSurvivalAlone_withoutDaemonProbe() throws {
        ...
        setenv("CALIX_SESSION_BIN", localBin, 1)
        defer {
            if let originalBinPath { setenv("CALIX_SESSION_BIN", originalBinPath, 1) } else { unsetenv("CALIX_SESSION_BIN") }
        }
