...
//  calix-agent-hook script, run as a real child Process, POSTing into a
//  real running CalixMCPServer. This isolates the one remaining gap in
//  the persistent-session pane hook path: CalixMCPServer's
//  /agent-event routing already resolves a calix-session ID via
//  SessionSurfaceMap (see CalixMCPServerSessionRoutingTests), and the
//  script's header value already prefers CALIX_SESSION_ID (see
//  AgentHookScriptSessionIDTests) — but the script's own fail-open
//  guard still only checks CALIX_SURFACE_ID, so a pane that somehow has
//  only CALIX_SESSION_ID set exits before ever sending anything.
//
//  Coverage:
//  - With only CALIX_SESSION_ID set (CALIX_SURFACE_ID left completely
...
final class AgentHookScriptSessionIDPipelineTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        appSupportDir = tempHome + "/Library/Application Support/Calix"
    ...
    /// Runs the installed `calix-agent-hook` script as a real child
    /// process (`/bin/sh <script>`), piping `stdinJSON` to its stdin.
    /// `sessionID` (when non-nil) sets `CALIX_SESSION_ID` alone —
    /// `CALIX_SURFACE_ID` is deliberately left completely unset, unlike
    /// every real production invocation (which always has
    /// CALIX_SURFACE_ID set), specifically to isolate whether the
    ...
    private func runHookScript(stdinJSON: String, sessionID: String?) throws -> Int32 {
        ...
        if let sessionID {
            env["CALIX_SESSION_ID"] = sessionID
        }
    ...
    // MARK: - Only CALIX_SESSION_ID set

    func test_realHookScript_onlyCalixSessionIDSet_stillExitsZeroAndForwardsEvent() async throws {
        let calixSessionID = "01ARZ3NDEKTSV4RRFFQ69G5FAV"
        let resolvedSurfaceID = UUID()
        server.sessionSurfaceMap.register(sessionID: calixSessionID, surfaceID: resolvedSurfaceID)

        let stdin = """
        {"session_id":"pipeline-session","cwd":"/Users/dev/pipeline-repo","hook_event_name":"SessionStart"}
        """

        let exitCode = try runHookScript(stdinJSON: stdin, sessionID: calixSessionID)

        XCTAssertEqual(exitCode, 0, "The hook script must always exit 0, with only CALIX_SESSION_ID set or otherwise")

        let entry = await waitForEntry(surfaceID: resolvedSurfaceID)
        XCTAssertNotNil(entry,
                        "With only CALIX_SESSION_ID set, the script must still attempt the POST — the " +
                        "current guard (which only checks CALIX_SURFACE_ID) exits before ever sending " +
    ...
    func test_realHookScript_neitherSurfaceIDNorSessionIDSet_exitsZeroWithoutForwarding() async throws {
        ...
        XCTAssertTrue(registry.entries.isEmpty,
                     "With neither CALIX_SURFACE_ID nor CALIX_SESSION_ID set, nothing must be forwarded")
    }
}
