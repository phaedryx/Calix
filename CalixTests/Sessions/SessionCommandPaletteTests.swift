...
//  registered by `CalixWindowController.setupCommandRegistry`, gated
...
final class SessionCommandPaletteTests: XCTestCase {
    ...
    /// Build a `CalixWindowController` whose active tab has exactly one
    ...
    private func makeControllerWithFocusedUntrackedPane() -> CalixWindowController {
    ...
    private func command(_ id: String, in controller: CalixWindowController) throws -> PaletteCommand {
        try XCTUnwrap(
    ...
    /// .shared.showBrowser(), CalixWindowController.swift ~541) -- from
    ...
    /// `CalixWindowControllerFocusSyncTests`'s `_testInsert`-backed
    /// two-pane fixture — no live ghostty app needed.
    private struct MultiPaneFixture {
        let controller: CalixWindowController
        let tab: Tab
    ...
    /// `CalixWindowController._closingTabIDsForTesting`'s state at the
    ...
    private final class ClosingTabIDsSpyAppDelegate: ConfirmQuitMockAppDelegate {
        weak var controller: CalixWindowController?
    ...
    // `CalixWindowControllerCloseArmsTests`) must set
