...
//  calix-session-backed persistent shell (.persistent), gated by
...
//  for the real calix-session binary (via an injected
...
final class SessionSpawnPlannerTests: XCTestCase {
    ...
    private let settingsSuiteName = "com.calix.tests.SessionSpawnPlannerTests"
    ...
    /// stub (standing in for the real calix-session binary), runs the
