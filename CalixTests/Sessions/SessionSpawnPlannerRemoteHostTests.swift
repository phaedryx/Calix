...
//  gates on the LOCAL calix-session binary being resolvable via
...
//      entirely on the remote machine; the local calix-session binary's
...
final class SessionSpawnPlannerRemoteHostTests: XCTestCase {
    ...
    private let settingsSuiteName = "com.calix.tests.SessionSpawnPlannerRemoteHostTests"
    ...
    func test_plan_remoteHost_noLocalBinaryResolvable_stillReturnsPersistentWithSSHCommand() {
        ...
        // FakeBinaryResolver(path: nil) simulates "no local calix-session
        ...
        guard case .persistent(let sessionID, let command, _) = plan else {
            XCTFail("A remote-host context must produce .persistent even when no local calix-session binary is resolvable")
            return
        }
