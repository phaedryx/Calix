...
//  the reconnect grace task (`CalixWindowController.performReconnect`'s
...
//    #endif
//
//  `CalixWindowController.performReconnect`'s grace `Task` would consult
...
//  and `CalixWindowControllerFullScreenTests`), this file is expected to
...
//  the build (e.g. temporarily moved out of `CalixTests/`) while running
//  the rest of the round's RED suite, since a compile failure anywhere
//  in the `CalixTests` target fails the WHOLE target (Swift compiles a
...
//  drives a real `performReconnect` with `CALIX_SESSION_BIN=/usr/bin/true`
...
//  the `CALIX_SESSION_BIN` env override, the inactive-second-tab fixture
...
//  themselves now live in the shared `CalixTests/ReconnectFixture.swift`
...
final class SessionReconnectGracePositiveSignalSeamTests: XCTestCase {
    ...
    override func tearDown() {
        CalixWindowControllerReconnectGraceOverrides.reconnectEstablishGraceMilliseconds = nil
    ...
    private func driveReconnect(
        ...
        CalixWindowControllerReconnectGraceOverrides.reconnectEstablishGraceMilliseconds = 30
    ...
    func test_probeReportsNotEstablished_attemptCountStaysUnchanged() {
        let originalBinPath = ProcessInfo.processInfo.environment["CALIX_SESSION_BIN"]
        setenv("CALIX_SESSION_BIN", "/usr/bin/true", 1)
        defer {
            if let originalBinPath {
                setenv("CALIX_SESSION_BIN", originalBinPath, 1)
            } else {
                unsetenv("CALIX_SESSION_BIN")
            }
        }
