...
//    calix-session/crates/cli/src/cli.rs, which has no --id flag)
...
//  calix-session binary, rather than string-matching
...
final class SessionCommandSynthesizerTests: XCTestCase {
    ...
    /// the real (nonexistent in tests) calix-session binary so the
    ...
    func test_attachCommand_basicForm_includesExecCreateIdCwdInOrder() throws {
        ...
        // now (calix-session/crates/cli/src/cli.rs:15-20), prepended
        ...
        XCTAssertEqual(argv, ["--runtime-dir", expectedRoot + "/.calix/run",
    ...
    func test_attachCommand_withName_appendsNameArgumentIntact() throws {
        ...
        XCTAssertEqual(
            argv,
            ["--runtime-dir", expectedRoot + "/.calix/run", "--state-dir", expectedRoot + "/.calix/state",
    ...
    func test_attachCommand_binaryPathWithSpace_isInvokedCorrectly() throws {
        let dirWithSpace = FileManager.default.temporaryDirectory
            .appendingPathComponent("calix dumper dir \(UUID().uuidString)").path
        try FileManager.default.createDirectory(atPath: dirWithSpace, withIntermediateDirectories: true)
        let binaryPath = dirWithSpace + "/calix-session"
        let outputPath = uniqueTempPath("calix-argv-capture")
        defer {
    ...
    private func makeMarkerTouchingScript(markerPath: String) throws -> String {
        let scriptPath = uniqueTempPath("calix-injection-script")
    ...
    private func assertCannotInject(
        ...
        let marker = uniqueTempPath("calix-injection-marker")
