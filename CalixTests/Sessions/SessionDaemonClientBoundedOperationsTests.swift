...
//  calix-session daemon can stall the reconnect decision indefinitely,
...
//  Process (SIGTERM), cancelling a slow `calix-session kill` mid-IPC-write
...
//  CalixWindowControllerFullScreenTests), this file is expected to FAIL
...
//  in for a calix-session subprocess that never exits, so a passing run
...
/// never resumes, standing in for a calix-session subprocess (or
