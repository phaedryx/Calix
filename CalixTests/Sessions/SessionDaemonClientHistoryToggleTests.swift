...
//  calix-session/crates/cli/src/commands/history.rs's own P6 RED2 header
...
//    ["--runtime-dir", "<root>/.calix/run"] before ["history", "on"/"off"],
...
final class SessionDaemonClientHistoryToggleTests: XCTestCase {
    ...
    func test_setHistoryEnabledTrue_prependsRuntimeDirBeforeHistoryOn() async {
        ...
        XCTAssertEqual(
            ...
            ["--runtime-dir", "/opt/calix-fixture/custom-home/.calix/run", "history", "on"],
            "setHistoryEnabled(true) must prepend --runtime-dir <root>/.calix/run before its history on " +
    ...
    func test_setHistoryEnabledFalse_prependsRuntimeDirBeforeHistoryOff() async {
        ...
        XCTAssertEqual(
            recorded.first?.arguments,
            ["--runtime-dir", "/opt/calix-fixture/custom-home/.calix/run", "history", "off"],
            "setHistoryEnabled(false) must prepend --runtime-dir <root>/.calix/run before its history off " +
            "subcommand arguments"
        )
    }
}
...
final class SessionDaemonClientHistoryToggleCancellationShieldTests: XCTestCase {
    ...
    func test_setHistoryEnabled_shieldsRunnerFromHostTaskCancellation() async {
        let resolver = FixedBinaryResolver(path: "/opt/calix-fixture/bin/calix-session")
