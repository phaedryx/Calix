...
//  turning (host, bundled resource paths) into the calix-session CLI's
//  own `remote-install` argv (calix-session/crates/cli/src/commands/remote_install.rs,
...
//    - Resources/session-remote/<x86_64|aarch64>/calix-session -- the
//      cross-compiled Linux musl payloads (cycle 4)
//    - Resources/bin/calix-session -- the LOCAL calix-session binary,
...
final class RemoteInstallArgvBuilderTests: XCTestCase {
    ...
    func test_buildArgv_includesEveryFlagWhenEveryPathIsGiven() {
        let argv = RemoteInstallArgvBuilder.buildArgv(
            host: "devbox",
            payloadX86_64Path: "/bundle/session-remote/x86_64/calix-session",
            payloadAarch64Path: "/bundle/session-remote/aarch64/calix-session",
            hostBinaryPath: "/bundle/bin/calix-session",
            terminfoPath: "/bundle/terminfo/78/xterm-ghostty"
        )

        XCTAssertEqual(argv, [
            "remote-install", "devbox",
            "--payload-x86-64", "/bundle/session-remote/x86_64/calix-session",
            "--payload-aarch64", "/bundle/session-remote/aarch64/calix-session",
            "--host-binary", "/bundle/bin/calix-session",
...
final class SessionDaemonClientInstallRemoteTests: XCTestCase {
    ...
    func test_installRemote_runsLocalBinaryWithBuiltArgv() async {
        ...
        let client = SessionDaemonClient(
            resolver: FixedBinaryResolver(path: "/opt/calix-fixture/bin/calix-session"),
            commandRunner: runner,
            payloadResolver: FakePayloadResolver(
                x86_64Path: "/bundle/session-remote/x86_64/calix-session",
                aarch64Path: "/bundle/session-remote/aarch64/calix-session",
        ...
        XCTAssertEqual(recorded[0].executable, "/opt/calix-fixture/bin/calix-session",
                       "installRemote must invoke the LOCAL bundled calix-session binary as the executable")
        XCTAssertEqual(recorded[0].arguments, [
            "remote-install", "devbox.example.com",
            "--payload-x86-64", "/bundle/session-remote/x86_64/calix-session",
            "--payload-aarch64", "/bundle/session-remote/aarch64/calix-session",
            "--host-binary", "/opt/calix-fixture/bin/calix-session",
            "--terminfo", "/bundle/terminfo/78/xterm-ghostty",
    ...
    func test_installRemote_noLocalBinaryResolvable_returnsNilWithoutRunningAnything() async {
        ...
        XCTAssertNil(result, "With no local calix-session binary resolvable, installRemote must return " +
