...
//  calix-session daemon/CLI resolves its on-disk state root
//  ($HOME/.calix: run/sessiond.sock, state/) from the literal HOME env
//  var at process start (see calix-session/crates/daemon/src/session.rs),
...
//  Calix.app itself has, and neither consults FileManager
//  .homeDirectoryForCurrentUser (which ignores HOME entirely -- proven
//  live: an app launched with HOME=/tmp/x still wrote ~/.calix/sessions.json
...
//  `SessionDaemonClient` agree on the calix-session binary path) so a
...
//  itself citing CalixWindowControllerFullScreenTests), this file is
...
//    present-but-empty-string handling for CALIX_SESSION_BIN)
//
...
final class SessionRootResolverTests: XCTestCase {
    ...
    func test_resolve_returnsInjectedHOMEValue_whenPresentAndNonEmpty() {
        let resolver = SessionRootResolver(environment: ["HOME": "/opt/calix-fixture/custom-home"])

        XCTAssertEqual(resolver.resolve(), "/opt/calix-fixture/custom-home",
                       "With HOME present in the injected environment, resolve() must return exactly that " +
                       "value -- this is the literal env var calix-session's own daemon/CLI resolves its " +
    ...
    func test_resolve_fallsBackToNSHomeDirectory_whenHOMEPresentButEmpty() {
        ...
        XCTAssertEqual(resolver.resolve(), NSHomeDirectory(),
                       ...
                       "handling for CALIX_SESSION_BIN")
    }
}
