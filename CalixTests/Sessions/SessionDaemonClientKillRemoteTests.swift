...
//  finding): CalixWindowController.killSessionIfPersistent never
...
//  shells `ssh -- <host> "$HOME/.calix/bin/calix-session kill
...
//    argv exactly ["--", host, "$HOME/.calix/bin/calix-session kill
//    '<sessionID>'"] -- no -t flag
...
final class SessionDaemonClientKillRemoteTests: XCTestCase {
    ...
    func test_killRemote_shellsSSHWithHostAndRemoteKillCommand_noPTYFlag() async {
        ...
        let client = SessionDaemonClient(
            ...
            sshResolver: FakeSSHResolver(path: "/opt/calix-fixture/bin/ssh")
        )
        ...
        XCTAssertEqual(recorded[0].executable, "/opt/calix-fixture/bin/ssh",
                       "killRemote must invoke the resolved ssh binary as the executable, never the local " +
                       "calix-session binary")
        XCTAssertEqual(recorded[0].arguments, [
            "--", "devbox.example.com",
            "$HOME/.calix/bin/calix-session kill '01ARZ3NDEKTSV4RRFFQ69G5FAV'",
    ...
    func test_killRemote_sessionIDContainingSingleQuote_isEscaped() async {
        ...
        XCTAssertEqual(recorded.first?.arguments.last,
                       "$HOME/.calix/bin/calix-session kill '01ARZ3'\\''; touch pwned; '\\'''",
    ...
    func test_killRemote_noLocalBinaryResolvable_stillInvokesSSH() async {
        ...
        XCTAssertEqual(recorded.count, 1,
                       ...
                       "killRemote only ever needs the ssh binary -- the local calix-session binary's " +
