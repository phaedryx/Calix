...
//       (see CalixWindowControllerLastWindowCloseSaveTests, already
...
//  fixture controllers (mirrors CalixWindowControllerLastWindowCloseSaveTests'
...
final class AppDelegatePendingTerminationSnapshotTests: XCTestCase {
    ...
    private func makeSessionDirActor() throws -> SessionPersistenceActor {
        ...
        setenv("CALIX_UITEST_SESSION_DIR", dir.path, 1)
        addTeardownBlock {
    ...
    /// CalixWindowControllerLastWindowCloseSaveTests.makeFixture()
    /// exactly, parameterized by title so multiple fixture controllers
    /// are distinguishable in a captured snapshot.
    private func makeFixtureController(title: String) -> CalixWindowController {
