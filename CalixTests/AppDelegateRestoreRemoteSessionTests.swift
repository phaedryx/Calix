...
//  ssh, not the local calix-session binary, no --runtime-dir/--state-dir
...
//  never hit the "no calix-session binary resolvable" degrade branch
...
//  suite (a compile failure anywhere fails the whole CalixTests target,
...
//    reattachCommand (regression guard: the CALIX_SESSION_BIN env
...
//    local calix-session env sentinel never appears at all.
//
...
final class AppDelegateRestoreRemoteSessionTests: XCTestCase {
    ...
    /// calix-session binary and the ssh binary, so the two synthesized
    ...
    func test_restoreTabSurfaces_localSessionRef_synthesizesReattachCommand_notSSH() throws {
        ...
        try withBinarySentinels(local: localBin, ssh: sshBin) {
            ...
            XCTAssertTrue(command.hasPrefix("'\(localBin)"),
                          ...
                          "is the local calix-session binary -- regression guard for existing, unchanged behavior")
    ...
    func test_restoreTabSurfaces_remoteSessionRef_synthesizesSSHCommand_notLocalReattach() throws {
        ...
        try withBinarySentinels(local: localBin, ssh: sshBin) {
            ...
            XCTAssertTrue(command.hasPrefix("'\(sshBin)"),
                          ...
                          "word is the ssh binary, never the local calix-session binary")
            XCTAssertFalse(command.contains(localBin),
                           "A remote restore must never invoke the local calix-session binary directly")
