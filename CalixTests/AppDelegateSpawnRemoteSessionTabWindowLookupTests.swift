...
//  `window?.makeKeyAndOrderFront(nil)`), not any `CalixWindowController`'s
...
//  even when a main Calix window is already open -- the exact same class
...
//  the whole CalixTests target before writing this file).
//
...
//  constructs a real `CalixWindowController` and calls `showWindow(nil)`
...
final class AppDelegateSpawnRemoteSessionTabWindowLookupTests: XCTestCase {
    ...
    private func makeController(title: String) -> CalixWindowController {
    ...
    func test_spawnRemoteSessionTab_withOneAvailableWindowController_addsATabToItInsteadOfOpeningNewWindow() {
        ...
        var addTabTargets: [CalixWindowController] = []
