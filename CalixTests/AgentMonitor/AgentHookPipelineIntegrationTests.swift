...
//  pipeline: calix-agent-hook (a real /bin/sh script, run as a real
//  child Process) -> a real CalixMCPServer bound to a real loopback
//  port -> a real AgentRegistry.
//
//  Every individual layer of this pipeline (AgentEvent.decode,
//  CalixMCPServer.routeAgentEvent, AgentRegistry.handleHookEvent) already
...
//  - CALIX_SURFACE_ID unset (a plain, non-Calix terminal) never reaches
//    the registry
...
final class AgentHookPipelineIntegrationTests: XCTestCase {
    ...
    /// `surfaceID` is set as `CALIX_SURFACE_ID` when non-nil; when nil
    /// the variable is left entirely unset, matching a plain
    /// (non-Calix) terminal invocation.
    @discardableResult
    private func runHookScript(stdinJSON: String, surfaceID: UUID?) throws -> Int32 {
        ...
        if let surfaceID {
            env["CALIX_SURFACE_ID"] = surfaceID.uuidString
        }
    ...
    // MARK: - CALIX_SURFACE_ID unset (plain terminal)
    ...
    func test_realHookScript_surfaceIDUnset_leavesRegistryUntouched() async throws {
        ...
        // CALIX_SURFACE_ID is set, proving the assertion below is caused
        ...
        XCTAssertNotNil(sanityEntry, "Precondition: the pipeline must register an entry when CALIX_SURFACE_ID is set")

        let exitCode = try runHookScript(stdinJSON: stdin, surfaceID: nil)

        XCTAssertEqual(exitCode, 0)
        XCTAssertEqual(registry.entries.count, 1,
                       "A hook invocation with no CALIX_SURFACE_ID must never reach the registry " +
