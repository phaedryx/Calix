...
//  calix-approval-hook pipeline: the real INSTALLED script, run as a
//  real child `/bin/sh` process, POSTing into a real `CalixMCPServer`
...
//  CalixMCPServerApprovalRequestTests) plus one addition that harness
...
//  string contract, `CalixMCPServerApprovalRequestTests` drives
...
//  and CALIX_SURFACE_ID/CALIX_SESSION_ID both unset never even attempts
...
final class ApprovalHookPipelineIntegrationTests: XCTestCase {
    ...
    private let settingsSuiteName = "com.calix.tests.ApprovalHookPipelineIntegrationTests"
    ...
    /// Runs the installed `calix-approval-hook` script as a real child
    ...
    ///
    /// `surfaceID` sets `CALIX_SURFACE_ID` when non-nil; `nil` (used only
    /// by the missing-surface-env test) leaves it entirely unset,
    /// matching a plain (non-Calix) terminal invocation. `kindArgument`,
    ...
    func test_missingSurfaceEnv_printsNothing_neverContactsServer() async throws {
        ...
        XCTAssertTrue(approvalInbox.pending.isEmpty,
                      "with neither CALIX_SURFACE_ID nor CALIX_SESSION_ID set, the script must exit " +
