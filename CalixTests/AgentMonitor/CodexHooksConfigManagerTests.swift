...
//  calix-agent-hook script. Follows the tempDir + configPath injection
...
//    strip the BEGIN line plus everything recognizable as Calix's own
...
final class CodexHooksConfigManagerTests: XCTestCase {
    ...
    private static let beginLine = "# BEGIN CALIX AGENT HOOKS (managed by Calix, do not edit)"
    private static let endLine = "# END CALIX AGENT HOOKS"
    ...
    func test_installHooks_newFile_writesAllSixEventPairsWithCommandEntry() throws {
        ...
        XCTAssertEqual(occurrences(of: expectedApprovalCommandLine, in: content), 1,
                       "PreToolUse's approval entry must POST through calix-approval-hook with the codex argv")
    ...
    func test_removeHooks_removesManagedBlockOnlyAndRestoresUserContent() throws {
        ...
        for eventName in Self.expectedEvents {
            XCTAssertFalse(content.contains("[[hooks.\(eventName)]]"),
                           "\(eventName)'s Calix entry must be removed")
        }
    }
    ...
    func test_installHooks_orphanBeginMarker_selfHealsWithoutDuplicating() throws {
        ...
        // strips a stale/partial set of Calix's own entries — identified by
        // their `command` line referencing calix-agent-hook's own script
        // filename — so a reinstall produces exactly one clean block.
        let staleScriptPath = tempDir + "/old-install/calix-agent-hook"
        let existing = """
        profile = "default"
        \(Self.beginLine)
        [[hooks.SessionStart]]
    ...
    // isCalixGeneratedBlockLine must also recognize a command line
    // referencing calix-approval-hook (not just calix-agent-hook), so an
    // orphan BEGIN left over from a Calix version that already installed
    ...
    func test_installHooks_orphanBeginMarker_withStaleApprovalCommand_selfHeals() throws {
        let staleApprovalPath = tempDir + "/old-install/calix-approval-hook"
        let existing = """
        profile = "default"
        \(Self.beginLine)
        [[hooks.PreToolUse]]
    ...
    func test_removeHooks_orphanBeginImmediatelyFollowedByUserContent_onlyStripsOwnBlockLines() throws {
        ...
        // remove the BEGIN line plus lines that look like Calix's own
    ...
    // broke hooks installation entirely in that setup. Calix now follows
    ...
    func test_installHooks_scriptPathContainingSingleQuote_throws() {
        let badScriptPath = tempDir + "/it's-a-path/calix-agent-hook"
    ...
    func test_installHooks_approvalScriptPathContainingSingleQuote_throws() {
        let badApprovalScriptPath = tempDir + "/it's-a-path/calix-approval-hook"
