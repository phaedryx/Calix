...
//  (snake_case) decoding for each hook_event_name Calix observes.
//
//  Coverage:
...
final class AgentEventTests: XCTestCase {
    ...
    //
    // A PreToolUse for one of Calix's own mcp__calix-ipc__* tools carries
    ...
    func test_decode_preToolUse_broadcast_extractsIpcSelfPeerIDFromFrom() throws {
        let data = json("""
        {
            "session_id": "abc-123",
            "hook_event_name": "PreToolUse",
            "tool_name": "mcp__calix-ipc__broadcast",
            "tool_input": {
    ...
    func test_decode_preToolUse_receiveMessages_extractsIpcSelfPeerIDFromPeerID() throws {
        let data = json("""
        {
            "session_id": "abc-123",
            "hook_event_name": "PreToolUse",
            "tool_name": "mcp__calix-ipc__receive_messages",
            "tool_input": {
    ...
    func test_decode_preToolUse_nonCalixIPCToolAndMissingToolInput_ipcSelfPeerIDIsNil() throws {
        // Sanity: a calix-ipc send_message PreToolUse must actually
        ...
        """)
        let calixEvent = try XCTUnwrap(AgentEvent.decode(from: calixData))
        XCTAssertEqual(calixEvent.ipcSelfPeerID, "66666666-6666-6666-6666-666666666666",
                       "Precondition: a calix-ipc PreToolUse must populate ipcSelfPeerID")
        ...
        XCTAssertNil(bashEvent.ipcSelfPeerID,
                    "A non-calix-ipc tool_name must never populate ipcSelfPeerID, even with an unrelated tool_input")

        let missingInputData = json("""
        { "hook_event_name": "PreToolUse", "tool_name": "mcp__calix-ipc__send_message" }
        """)
        let missingInputEvent = try XCTUnwrap(AgentEvent.decode(from: missingInputData))
        XCTAssertNil(missingInputEvent.ipcSelfPeerID,
                    "A calix-ipc tool_name with no tool_input at all must decode ipcSelfPeerID as nil, not crash")
    }
}
