...
//  CalixTests
//
//  TDD Red Phase for ApprovalHookScript: the `calix-approval-hook` script
//  installed alongside `calix-agent-hook` (see AgentHookScriptTests.swift,
//  which this mirrors). Unlike calix-agent-hook's fire-and-forget async
...
//
//  Coverage:
//  - scriptBody guards on both CALIX_SURFACE_ID and CALIX_SESSION_ID being
...
final class ApprovalHookScriptTests: XCTestCase {
    ...
    func test_fileName_isCalixApprovalHook() {
        XCTAssertEqual(ApprovalHookScript.fileName, "calix-approval-hook")
    }
    ...
    func test_scriptBody_guardsOnMissingSurfaceAndSessionID() {
        ...
        XCTAssertTrue(body.contains("[ -z \"$CALIX_SURFACE_ID\" ] && [ -z \"$CALIX_SESSION_ID\" ]"),
                     "Script must guard on BOTH CALIX_SURFACE_ID and CALIX_SESSION_ID being unset " +
                     "(a plain, non-Calix-launched terminal must be unaffected)")
    }
    ...
    func test_scriptBody_sendsSurfaceIDHeaderPreferringSessionID() {
        XCTAssertTrue(ApprovalHookScript.scriptBody.contains("X-Calix-Surface-ID: ${CALIX_SESSION_ID:-$CALIX_SURFACE_ID}"),
                     "Script must send X-Calix-Surface-ID, preferring CALIX_SESSION_ID (survives " +
                     "ghostty surface reconnect) over CALIX_SURFACE_ID")
    }

    func test_scriptBody_sendsAgentKindHeader() {
        XCTAssertTrue(ApprovalHookScript.scriptBody.contains("X-Calix-Agent-Kind: $kind"),
                     "Script must send the X-Calix-Agent-Kind header so the server can attribute " +
    ...
    func test_scriptBody_failSafe_claudePrintsAskJSON() {
        ...
        let expectedJSON = "{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"ask\"," +
            "\"permissionDecisionReason\":\"Calix approval inbox unavailable\"}}"
