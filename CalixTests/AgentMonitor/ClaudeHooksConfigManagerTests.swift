...
//  ~/.claude/settings.json for the calix-agent-hook script, following the
...
//  - removeHooks removes only Calix's own entries, identified by command
...
final class ClaudeHooksConfigManagerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        scriptPath = tempDir + "/bin/calix-agent-hook"
        approvalScriptPath = tempDir + "/bin/calix-approval-hook"
    }
    ...
    //
    // Stage C: PreToolUse alone now carries a SECOND Calix-owned command
    ...
    func test_installHooks_preservesExistingUserHooksAndOtherTopLevelKeys() throws {
        ...
        let calixGroup = preToolUseGroups.first { ($0["matcher"] as? String) == "*" }
        XCTAssertNotNil(calixGroup, "Calix's own '*' matcher group must be added")
    ...
    func test_installHooks_reinstall_isIdempotent() throws {
        ...
        for eventName in Self.expectedEvents {
            ...
            XCTAssertEqual(commands.count, expectedCount,
                           "\(eventName) must contain exactly \(expectedCount) Calix hook entry(ies) " +
    ...
    // Stage C: PreToolUse's group now carries two Calix-owned command
    ...
    // until Calix's own /approval-request long-poll resolves.
    func test_installHooks_preToolUse_writesMonitorAndApprovalEntriesWithCorrectShapes() throws {
        ...
        XCTAssertEqual(commands.count, 2, "PreToolUse must have exactly two Calix command entries")
    ...
    // Upgrade path: a config written by a pre-Stage-C Calix version has
    ...
    // isOwnCommandEntry must recognize calix-approval-hook's own filename
    ...
    func test_removeHooks_stripsApprovalOnlyEntryByCommandPath() throws {
        ...
        let dict = try readConfigDict()
        let commands = commandEntries(hookGroups(dict, event: "PreToolUse"))
        XCTAssertEqual(commands.count, 1, "Only Calix's own approval entry should be removed")
    ...
    func test_removeHooks_removesOnlyOwnEntriesByCommandPath() throws {
        let calixCommand = "\"\(scriptPath!)\""
        let existingConfig: [String: Any] = [
            "hooks": [
                "Stop": [
                    ["hooks": [
                        ["type": "command", "command": calixCommand, "timeout": 5, "async": true],
        ...
        XCTAssertEqual(stopCommands.count, 1,
                       "Only Calix's own Stop hook entry should be removed; the co-located user hook must remain")
        ...
        XCTAssertTrue(sessionStartCommands.isEmpty,
                      "Calix's only SessionStart hook entry should leave the group empty")
    ...
    // installHooks must add Calix's own entry without disturbing it, and
    // removeHooks must take only Calix's entry back out.
    func test_installThenRemove_permissionRequest_preservesUserOwnHookAndRemovesOnlyCalixEntry() throws {
        let existingConfig: [String: Any] = [
            "hooks": [
                "PermissionRequest": [
                    ["hooks": [
        ...
        XCTAssertEqual(installedCommands.count, 2,
                       "installHooks must add Calix's own PermissionRequest entry alongside the " +
        ...
        XCTAssertEqual(remainingCommands.count, 1,
                       "removeHooks must remove only Calix's own PermissionRequest entry")
    ...
    func test_removeHooks_dropsEmptyEventKeysAndHooksKeyWhenAllEmpty() throws {
        // The 8 events installHooks writes only ever contain Calix's own
    ...
    // real-environment reproduction). Calix now follows the link and
    ...
    func test_installHooks_createsBackupOfExistingFile() throws {
        ...
        XCTAssertFalse(bakContent.contains("calix-agent-hook"),
                       "Backup must contain the pre-install content, not the newly added Calix hooks")
    }
    ...
    func test_removeHooks_createsBackupOfPreRemovalContent() throws {
        ...
        let bakContent = try String(contentsOfFile: bakPath, encoding: .utf8)
        XCTAssertTrue(bakContent.contains("calix-agent-hook"),
    ...
    func test_installHooks_unrecognizedEventShape_isLeftUntouched() throws {
        ...
        // replaced with just Calix's own group.
        let existingConfig: [String: Any] = [
            "hooks": [
    ...
    func test_installHooks_dotfilesSymlinkedSettingsJSON_writesRealFilePreservesUserHooksAndKeepsLinkIntact() throws {
        ...
        // The real dotfiles-managed file received Calix's 8 events...
