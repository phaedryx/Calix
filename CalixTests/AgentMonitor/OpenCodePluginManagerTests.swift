...
//  `<pluginsDirectory>/plugins/calix-agent-monitor.js`, a Bun-runtime
...
//  - scriptBody invariants: CALIX_SURFACE_ID env guard (`return {}`),
//    agent-endpoint.json reference, X-Calix-Agent-Kind: opencode header,
...
final class OpenCodePluginManagerTests: XCTestCase {
    ...
    /// isInstalled() operate on `<this>/plugins/calix-agent-monitor.js`.
    ...
    private var expectedPluginPath: String {
        configRoot + "/plugins/calix-agent-monitor.js"
    }
    ...
    func test_scriptBody_guardsOnMissingSurfaceIDAndReturnsEmptyHooks() {
        ...
        XCTAssertTrue(body.contains("CALIX_SURFACE_ID"),
                     "Plugin must gate on process.env.CALIX_SURFACE_ID")
        XCTAssertTrue(body.contains("return {}"),
                     "When CALIX_SURFACE_ID is unset, the plugin must register no hooks (return {})")
    }
    ...
    func test_scriptBody_sendsAgentKindHeaderAsOpenCode() {
        let body = OpenCodePluginManager.scriptBody
        guard let headerRange = body.range(of: "X-Calix-Agent-Kind") else {
            XCTFail("scriptBody must send the X-Calix-Agent-Kind header")
            return
        }
        ...
        XCTAssertTrue(body[headerRange.upperBound..<windowEnd].contains("opencode"),
                     "X-Calix-Agent-Kind header value must be opencode")
    }
    ...
    func test_scriptBody_cachesEndpointFileByMtime() {
        ...
        XCTAssertTrue(body.contains("import { stat }") && body.contains("node:fs/promises"),
                     "Plugin must stat the endpoint file to detect changes (e.g. a Calix server restart)")
    }
    ...
    func test_install_symlinkAtDestinationPath_followsAndOverwritesRealFileKeepingLinkIntact() throws {
        ...
        let destinationPath = pluginsDir + "/calix-agent-monitor.js"
    ...
    func test_install_multiHopDanglingSymlink_writesToFinalDestinationKeepingBothLinksIntact() throws {
        ...
        let finalTarget = dotfilesDir + "/calix-agent-monitor.js"
