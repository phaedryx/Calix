...
//  calix-agent-hook script body invariants, its 0755 installation, and the
//  0600 agent-endpoint.json port/token file it reads on every invocation.
//
//  Coverage:
//  - scriptBody guards on CALIX_SURFACE_ID being unset, always exits 0,
//    posts stdin verbatim via curl -m 2 --data-binary, sends the
//    X-Calix-Surface-ID header, and reads agent-endpoint.json
...
final class AgentHookScriptTests: XCTestCase {
    ...
    func test_scriptBody_exitsEarlyWhenSurfaceIDUnset() {
        ...
        XCTAssertTrue(body.contains("CALIX_SURFACE_ID"),
                     "Script must reference CALIX_SURFACE_ID")
        XCTAssertTrue(body.contains("exit 0"),
                     "Script must exit 0 when CALIX_SURFACE_ID is unset (external terminals unaffected)")
    }
    ...
    func test_scriptBody_sendsSurfaceIDHeader() {
        XCTAssertTrue(AgentHookScript.scriptBody.contains("X-Calix-Surface-ID"),
                     "Script must send the X-Calix-Surface-ID header")
    }
    ...
    // MARK: - Phase 2: kind argv + X-Calix-Agent-Kind header
    ...
    func test_scriptBody_sendsAgentKindHeader() {
        XCTAssertTrue(AgentHookScript.scriptBody.contains("X-Calix-Agent-Kind"),
    ...
    func test_agentEndpointFile_write_isAtomicAndLeavesNoTempFile() throws {
        ...
        // concurrent reader (calix-agent-hook, invoked from every active
