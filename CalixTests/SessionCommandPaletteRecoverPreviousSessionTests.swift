...
//  throughout CalixWindowController for every other AppDelegate-level
...
//    }
//
//  Registered in CalixWindowController.setupCommandRegistry, alongside
...
//  using CalixWindowController(restoring: true) so no real ghostty
...
final class SessionCommandPaletteRecoverPreviousSessionTests: XCTestCase {
    ...
    func test_recoverPreservedSession_withNothingPreserved_isSafeNoOpAndLeavesFlagFalse() async throws {
        ...
        setenv("CALIX_UITEST_SESSION_DIR", resolvedDir.path, 1)
        addTeardownBlock {
            unsetenv("CALIX_UITEST_SESSION_DIR")
