//
//  CalixWindowControllerFocusSyncTests.swift
//  CalixTests
//
...
//    `CalixWindowController.restoreFocus()` reads the stale
...
//    • Wire `CalixWindowController` to update
//      `activeTab?.splitTree.focusedLeafID` through the callback.
//
//  Approach:
//    These tests DO NOT instantiate `CalixWindowController` (which requires
...
@MainActor
final class CalixWindowControllerFocusSyncTests: XCTestCase {
    ...
    /// `CalixWindowController` is expected to install.
    private func makeTwoPaneFixture(initialFocus: SplitContainerViewFocus = .first) -> Fixture {
        ...
        // mirrors the `CalixWindowController` wiring so the closure does not
    ...
    ///        `CalixWindowController.attemptFocusRestore` (see
    ///        `CalixWindowController.swift:1022-1024`):
    ...
    func testFocusedReadPathReflectsClick() {
        ...
        XCTAssertTrue(
            ...
            "The CalixWindowController restore-focus read path must resolve to the clicked pane's SurfaceView"
        )
    }
    ...
    /// `CalixWindowController.setupUI()` and `rebuildSplitContainer()`)
    ...
    ///   `CalixWindowController` cannot be instantiated in a unit test
