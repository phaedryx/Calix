//
//  CalixWindowControllerLastWindowCloseSaveTests.swift
//  CalixTests
//
...
//  (CalixWindowController.swift, guarded only by
...
//  CalixWindowControllerNonLastWindowCloseTests' established pattern
//  (itself mirroring CalixWindowControllerFullScreenTests): a
//  _testInsert-only, no-live-ghostty-surface fixture (plain Tab(title:)
//  tabs, no SurfaceRegistry surfaces, matching
//  CalixWindowControllerCloseArmsTests' makeSingleTabFixture), NSApp
...
//  (which would write to the developer's actual ~/.calix).
//
//  Coverage:
...
@MainActor
final class CalixWindowControllerLastWindowCloseSaveTests: XCTestCase {
    ...
    /// (mirrors CalixWindowControllerCloseArmsTests.makeSingleTabFixture)
    ...
    /// CalixWindowControllerNonLastWindowCloseTests' own reasoning), and
    ...
    /// and write to the developer's actual ~/.calix.
    private final class SaveOnCloseSpyAppDelegate: ConfirmQuitMockAppDelegate {
        ...
        override func isClosingLastManagedWindow(_ controller: CalixWindowController) -> Bool {
