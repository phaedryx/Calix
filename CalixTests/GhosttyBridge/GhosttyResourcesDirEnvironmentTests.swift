...
//  §3). Calix's own bundled ghostty resources are version-matched to the
...
//  match the libghostty version Calix actually embeds.
//
...
//  file's RED evidence.
//
//  Proposed API (Calix/GhosttyBridge/GhosttyResourcesDirEnvironment.swift):
//
...
//
//  Environment save/restore mirrors the CALIX_SESSION_BIN idiom already
//  established by AppDelegateRestoreRemoteSessionTests/
//  CalixWindowControllerRemoteReconnectCommandTests (save the original
...
final class GhosttyResourcesDirEnvironmentTests: XCTestCase {
    ...
    func test_apply_withResolvedPath_overwritesPreExistingBogusValue() {
        ...
        GhosttyResourcesDirEnvironment.apply("/opt/calix-fixture/Resources/ghostty")

        XCTAssertEqual(
            currentValue(), "/opt/calix-fixture/Resources/ghostty",
            "a resolved bundled path must overwrite an inherited value, since Calix's bundled scripts are version-matched to the embedded libghostty and an inherited path may not be"
        )
    }
    ...
    func test_apply_withResolvedPath_setsVariableWhenNoneWasSetBefore() {
        ...
        XCTAssertEqual(currentValue(), "/opt/calix-fixture/Resources/ghostty")
    }
