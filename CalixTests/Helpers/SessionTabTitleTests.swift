...
// the whole CalixTests target fails to build until the Green phase adds
...
struct SessionTabTitleFromCwdTests {
    ...
    func nestedPathAbbreviatesRelativeToHome() {
        let result = SessionTabTitle.fromCwd("\(home)/projects/Calix", home: home)
        #expect(
            result == "~/projects/Calix",
            "A path nested under home must abbreviate to \"~/projects/Calix\" but got \(result)"
        )
    }
