...
//  CalixTests scheme uses Calix.app as its test HOST, and
//  applicationDidFinishLaunching runs its full production launch body
//  against the developer's real ~/.calix environment before any test
...
//  RED evidence.
//
//  Proposed API (Calix/Helpers/LaunchEnvironmentPolicy.swift), sibling to
...
final class LaunchEnvironmentPolicyTests: XCTestCase {
    ...
    func test_xcTestPresent_noUITestingFlag_isUnitTestHost() {
        XCTAssertTrue(
            ...
            "XCTest loaded and no --uitesting flag is exactly the CalixTests host shape"
        )
    }
    ...
    func test_noXCTest_noUITestingFlag_isNotUnitTestHost() {
        XCTAssertFalse(
            LaunchEnvironmentPolicy.isUnitTestHost(xcTestPresent: false, arguments: ["/path/to/Calix"]),
    ...
    func test_noXCTest_withUITestingFlag_isNotUnitTestHost() {
        XCTAssertFalse(
            LaunchEnvironmentPolicy.isUnitTestHost(xcTestPresent: false, arguments: ["/path/to/Calix", "--uitesting"]),
            "the real CalixUITests app-under-test process shape (--uitesting, no XCTest loaded) must run the full launch"
        )
    }
    ...
    func test_realProcess_insideUnitTestHost_isUnitTestHost() {
        XCTAssertTrue(
            ...
            "this very test process is a unit-test host: XCTest is loaded and the CalixTests scheme never passes --uitesting"
        )
    }
}
