...
//
//  Script bodies (zshenvBody/calixZshBody/fishIntegrationBody) are
...
final class ShellIntegrationInstallerTests: XCTestCase {
    ...
    func test_install_writesThreeFilesWithExactBodiesAt0644() throws {
        ...
        let paths: [(URL, String)] = [
            ...
            (ShellIntegrationInstaller.calixZshPath(in: installRoot), ShellIntegrationInstaller.calixZshBody),
    ...
    func test_remove_deletesAllThreeInstalledFiles() throws {
        ...
        for path in [
            ...
            ShellIntegrationInstaller.calixZshPath(in: installRoot),
    ...
    func test_calixZshBody_isNonEmptyAndPassesZshSyntaxCheck() throws {
        XCTAssertFalse(ShellIntegrationInstaller.calixZshBody.isEmpty,
                       "calixZshBody must contain the real preexec/precmd hook registration, not the RED-phase stub")

        let exitCode = try syntaxCheckExitCode(interpreter: "/bin/zsh", body: ShellIntegrationInstaller.calixZshBody)
        XCTAssertEqual(exitCode, 0, "calixZshBody must be syntactically valid zsh (`zsh -n`)")
    }
    ...
    /// calix.zsh is correct) rather than actually exercising the hooks.
    ...
    private func writeFixtureEndpoint(port: Int, token: String) throws {
        let appSupportDir = tempDir.appendingPathComponent("Library/Application Support/Calix")
    ...
    func test_realZshSession_singleCommand_postsMatchingStartAndEndEvents() async throws {
        ...
        try runInteractiveZsh(command: "true; exit", extraEnv: ["CALIX_SURFACE_ID": surfaceID.uuidString])
        ...
        XCTAssertTrue(startLine.contains("X-Calix-Surface-ID: \(surfaceID.uuidString)"),
                     "the start POST must carry the CALIX_SURFACE_ID as X-Calix-Surface-ID")
    ...
    func test_realZshSession_nonZeroExitCommand_postsMatchingExitCode() async throws {
        ...
        try runInteractiveZsh(command: "(exit 3); exit", extraEnv: ["CALIX_SURFACE_ID": surfaceID.uuidString])
    ...
    func test_realZshSession_emptyInteraction_postsNothing() async throws {
        ...
        try runInteractiveZsh(command: "true; exit", extraEnv: ["CALIX_SURFACE_ID": sanitySurfaceID.uuidString])
        ...
        try runInteractiveZsh(command: "exit", extraEnv: ["CALIX_SURFACE_ID": surfaceID.uuidString])
    ...
    func test_realZshSession_surfaceAndSessionIDBothUnset_postsNothing() async throws {
        ...
        let lines = await waitForCurlLogLines(minCount: 1, timeout: 0.5)
        XCTAssertTrue(lines.isEmpty,
                     "with both CALIX_SURFACE_ID and CALIX_SESSION_ID unset (a plain, non-Calix zsh), " +
    ...
    func test_realZshSession_restoresOriginalZdotdirAndSourcesItsZshenv() async throws {
        ...
        try runInteractiveZsh(
            ...
            extraEnv: ["CALIX_ZSH_ZDOTDIR": userZdotdir.path]
        )
        ...
        XCTAssertEqual(
            ...
            "Calix's own .zshenv must restore ZDOTDIR to the user's original value from CALIX_ZSH_ZDOTDIR " +
    ...
    // _calix_preexec exactly as fish's own event system would for a real
    ...
    // printed straight to the user's terminal, with `_calix_cmd_active`
    ...
    /// `_calix_cmd_active`'s resulting value. The internal `sleep 1`
    ...
    func test_realFishSession_multiLineCommand_setsTrackingStateAndPostsStartWithoutError() async throws {
        ...
        let (stdout, stderr) = try runFishDirectPreexec(
            fishPath: fishPath,
            command: "echo hello\necho world",
            extraEnv: ["CALIX_SESSION_ID": UUID().uuidString]
        )
        ...
        XCTAssertTrue(
            ...
            "a real (non-whitespace) multi-line command must still set _calix_cmd_active -- stdout: \(stdout)"
        )
    ...
    func test_realFishSession_whitespaceOnlyCommand_postsNothingAndLeavesTrackingInactive() async throws {
        ...
        XCTAssertFalse(
            stdout.contains("ACTIVE=1"),
            "a whitespace-only $argv[1] must return before setting _calix_cmd_active -- stdout: \(stdout)"
        )
