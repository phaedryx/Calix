//
//  CalixShellIntegrationEnvironmentTests.swift
//  CalixTests
//
...
//  CalixShellIntegrationEnvironment.swift's own header for why this
//  follows that shape rather than an injected seam).
//
//  CalixShellIntegrationEnvironment.apply/remove are no-op stubs this
...
final class CalixShellIntegrationEnvironmentTests: XCTestCase {

    private let zdotdirVariableName = "ZDOTDIR"
    private let originalZdotdirVariableName = "CALIX_ZSH_ZDOTDIR"
    ...
    override func setUp() {
        ...
        root = URL(fileURLWithPath: "/opt/calix-fixture/shell-integration")
    }
    ...
    func test_apply_setsZdotdirToRootZshSubdirectory() {
        ...
        CalixShellIntegrationEnvironment.apply(rootDirectory: root)

        XCTAssertEqual(currentValue(zdotdirVariableName), expectedZdotdir,
                       "apply must point ZDOTDIR at <rootDirectory>/zsh")
    }

    func test_apply_withExistingZdotdir_savesOriginalIntoCalixZshZdotdir() {
        ...
        XCTAssertEqual(currentValue(originalZdotdirVariableName), "/Users/alice/.config/zsh",
                       "an existing ZDOTDIR must be preserved into CALIX_ZSH_ZDOTDIR before being overwritten")
        XCTAssertEqual(currentValue(zdotdirVariableName), expectedZdotdir)
    }

    func test_apply_withNoExistingZdotdir_leavesCalixZshZdotdirUnset() {
        unsetenv(zdotdirVariableName)
        unsetenv(originalZdotdirVariableName)
        ...
        XCTAssertNil(
            ...
            "with no original ZDOTDIR, CALIX_ZSH_ZDOTDIR must be ABSENT (not present-and-empty) -- " +
            "the installed .zshenv distinguishes unset (fall back to $HOME) from set-to-empty via " +
            "${CALIX_ZSH_ZDOTDIR+X}, matching ghostty's own GHOSTTY_ZSH_ZDOTDIR chain"
        )
    }

    func test_apply_withZdotdirGenuinelyChangedExternally_overwritesTheStaleCalixZshZdotdir() {
        // ZDOTDIR now holds a genuinely DIFFERENT external value (not
        // our own installed dir) -- e.g. some other tool re-pointed it
        // between two apply() calls. re-deriving CALIX_ZSH_ZDOTDIR from
    ...
    func test_remove_restoresOriginalZdotdirWhenCurrentValueStillPointsAtOurDir() {
        ...
        CalixShellIntegrationEnvironment.remove(rootDirectory: root)
    ...
    func test_remove_unconditionallyUnsetsCalixZshZdotdirEvenWhenZdotdirNoLongerOurs() {
        // Review finding: CALIX_ZSH_ZDOTDIR is purely our own internal
        ...
        XCTAssertNil(currentValue(originalZdotdirVariableName),
                     "remove() must unset CALIX_ZSH_ZDOTDIR unconditionally, even when ZDOTDIR no longer " +
