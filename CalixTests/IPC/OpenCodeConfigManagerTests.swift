...
final class OpenCodeConfigManagerTests: XCTestCase {
    ...
    /// BEGIN delimiter marker for the Calix IPC managed block in AGENTS.md.
    private let beginDelimiter = "<!-- BEGIN CALIX IPC"
    /// END delimiter marker for the Calix IPC managed block in AGENTS.md.
    private let endDelimiter = "<!-- END CALIX IPC -->"
    ...
    func test_enableIPC_createsOpenCodeJSONFromScratch() throws {
        ...
        let calixIPC = mcp?["calix-ipc"] as? [String: Any]
        XCTAssertNotNil(calixIPC, "calix-ipc entry should exist")
        XCTAssertEqual(calixIPC?["type"] as? String, "remote",
    ...
    func test_enableIPC_createsAgentsMDFromScratch() throws {
        ...
        XCTAssertTrue(content.contains(beginDelimiter),
                      "AGENTS.md should contain BEGIN CALIX IPC delimiter")
        XCTAssertTrue(content.contains(endDelimiter),
                      "AGENTS.md should contain END CALIX IPC delimiter")
    ...
    func test_enableIPC_preservesOtherMCPServers() throws {
        ...
        // Then: other-server preserved, calix-ipc added
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
        XCTAssertNotNil(mcp?["other-server"], "other-server should be preserved")
        XCTAssertNotNil(mcp?["calix-ipc"], "calix-ipc should be added")
    ...
    func test_enableIPC_preservesOtherTopLevelKeys() throws {
        ...
        // Then: theme and model still present, calix-ipc added
        let dict = try readOpenCodeDict()
    ...
    func test_enableIPC_upsertsExistingCalixEntry() throws {
        // Given: opencode.json already has calix-ipc with old port/token
        let existingJSON = """
        {
            "mcp": {
                "calix-ipc": {
        ...
        // Then: calix-ipc is REPLACED with new values (not duplicated, not merged)
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
    ...
    func test_enableIPC_correctURLFormat() throws {
        ...
        let mcp = dict["mcp"] as? [String: Any]
        let calixIPC = mcp?["calix-ipc"] as? [String: Any]
        XCTAssertEqual(calixIPC?["url"] as? String, "http://127.0.0.1:12345/mcp",
    ...
    func test_enableIPC_preservesAgentsMDUserContent() throws {
        ...
        // Then: user content preserved AND Calix IPC block present
        let content = try readAgentsMD()
        ...
        XCTAssertTrue(content.contains("- write tests"),
                      "User bullet should be preserved")
        XCTAssertTrue(content.contains(beginDelimiter),
                      "Calix IPC BEGIN delimiter should be appended")
        XCTAssertTrue(content.contains(endDelimiter),
                      "Calix IPC END delimiter should be appended")
        XCTAssertTrue(content.contains(canarySubstring),
                      "Calix IPC canary substring should be present")
    }

    func test_enableIPC_replacesExistingCalixBlock() throws {
        // Given: AGENTS.md already contains a (stale) Calix IPC block, plus user content
        let stale = """
        # User Rules

        do what you need to

        \(beginDelimiter) (managed by Calix, do not edit) -->
        ## Calix IPC
    ...
    func test_enableIPC_symlinkFollowedToRealFile_opencodeJson() throws {
        ...
        // Then: the real file received the calix-ipc entry...
        ...
        XCTAssertNotNil(mcp?["calix-ipc"], "enableIPC must write through the symlink into the real opencode.json")
    ...
    func test_enableIPC_partialFailure_jsonWrittenAgentsFailed() throws {
        ...
        XCTAssertNotNil(mcp?["calix-ipc"], "partial-failure leaves calix-ipc entry in opencode.json")
    }
    ...
    func test_disableIPC_removesCalixFromOpenCodeJSON() throws {
        // Given: opencode.json has calix-ipc and another server
        let existingJSON = """
        {
            "mcp": {
                "calix-ipc": {
        ...
        // Then: calix-ipc removed, other-server preserved
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
        XCTAssertNil(mcp?["calix-ipc"], "calix-ipc should be removed")
        XCTAssertNotNil(mcp?["other-server"], "other-server should be preserved")
    }

    func test_disableIPC_removesMcpKeyIfEmpty() throws {
        // Given: opencode.json has only calix-ipc under mcp, plus another top-level key
        let existingJSON = """
        {
            "mcp": {
                "calix-ipc": {
    ...
    func test_disableIPC_removesBlockFromAgentsMD() throws {
        // Given: AGENTS.md has user content surrounding a Calix IPC block
    ...
    func test_disableIPC_onlyOpenCodeJsonExists() throws {
        // Given: only opencode.json exists (with calix-ipc)
        let existingJSON = """
        {
            "mcp": {
                "calix-ipc": {
        ...
        // Then: opencode.json has calix-ipc removed; AGENTS.md still missing
        let dict = try readOpenCodeDict()
        let mcp = dict["mcp"] as? [String: Any]
        XCTAssertNil(mcp?["calix-ipc"], "calix-ipc should be removed")
        XCTAssertFalse(FileManager.default.fileExists(atPath: agentsMDPath),
    ...
    func test_disableIPC_onlyAgentsMDExists() throws {
        // Given: only AGENTS.md exists (with Calix block)
        let existing = """
        # Header
    ...
    func test_disableIPC_symlinkFollowedToRealFile() throws {
        ...
        let realDict = try JSONSerialization.jsonObject(with: realData) as? [String: Any]
        let mcp = realDict?["mcp"] as? [String: Any]
        XCTAssertNil(mcp?["calix-ipc"], "disableIPC must remove the entry from the real file reached through the symlink")
    ...
    func test_isIPCEnabled_trueWhenPresent() {
        // Given: opencode.json has calix-ipc entry
        let existingJSON = """
        {
            "mcp": {
                "calix-ipc": {
    ...
    func test_isIPCEnabled_falseWhenAbsent() {
        // Given: opencode.json has mcp but no calix-ipc
        let existingJSON = """
        {
            "mcp": {
                "other-server": {
