//
//  CalixMCPServerAgentEventTests.swift
//  CalixTests
//
//  TDD Red Phase for CalixMCPServer.route(request:): the extracted HTTP
...
//  - Missing / malformed X-Calix-Surface-ID header → 400
...
@MainActor
final class CalixMCPServerAgentEventTests: XCTestCase {
    ...
    /// ~/Library/Application Support/Calix/agent-endpoint.json.
    ...
    private func agentEventRequest(
        ...
        if let kindHeader { headers["X-Calix-Agent-Kind"] = kindHeader }
    ...
    // MARK: - X-Calix-Agent-Kind propagation (Phase 2)

    func test_route_agentEvent_agentKindHeader_propagatesToRegistryEntryKind() async {
        // Case A: X-Calix-Agent-Kind: codex must propagate to the entry's kind.
        ...
        XCTAssertEqual(registryWithKindHeader.entries[surfaceWithKindHeader]?.kind, "codex",
                       "X-Calix-Agent-Kind: codex must propagate to the registry entry's kind")
        ...
        XCTAssertEqual(registryWithoutKindHeader.entries[surfaceWithoutKindHeader]?.kind, "claude-code",
                       "A missing X-Calix-Agent-Kind header must default to claude-code")
    }
    ...
    func test_route_agentEvent_emptyStringKindHeader_defaultsToClaudeCode() async {
        ...
        XCTAssertEqual(registry.entries[surfaceID]?.kind, "claude-code",
                       "An empty-string X-Calix-Agent-Kind header must default to claude-code")
    }
    ...
    func test_route_agentEvent_whitespaceOnlyKindHeader_defaultsToClaudeCode() async {
        ...
        XCTAssertEqual(response.statusCode, 204)
        XCTAssertEqual(registry.entries[surfaceID]?.kind, "claude-code",
                       "A whitespace-only X-Calix-Agent-Kind header must default to claude-code")
    }
    ...
    func test_agentEventBinding_postToolUseRegisterPeerResponse_bindsSurface_firstMessageLightsUpBadge() async throws {
        ...
        // one of the other calix-ipc tools — so a message sent to a pane
        ...
        let bindingRequest = agentEventRequest(
            ...
            {"hook_event_name":"PostToolUse","session_id":"s1",
             "tool_name":"mcp__calix-ipc__register_peer",
    ...
    func test_agentEventBinding_lspToolCall_doesNotRefreshInboxCounts_messagingToolDoes() async throws {
        ...
        let bindingRequest = agentEventRequest(
            ...
            {"hook_event_name":"PreToolUse","session_id":"s1",
             "tool_name":"mcp__calix-ipc__send_message",
    ...
    /// (optionally) an `X-Calix-Surface-ID` header, mirroring
    ...
    func test_userScenario_bothPanesInitializeWithSurfaceHeader_passiveRecipientGetsBadgeWithoutCallingAnyTool() async throws {
        ...
        // ClaudeConfigManager emits `X-Calix-Surface-ID` on the calix-ipc
        // MCP entry.
        ...
        // calix-ipc tool itself, so the pre-Round-4 PreToolUse/
    ...
    // gated on a valid `X-Calix-Surface-ID` header (see
        ...
        for testCase in cases {
            ...
            guard testCase.shouldBind else {
                ...
                XCTAssertFalse(instructions.contains("Your peer_id is:"),
                               "[\(testCase.label)] an invalid/missing X-Calix-Surface-ID header must not " +
            ...
            XCTAssertEqual(registry.entries[surfaceID]?.unreadCount ?? 0, 1,
                           "[\(testCase.label)] a valid X-Calix-Surface-ID header must auto-register and " +
    ...
    func test_registerPeerToolCall_surfaceHeader_bindsNewPeerToSurface() async throws {
        ...
        XCTAssertEqual(registry.entries[surfaceID]?.unreadCount, 1,
                       "register_peer's own X-Calix-Surface-ID header must bind the newly created " +
    ...
    func test_initialize_surfaceAlreadyBound_secondInitializeDoesNotDisruptMessageDelivery() async throws {
        ...
        // same CALIX_SURFACE_ID env var) resolves to the SAME peer — the
    ...
    func test_surfaceIDHeader_leadingTrailingWhitespace_parsesIdenticallyOnMCPAndAgentEvent() async throws {
        ...
        XCTAssertEqual(sessionStart.statusCode, 204,
                       "A padded X-Calix-Surface-ID must still parse on /agent-event, not 400")
        XCTAssertEqual(registry.entries[surfaceID]?.sessionID, "session-1",
        ...
        XCTAssertEqual(sendResponse.statusCode, 200)
        XCTAssertEqual(registry.entries[surfaceID]?.unreadCount, 1,
                       "A padded X-Calix-Surface-ID on /mcp's initialize must bind the trimmed UUID, " +
    ...
    func test_registerPeer_noSurfaceHeader_alwaysRegistersNewPeer() async throws {
        ...
        // An external MCP client without X-Calix-Surface-ID (e.g. a
    ...
    // external MCP client with no X-Calix-Surface-ID (e.g. OpenCode) has
    ...
    // valid X-Calix-Surface-ID is present; instructions retain the
    ...
    func test_initialize_withoutSurfaceHeader_doesNotAutoRegisterPeer() async throws {
        ...
        // MCP client that can't send X-Calix-Surface-ID.
        let initResponse = await server.route(request: mcpRequest(
        ...
        XCTAssertEqual(countAfter, countBefore,
                       "initialize without X-Calix-Surface-ID must not auto-register a new peer — " +
    ...
    func test_registerPeer_rename_missingRoleArgument_preservesExistingRole() async throws {
        ...
        // CalixMCPServer's `initialize` case) — the value the omitted
    ...
    // elsewhere in `CalixMCPServer` (`parseSurfaceID`, the
    // `X-Calix-Agent-Kind` handling in `routeAgentEvent`).
