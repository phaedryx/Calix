//
//  CalixMCPServerLSPIntegrationTests.swift
//  CalixTests
//
//  Integration tests for the merge of the LSP tool surface (MCPLSPBridge)
//  into the existing IPC-flavoured MCP server (CalixMCPServer / MCPRouter).
//
//  Scope:
...
//  - `CalixMCPServer.lspBridge` is `nil` until `startLSP()` runs.
...
//  `CalixMCPServer.lspBridge`, `CalixMCPServer.startLSP()`,
//  `CalixMCPServer._testInjectLSPBridge(_:)`) exist yet. This file is
//  expected to fail compilation until swift-specialist adds them in
//  `Calix/Features/IPC/MCPProtocol.swift` and
//  `Calix/Features/IPC/CalixMCPServer.swift`. Once implemented, every
...
/// `CalixMCPServer` can dispatch real `lsp_*` tool calls without a
...
// MARK: - CalixMCPServerLSPIntegrationTests

@MainActor
final class CalixMCPServerLSPIntegrationTests: XCTestCase {

    // MARK: - Properties

    private var server: CalixMCPServer!
    private let testToken = "lsp-integration-token"
    private let workspace = URL(fileURLWithPath: "/tmp/calix-lsp-integration")
    private let fileURI = "file:///tmp/calix-lsp-integration/main.ts"
    ...
    func test_calixMCPServer_testInjectLSPBridge_replacesBridge() async {
    ...
    func test_calixMCPServer_handleConnection_lspToolCall_routesToBridge() async throws {
    ...
    func test_calixMCPServer_handleConnection_unknownLSPTool_returnsError() async throws {
        // Arrange
        let (bridge, _) = await makeBridge()
        server._testInjectLSPBridge(bridge)
    ...
    func test_calixMCPServer_handleConnection_ipcToolCall_stillWorks() async throws {
    ...
    func test_calixMCPServer_handleConnection_lspBridgeNotStarted_returnsError() async throws {
