//
//  CalixMCPServerCommandEventTests.swift
//  CalixTests
//
//  TDD Red Phase for CalixMCPServer's new POST /command-event endpoint,
//  mirroring CalixMCPServerAgentEventTests' structure for /agent-event.
//
//  Coverage:
//  - Wrong / missing bearer token -> 401
//  - Missing body / malformed JSON body -> 400
//  - Missing / empty X-Calix-Surface-ID header -> 400 (malformed client)
...
@MainActor
final class CalixMCPServerCommandEventTests: XCTestCase {
    ...
    /// same rationale as `CalixMCPServerAgentEventTests`.
    ...
    func test_routeCommandEvent_unresolvableSessionIDHeader_dropsSilently_returns204NoResidueOnceRegistered() async {
        ...
        XCTAssertEqual(dropResponse.statusCode, 204,
                       "An unresolvable X-Calix-Surface-ID (a session ID with no SessionSurfaceMap entry) " +
