//
//  CalixMCPServerLoopbackBugSpecTests.swift
//  CalixTests
//
//  Regression tests for two related defects in the IPC MCP server
//  surface area:
//
//    1. Dual-stack loopback mismatch. `CalixMCPServer` binds its
...
//
//  Both tests live under `CalixTests/IPC/` to sit next to the existing
//  `CalixMCPServerWiringBugSpecTests` — the established home for
...
@MainActor
final class CalixMCPServerLoopbackBugSpecTests: XCTestCase {

    // MARK: - Properties

    /// Each test uses its own server instance to avoid perturbing
    /// `CalixMCPServer.shared` (which `CalixWindowController` and other
    /// suites observe).
    ...
    private func readPublishedURL() throws -> String {
        ...
        let calixEntry = try XCTUnwrap(
            mcpServers["calix-ipc"] as? [String: Any],
            "published config must contain a `calix-ipc` entry"
        )
        return try XCTUnwrap(
            calixEntry["url"] as? String,
            "calix-ipc entry must contain a `url` field"
        )
    }
    ...
    /// the helper shape in `CalixMCPServerWiringBugSpecTests`.
    @discardableResult
    ...
    /// caps the server at 10 simultaneous Calix instances and leaves no
    ...
    ///      `CalixMCPServer.start`'s linear scan to fail on every
    ///      iteration.
