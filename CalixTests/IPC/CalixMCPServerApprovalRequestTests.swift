//
//  CalixMCPServerApprovalRequestTests.swift
//  CalixTests
//
...
//  CalixMCPServer. Mirrors CalixMCPServerAgentEventTests' harness
//  (HTTPRequest construction + `await server.route(request:)` direct
//  drive, no sockets, isolated `agent-endpoint.json` directory) and
//  CalixMCPServerCockpitToolsTests' injected-ApprovalInboxStore /
...
//    true }`
//  - Unknown X-Calix-Agent-Kind and the agentHookApprovalEnabled toggle
...
@MainActor
final class CalixMCPServerApprovalRequestTests: XCTestCase {

    // MARK: - Properties

    private var server: CalixMCPServer!
    private let testToken = "approval-request-test-token"
    private let settingsSuiteName = "com.calix.tests.CalixMCPServerApprovalRequestTests"

    /// Test-isolated `agent-endpoint.json` directory -- same rationale as
    /// CalixMCPServerAgentEventTests: this suite never calls `start()`,
    ...
    /// surface -- mirrors this codebase's other `CalixMCPServer`
    ...
    func test_route_unknownKind_returns200EmptyBody_andSubmitsNothing() async {
        ...
        XCTAssertEqual(response.statusCode, 200,
                       "an unrecognized X-Calix-Agent-Kind must still return 200, not an error status")
        XCTAssertNil(response.body, "an unrecognized X-Calix-Agent-Kind must return an EMPTY body")
        XCTAssertTrue(approvalInbox.pending.isEmpty,
                      "an unrecognized X-Calix-Agent-Kind must never submit a request to the inbox, " +
    ...
    func test_route_pending_submitsAgentHookRequest_withSurfaceAndSummary() async throws {
        ...
        XCTAssertEqual(pendingRequest.targetSurfaceID, surfaceID,
                       "targetSurfaceID must be the surface resolved from X-Calix-Surface-ID")
        ...
        case .agentHook(let toolName, let kind, let summary):
            ...
            XCTAssertEqual(kind, AgentEntry.claudeCodeKind,
                           "a missing X-Calix-Agent-Kind header must default to claude-code, same as /agent-event")
    ...
    func test_route_defaultTimeout_is570000ms() {
        let freshServer = CalixMCPServer()
    ...
    /// No new seam on `CalixMCPServer` is needed -- this existing one
