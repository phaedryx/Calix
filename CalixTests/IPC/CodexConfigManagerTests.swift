...
final class CodexConfigManagerTests: XCTestCase {
    ...
    func test_enableIPC_createsNewFile() throws {
        ...
        XCTAssertTrue(content.contains("[mcp_servers.calix-ipc]"))
    ...
    func test_enableIPC_addsToExistingFile() throws {
        ...
        // Then: original content preserved and calix-ipc appended
        let content = readConfig()
    ...
    func test_enableIPC_updatesExistingEntry() throws {
        // Given: file already has calix-ipc section with old port/token
        let existing = """
    ...
    func test_disableIPC_removeSection() throws {
        // Given: file has calix-ipc section
        let existing = """
        [mcp_servers.calix-ipc]
    ...
    func test_disableIPC_preservesOtherMCPServers() throws {
        ...
        // Then: calix-ipc removed, other preserved
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.calix-ipc]"))
        XCTAssertTrue(content.contains("[mcp_servers.other]"))
        XCTAssertTrue(content.contains("http://localhost:9999/mcp"))
    ...
    func test_disableIPC_noSection() throws {
        // Given: file exists but has no calix-ipc section
        let existing = """
        [general]
        model = "gpt-4"
        """
    ...
    func test_disableIPC_unreadableFile() throws {
        ...
        writeConfig("[mcp_servers.calix-ipc]\nurl = \"http://localhost:41830/mcp\"\n")
    ...
    // rejection silently broke IPC configuration for that setup. Calix
    ...
    func test_enableIPC_symlinkFollowedToRealFile_writesSuccessfullyAndKeepsLinkIntact() throws {
        ...
        XCTAssertTrue(realContent.contains("[mcp_servers.calix-ipc]"),
    ...
    func test_disableIPC_symlinkFollowedToRealFile_removesSuccessfullyAndKeepsLinkIntact() throws {
        ...
        let realContent = try String(contentsOfFile: realFile, encoding: .utf8)
        XCTAssertFalse(realContent.contains("[mcp_servers.calix-ipc]"),
    ...
    func test_enableIPC_duplicateSections() throws {
        // Given: file has TWO calix-ipc sections
        let existing = """
        [mcp_servers.calix-ipc]
        url = "http://localhost:40000/mcp"
        ...
        // Exactly one calix-ipc section header
        let headerCount = content.components(separatedBy: "[mcp_servers.calix-ipc]").count - 1
        XCTAssertEqual(headerCount, 1, "Should have exactly one calix-ipc section")
    }

    func test_enableIPC_arrayOfTablesIgnored() throws {
        // Given: file has [[array_of_tables]] immediately after the
        // calix-ipc section header, with no blank-line separator.
        //
        // NOTE: this test's expectation was flipped from the original
        // (which asserted the [[array_of_tables]] block was removed along
        // with calix-ipc). Per TOML semantics, `[[array_of_tables]]` is
        // itself a table header — it always starts a new table, which means
        // it always ends the calix-ipc table that precedes it, regardless
        // of adjacency. The old behavior (treating `[[` as "not a
        // boundary") was a semantics bug: it caused a real [[hooks.*]] or
        // other array-of-tables block placed directly after calix-ipc (no
        ...
        // Then: calix-ipc's own body is removed, but [[array_of_tables]]
        // (a table header, hence a section boundary) and everything under
        // it survives.
        let content = readConfig()
        XCTAssertTrue(content.contains("[[array_of_tables]]"),
                       "[[array_of_tables]] is a table header and ends the calix-ipc section")
    ...
    // MARK: - Regression (Phase 2): [[hooks.*]] adjacent to calix-ipc section
    ...
    func test_disableIPC_preservesAdjacentCodexHooksArrayOfTablesBlock() throws {
        ...
        // block immediately following `[mcp_servers.calix-ipc]` was never
        // recognized as the section's boundary and got swallowed as if it
        // were part of the calix-ipc section body.
        let existing = """
        [mcp_servers.calix-ipc]
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }
        ...
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.calix-ipc]"), "calix-ipc section must be removed")
        XCTAssertTrue(content.contains("[[hooks.SessionStart]]"),
                      "A [[hooks.*]] array-of-tables block immediately after [mcp_servers.calix-ipc] " +
    ...
    func test_disableIPC_preservesAdjacentCalixAgentHooksManagedBlock() throws {
        // Regression: the same bug applied to Calix's own future
        // CodexHooksConfigManager-managed block — its BEGIN marker line is
        // an ordinary comment line (not a table header), so it too got
        // swallowed by the calix-ipc section removal.
        let existing = """
        [mcp_servers.calix-ipc]
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }

        # BEGIN CALIX AGENT HOOKS (managed by Calix, do not edit)
        [[hooks.SessionStart]]
        [[hooks.SessionStart.hooks]]
        type = "command"
        command = '"/path/to/calix-agent-hook" codex'
        timeout = 5
        # END CALIX AGENT HOOKS
        """
        writeConfig(existing)
        ...
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.calix-ipc]"), "calix-ipc section must be removed")
        XCTAssertTrue(content.contains("# BEGIN CALIX AGENT HOOKS"),
                      "The managed block's BEGIN marker line itself must survive disableIPC")
        XCTAssertTrue(content.contains("[[hooks.SessionStart]]"))
        XCTAssertTrue(content.contains("# END CALIX AGENT HOOKS"),
    ...
    func test_disableIPC_noBlankLineBeforeHooksArrayOfTablesBlock_survives() throws {
        ...
        // immediately following calix-ipc with NO blank-line separator must
    ...
    func test_disableIPC_noBlankLineBeforeCalixAgentHooksMarker_survives() throws {
        // Regression: same as above, but for the `# BEGIN CALIX` managed
        // block marker directly abutting calix-ipc with no blank line.
        let existing = """
        [mcp_servers.calix-ipc]
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }
    ...
    func test_disableIPC_removesCalixIpcSubTables() throws {
        // A [mcp_servers.calix-ipc.*] / [[mcp_servers.calix-ipc.*]]
        // sub-table header is still part of the calix-ipc entry per TOML's
        ...
        url = "http://localhost:41830/mcp"
        http_headers = { "Authorization" = "Bearer tok" }
        [mcp_servers.calix-ipc.env]
        FOO = "bar"
        [[mcp_servers.calix-ipc.extra]]
        name = "baz"
        [other.section]
        key = "value"
        """
        ...
        let content = readConfig()
        XCTAssertFalse(content.contains("[mcp_servers.calix-ipc]"))
        XCTAssertFalse(content.contains("[mcp_servers.calix-ipc.env]"))
        XCTAssertFalse(content.contains("FOO = \"bar\""))
        XCTAssertFalse(content.contains("[[mcp_servers.calix-ipc.extra]]"))
    ...
    func test_concurrentEnableDisable_noCorruption() throws {
        ...
        if FileManager.default.fileExists(atPath: configPath) {
            ...
            // If calix-ipc section exists, it should have the url key
            if content.contains("[mcp_servers.calix-ipc]") {
                XCTAssertTrue(content.contains("url = \"http://127.0.0.1:"),
                              "calix-ipc section should have url key")
            }
        }
