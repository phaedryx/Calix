...
// - Strict managed-block detection (BEGIN + END + `calix-ipc:` all required)
...
final class HermesConfigManagerTests: XCTestCase {
    ...
    private let beginLine = "# BEGIN CALIX IPC (managed by Calix, do not edit)"
    /// Canonical END line literal as written by enableIPC.
    private let endLine = "# END CALIX IPC"
    ...
    func test_enableIPC_createsConfigFromScratch() throws {
        ...
        XCTAssertTrue(content.contains("calix-ipc:"), "Should contain calix-ipc: key")
    ...
    func test_enableIPC_appendsToFileWithoutMcpServers() throws {
        ...
        XCTAssertTrue(content.contains("calix-ipc:"),
                      "Managed block must contain calix-ipc:")
    ...
    func test_enableIPC_insertsIntoExistingMcpServers() throws {
        ...
        XCTAssertTrue(content.contains("calix-ipc:"),
                      "calix-ipc child should be added")
        // Marker comments are indented to child level (2 spaces) for Case B.
        XCTAssertTrue(content.contains("  # BEGIN CALIX IPC"),
                      "BEGIN marker should be indented 2 spaces (child level)")
        XCTAssertTrue(content.contains("  # END CALIX IPC"),
    ...
    func test_enableIPC_insertsWith4SpaceIndent() throws {
        ...
        XCTAssertTrue(content.contains("    calix-ipc:"),
                      "calix-ipc: key should be at 4-space indent")
    }
    ...
    func test_enableIPC_repeatedUpdatePreservesFollowingTopLevelSection() throws {
        ...
        mcp_servers:
          stripe:
            url: "https://mcp.stripe.com"
          # BEGIN CALIX IPC (managed by Calix, do not edit)
          calix-ipc:
    ...
    // config managers. Calix now follows the link and writes through to
    ...
    func test_disableIPC_removesManagedBlock_preservesUserContent() throws {
        ...
        XCTAssertFalse(content.contains("calix-ipc:"),
                       "calix-ipc key should be removed")
    }
    ...
    func test_disableIPC_keepsExistingMcpServers() throws {
        ...
        XCTAssertTrue(afterEnable.contains("calix-ipc:"))
        ...
        XCTAssertFalse(content.contains("calix-ipc:"),
                       "calix-ipc child should be removed")
    }

    func test_disableIPC_removesEmptyMcpServersWhenOnlyChild() throws {
        // Given: Case A setup — mcp_servers: was created by enableIPC and contains only calix-ipc
        try HermesConfigManager.enableIPC(port: 41830, token: "tok", configPath: configPath)
    ...
    func test_disableIPC_throwsOnMalformedBlock_missingCalixIpc() throws {
        // Given: pre-existing BEGIN/END pair but NO calix-ipc: key between them
        let existing = """
        agent_name: "hermes"
        # BEGIN CALIX IPC (managed by Calix, do not edit)
        # someone deleted the calix-ipc body
        # END CALIX IPC
        max_tokens: 4096
        """
        try writeConfig(existing)
    ...
    func test_isIPCEnabled_falseForBeginOnly() throws {
        // Given: BEGIN line only (no END, no calix-ipc body in well-formed pair)
        let existing = """
        # BEGIN CALIX IPC (managed by Calix, do not edit)
        mcp_servers:
          calix-ipc:
    ...
    func test_isIPCEnabled_falseForBeginEndWithoutCalixIpc() throws {
        // Given: BEGIN/END pair but no calix-ipc: between
        let existing = """
        # BEGIN CALIX IPC (managed by Calix, do not edit)
        # nothing useful here
        # END CALIX IPC
    ...
    func test_isIPCEnabled_ignoresUserCommentMentioningBeginCalixIpc() throws {
        // Given: a user-written line that mentions "BEGIN CALIX IPC" mid-line —
        // this is NOT a real managed block start because the regex requires line-start.
        let existing = """
        agent_name: "hermes"
        # something # BEGIN CALIX IPC blah blah — note from a user, not a real marker
        mcp_servers:
          calix-ipc:
            url: "http://localhost:1111/mcp"
        """
        ...
        // When/Then: not a real managed block (no real BEGIN/END pair surrounding calix-ipc)
        XCTAssertFalse(HermesConfigManager.isIPCEnabled(configPath: configPath),
                       "isIPCEnabled should ignore mid-line mentions of BEGIN CALIX IPC")
    }
}
