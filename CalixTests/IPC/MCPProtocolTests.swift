...
final class MCPProtocolTests: XCTestCase {
    ...
    func test_initializeResponse_matchesMCPSpec() throws {
        ...
        XCTAssertEqual(initResult.serverInfo.name, "calix-ipc",
                       "Server name must be calix-ipc")
    ...
    // CalixMCPServerAgentEventTests' `test_initialize_withoutSurfaceHeader_
    // doesNotAutoRegisterPeer`) — an external client with no
    // X-Calix-Surface-ID (e.g. OpenCode) has no surface for a renamed peer
