...
final class ClaudeConfigManagerTests: XCTestCase {
    ...
    func test_enableIPC_createsConfigFromScratch() throws {
        ...
        // Then: file is created with correct calix-ipc entry
        ...
        XCTAssertEqual(calixIPC?["type"] as? String, "http")
    ...
    func test_enableIPC_addsToExistingConfig() throws {
        ...
        // Then: calix-ipc is added, other-server is preserved
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
    ...
    func test_enableIPC_updatesExistingEntry() throws {
        // Given: calix-ipc already exists with old token
        let existingJSON = """
        {
            "mcpServers": {
                "calix-ipc": {
        ...
        // Then: calix-ipc is updated with new values
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        let calixIPC = mcpServers?["calix-ipc"] as? [String: Any]
    ...
    func test_enableIPC_createsBackup() throws {
        ...
        XCTAssertFalse(bakContent.contains("calix-ipc"),
                       "Backup should not contain the newly added calix-ipc entry")
    }
    ...
    // passive recipient that never calls a calix-ipc tool (and so never
    // fires the PreToolUse/PostToolUse hook the old binding path relied
    // on). Must be the `${CALIX_SURFACE_ID:-}` empty-default form — an
    ...
    func test_enableIPC_headers_includeSurfaceIDPlaceholderWithEmptyDefault() throws {
        ...
        // Then: headers contain exactly Authorization and X-Calix-Surface-ID,
        // the latter as the literal `${CALIX_SURFACE_ID:-}` expansion string.
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        let calixIPC = mcpServers?["calix-ipc"] as? [String: Any]
        let headers = calixIPC?["headers"] as? [String: String]
        ...
        XCTAssertEqual(headers?["Authorization"], "Bearer abc123")
        XCTAssertEqual(headers?["X-Calix-Surface-ID"], "${CALIX_SURFACE_ID:-}",
                       "X-Calix-Surface-ID must be the literal ${CALIX_SURFACE_ID:-} placeholder " +
                       "(empty default) so Claude Code's own env expansion fills it per-pane, and an " +
                       "external terminal with no CALIX_SURFACE_ID env still parses the config")
        XCTAssertEqual(headers?.count, 2,
                       "headers must contain exactly Authorization and X-Calix-Surface-ID, no more")
    }

    func test_enableIPC_updatesExistingEntry_headersIncludeSurfaceIDPlaceholder() throws {
        // Given: calix-ipc already exists from a pre-Round-4 config (single header)
        let existingJSON = """
        {
            "mcpServers": {
                "calix-ipc": {
        ...
        // Then: the regenerated entry gains the X-Calix-Surface-ID header too
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        let calixIPC = mcpServers?["calix-ipc"] as? [String: Any]
        let headers = calixIPC?["headers"] as? [String: String]
        ...
        XCTAssertEqual(headers?["Authorization"], "Bearer new-token")
        XCTAssertEqual(headers?["X-Calix-Surface-ID"], "${CALIX_SURFACE_ID:-}",
                       "Re-running enableIPC on a pre-Round-4 entry must add X-Calix-Surface-ID, " +
    ...
    func test_disableIPC_removesCalixEntry() throws {
        // Given: config has calix-ipc and another server
        let existingJSON = """
        {
            "mcpServers": {
                "calix-ipc": {
        ...
        // Then: calix-ipc is removed, other-server is preserved
        let dict = try readConfigDict()
        let mcpServers = dict["mcpServers"] as? [String: Any]
        XCTAssertNil(mcpServers?["calix-ipc"], "calix-ipc should be removed")
        XCTAssertNotNil(mcpServers?["other-server"], "other-server should be preserved")
    }

    func test_disableIPC_removesMcpServersIfEmpty() throws {
        // Given: config has only calix-ipc in mcpServers
        let existingJSON = """
        {
            "mcpServers": {
                "calix-ipc": {
    ...
    func test_isIPCEnabled_falseWhenAbsent() {
        // Given: config has mcpServers but no calix-ipc
        let existingJSON = """
        {
            "mcpServers": {
                "other-server": {
    ...
    // entirely for that (legitimate, self-authored) setup. Calix now
    ...
    func test_symlink_followedToRealFile_writesSuccessfullyAndKeepsLinkIntact() throws {
        ...
        XCTAssertNotNil(mcpServers?["calix-ipc"], "enableIPC must write through the symlink into the real file")
    ...
    func test_multiHopDanglingSymlink_writesToFinalDestinationKeepingBothLinksIntact() throws {
        ...
        XCTAssertNotNil((json?["mcpServers"] as? [String: Any])?["calix-ipc"])
