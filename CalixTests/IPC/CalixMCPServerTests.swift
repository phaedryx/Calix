//
//  CalixMCPServerTests.swift
//  CalixTests
//
//  Tests for CalixMCPServer: the MCP server's handleJSONRPC method
...
@MainActor
final class CalixMCPServerTests: XCTestCase {
    ...
    /// real `~/Library/Application Support/Calix/agent-endpoint.json`.
    ...
    func test_handleJSONRPC_initialize_returnsInitResult() async throws {
        ...
        XCTAssertEqual(serverInfo["name"] as? String, "calix-ipc",
                       "serverInfo.name must be calix-ipc")
    ...
    // observes AgentRegistry (not CalixMCPServer, which isn't @Observable)
    ...
    // ~/Library/Application Support/Calix path in every test in this suite.
    ...
    func test_start_survivesAgentEndpointFileWriteFailure() throws {
        ...
        // ENOTDIR (the lock file itself lives elsewhere, under Calix's
    ...
    // unreachable endpoint to both MCP clients and calix-agent-hook.
    //
    // Real Calix instances hold 41830-41839 on this host, so both tests
    ...
    /// Find a free loopback port via `CalixMCPServer.askKernelForFreeLoopbackPort()`
    ...
    /// window, since the real Calix app may be holding `41830` on this
    /// host.
    ...
    func test_start_preferredPortZero_recordsActualBoundPortAndIsReachable() async throws {
        ...
        XCTAssertNotEqual(
            publishedPort, 0,
            "agent-endpoint.json must not publish port: 0 — calix-agent-hook cannot reach that"
        )
    ...
    func test_headersOnlyRequest_neverReceivesASecondResponseAfter408() async throws {
        ...
        // itself. See both counters' doc comments on `CalixMCPServer`.
    ...
    // `CalixMCPServer.ReceiveAccumulator`'s append loop — must still
...
/// `CalixMCPServerTests`) so it carries no `@MainActor` isolation and
...
/// `CalixMCPServer` double-sends on this connection. Callers that only
...
/// `CalixMCPServer.ReceiveAccumulator` to accumulate one logical
