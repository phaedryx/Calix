...
/// `--uitesting` app process, driven by the `CALIX_UITEST_DEFAULTS_SUITE`
...
final class SettingsStore: @unchecked Sendable {
    ...
    /// full `CALIX_UITEST_DEFAULTS_SUITE` rationale. A `let` resolved
    ...
    /// `CALIX_UITEST_DEFAULTS_SUITE` is fixed for the process's entire
    ...
    init() {
        if let name = ProcessInfo.processInfo.environment["CALIX_UITEST_DEFAULTS_SUITE"] {
