...
private let logger = Logger(
    ...
    category: "CalixWindowController"
)
...
class CalixWindowController: NSWindowController, NSWindowDelegate {
    ...
    private lazy var sessionReconnectCoordinator: SessionReconnectCoordinator = {
        let coordinator = SessionReconnectCoordinator(
        ...
        coordinator.surfaceSwapDelegate = self
        return coordinator
    }()
    /// Git Changes sidebar + diff-tab orchestration, extracted from this
    /// controller (see GitChangesController.swift's header comment).
    /// `lazy` for the same reason as `sessionReconnectCoordinator` above:
    /// its closures capture `self` before `init` finishes, which is safe
    ...
    /// call inside `performReconnectSurfaceSwap`, mirroring `AppDelegate
    ...
    /// this call in `performReconnectSurfaceSwap`, including the
    /// `SessionReconnectCoordinator`-owned `markEstablished` timing
    /// under test, remains real, unmodified production code. DO NOT
    /// use from production code.
    var _performReconnectSurfaceCreationHookForTesting: (() -> UUID?)?

    /// Test seam (P5, remote sessions, contract R3): when non-nil, called
    /// with the synthesized `command` from inside `performReconnectSurfaceSwap`,
    ...
    /// `performReconnectSurfaceSwap` (populated right before, and
    /// cleared right after, its `destroySurface` call). Consulted by
    /// `killSessionIfPersistent` via `SessionCloseKillPolicy` — defense
    /// in depth on top of `performReconnectSurfaceSwap`'s own ordering (re-pointing
    ...
    /// the very session `performReconnectSurfaceSwap` is reconnecting to.
    ...
    private func setupCommandRegistry() {
        ...
        commandRegistry.register(PaletteCommand(id: "ipc.enable", title: "Enable AI Agent IPC", category: "IPC", isAvailable: {
            !CalixMCPServer.shared.isRunning
        }) { [weak self] in
            self?.enableIPC()
        })
        ...
        commandRegistry.register(PaletteCommand(id: "cli.install", title: "Install CLI to PATH", category: "System") {
            ...
            let cliSource = "\(appPath)/Contents/Resources/bin/calix"
            let cliDest = "/usr/local/bin/calix"
            ...
            if let appleScript = NSAppleScript(source: script) {
                ...
                if let error {
                    ...
                    alert.informativeText = "The 'calix' command is now available. Run 'calix browser --help' to get started."
        ...
        commandRegistry.register(PaletteCommand(
            ...
            category: "Git",
            isAvailable: { [weak self] in (self?.gitChangesController.reviewFileCount ?? 0) >= 2 }
        ) { [weak self] in
    ...
    /// `CalixWindowControllerCreateManagedSurfaceRemoteHostTests` drives
    ...
    /// user-initiated pane/tab close ends the underlying calix-session
    ...
    /// `performReconnectSurfaceSwap` (which destroys the OLD surface to make room
    ...
    /// clears the entry two lines later -- exactly like `performReconnectSurfaceSwap`'s
    ...
    /// orphans the calix-session running entirely on the remote host.
    ...
    /// calls `daemonClient.kill` — the underlying calix-session keeps
    ...
    /// `.calixApprovalInboxChanged` notification, observed below) or
    ...
    // MARK: - Cockpit Seams
    //
    // Backs LiveCockpitAppAccess (Calix/Features/Cockpit/CockpitAppAccess.swift)
    // -- see CalixTests/Cockpit/CockpitAppAccessSeamTests.swift for the
    ...
    /// `CalixWindowControllerCreateManagedSurfaceRemoteHostTests`'s
    ...
    /// Not `private` (P4 round-4 fix RED phase): `CalixWindowControllerCloseArmsTests`
    ...
    private func registerNotificationObservers() {
        ...
        center.addObserver(self, selector: #selector(handleConfirmingQuitDidEnd(_:)),
                           name: .calixConfirmingQuitDidEnd, object: nil)
    ...
    /// comment): `CalixWindowControllerChildExitedTasksTests` calls
    ...
    private func dispatchReconnectDecision(surfaceID: UUID, decision: SessionReconnectDecision) {
        ...
        case .reconnect(let sessionID, let attempt):
            sessionReconnectCoordinator.scheduleReconnect(oldSurfaceID: surfaceID, sessionID: sessionID, attempt: attempt)
        case .giveUp:
    ...
    /// only ever reported unreachable — the underlying `calix-session`
    ...
    /// (`calix-session ls --all` still lists it) or a future restore.
    ///
    ...
    private func sendReconnectGiveUpNotification(tabID: UUID) {
        NotificationManager.shared.sendNotification(
            ...
            body: "Calix couldn't reattach this session, but it may still be running with its state intact. " +
                  "Reattach it from the session browser once the daemon is reachable again.",
            tabID: tabID
        )
    }

    /// R6-B (r6-fix-spec.md, r5-verdicts.md V6): a Calix-side, persistent
    ...
    /// Re-runs `calix-session attach --create` for `sessionID` in a
    ...
    /// `performReconnectSurfaceSwap` makes (`tab.registry.createSurface`, a real
    ...
    /// Relays `.calixSurfaceDestroyed` (posted by `SurfaceRegistry`) into
    ...
    func windowWillClose(_ notification: Notification) {
        ...
        sessionReconnectCoordinator.cancelAllReconnectWork()
...
extension CalixWindowController: SessionReconnectSurfaceSwapping {
    /// Re-runs `calix-session attach --create` for `sessionID` in a
    /// fresh surface, then swaps it in for `oldSurfaceID` everywhere
    /// that referenced it: the split tree (`SplitTree.remapLeafIDs`,
    /// the same contract `AppDelegate.restoreTabSurfaces` uses),
    ...
    /// ordering.
    ///
    /// Satisfies `SessionReconnectSurfaceSwapping` for
    /// `sessionReconnectCoordinator`: returns `nil` on any failure (no
    /// such tab, no ghostty app/window, no resolvable `calix-session`
    /// binary, surface-creation failure), mirroring this method's own
    /// prior early-return contract exactly. The deferred-reset grace
    /// `Task` that used to live at the end of this method (see the
    /// "HIGH-SPEED RECONNECT FLASHING BUG"/round-18 G6 history in
    /// `SessionReconnectCoordinator.performReconnect`'s doc comment) now
    /// lives entirely in the coordinator, driven by the `isRemote`/
    /// `newSurfaceID` this returns.
    func performReconnectSurfaceSwap(oldSurfaceID: UUID, sessionID: String) -> ReconnectSurfaceSwapResult? {
        guard let tab = findTab(surfaceID: oldSurfaceID) else { return nil }
        guard let app = GhosttyAppController.shared.app, let window = self.window else { return nil }
        ...
        if let host {
            ...
            guard let localCommand = SessionCommandSynthesizer.reattachCommand(sessionID: sessionID, cwd: tab.pwd ?? NSHomeDirectory()) else {
                logger.error("No calix-session binary resolvable; cannot reconnect session \(sessionID, privacy: .public)")
                return nil
            }
        ...
        return ReconnectSurfaceSwapResult(newSurfaceID: newSurfaceID, isRemote: host != nil)
    }
}
