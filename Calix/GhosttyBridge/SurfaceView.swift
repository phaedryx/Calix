...
private let logger = Logger(subsystem: "com.calix.terminal", category: "SurfaceView")
...
extension SurfaceView {
    ...
    // the search field, CalixWindowController provides the same gating in
    ...
    @objc func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
        ...
        // to `return true`) and CalixWindowController's matching gate would
        // never be consulted.
    ...
    private var isActiveTabSplit: Bool {
        guard let controller = window?.windowController as? CalixWindowController else {
            return false
        }
