...
private let logger = Logger(subsystem: "com.calix.terminal", category: "GhosttyAction")
...
enum GhosttyActionRouter {
    ...
    /// Calix has no per-pane title distinct from the tab (no split-pane
    /// header UI), so both variants forward the same way, as
    /// `.ghosttyPromptTabTitle`, for `CalixWindowController` to show the
    ...
    /// `CalixWindowController` to feed into `AgentRegistry.
