...
private let logger = Logger(subsystem: "com.calix.terminal", category: "GhosttyConfig")
...
final class GhosttyConfigManager {
    ...
    /// Keys that Calix manages (overrides from user's ghostty config).
    ...
    private static var calixConfigDir: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/calix", isDirectory: true)
    }

    private static var calixGlassPresetURL: URL {
        calixConfigDir.appendingPathComponent("calix-glass.conf", isDirectory: false)
    }

    private static var calixRuntimeOverrideURL: URL {
        calixConfigDir.appendingPathComponent("calix-runtime.conf", isDirectory: false)
    }
    ...
    /// Note: configLoadFile for calix-glass.conf runs on EVERY loadDefaultConfig call regardless of this flag.
    ...
    static func loadDefaultConfig() -> ghostty_config_t? {
        ...
        if !hasAppliedPreset {
            applyCalixGlassPresetIfPossible()
        ...
        // Load Calix glass preset after default files so its values take precedence.
        let presetPath = calixGlassPresetURL.path
    ...
    private static func applyRuntimeOverrides(_ cfg: ghostty_config_t) {
        ...
        do {
            try fm.createDirectory(at: calixConfigDir, withIntermediateDirectories: true)
            ...
            let text = """
            # --- Calix Runtime Override (managed) ---
            ...
            try (finalText + "\n").write(to: calixRuntimeOverrideURL, atomically: true, encoding: .utf8)
            GhosttyFFI.configLoadFile(cfg, path: calixRuntimeOverrideURL.path)
        } catch {
            logger.warning("Failed to apply runtime overrides: \(error.localizedDescription, privacy: .public)")
        }
    }

    private static func applyCalixGlassPresetIfPossible() {
        let fm = FileManager.default
        let presetConfigURL = calixGlassPresetURL

        do {
            // 1. Create ~/.config/calix/ directory if needed.
            try fm.createDirectory(at: calixConfigDir, withIntermediateDirectories: true)
            ...
            if let oldBundleID = Bundle.main.bundleIdentifier,
               let oldAppSupport = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
                let oldPath = oldAppSupport
                    ...
                    .appendingPathComponent("calix-glass.conf", isDirectory: false)
            ...
            // 3. Create calix-glass.conf with defaults if it doesn't exist.
            if !fm.fileExists(atPath: presetConfigURL.path) {
                ...
                // 4. Remove deprecated keys from existing calix-glass.conf (migration).
            ...
            logger.warning("Failed to apply Calix glass preset: \(error.localizedDescription, privacy: .public)")
        }
    }

    /// Removes any stale Calix-managed blocks from the main ghostty config file.
    ...
    private static func cleanupMainGhosttyConfig(fileManager fm: FileManager) {
        ...
        // Remove all known Calix-managed block types.
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- Calix Visual Defaults (managed) ---",
            endMarker: "# --- End Calix Visual Defaults ---"
        )
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- Calix Glass Preset (managed) ---",
            endMarker: "# --- End Calix Glass Preset ---"
        )
        cleaned = removeManagedBlock(
            from: cleaned,
            startMarker: "# --- Calix Include (managed) ---",
            endMarker: "# --- End Calix Include ---"
        )
        ...
        if cleaned != existingMain {
            do {
                ...
                logger.info("Cleaned stale Calix managed blocks from ghostty config")
            } catch {
