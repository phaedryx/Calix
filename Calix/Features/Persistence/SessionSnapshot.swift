...
struct TabSnapshot: Codable, Equatable {
    ...
    let browserURL: URL?
    /// Schema v6: calix-session references keyed by leaf surface UUID
...
extension Dictionary where Key == UUID, Value == SessionRef {
    ...
    /// `CalixWindowController.performReconnect` (surface swap) — there
...
extension TabGroup {
    ...
    ///   `CalixWindowController.windowSnapshot()` delegate its
