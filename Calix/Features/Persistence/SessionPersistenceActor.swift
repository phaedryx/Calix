...
private let logger = Logger(subsystem: "com.calix.terminal", category: "SessionPersistence")
...
actor SessionPersistenceActor {
    ...
    init() {
        ...
        if let testDir = ProcessInfo.processInfo.environment["CALIX_UITEST_SESSION_DIR"] {
            calixDir = URL(fileURLWithPath: testDir, isDirectory: true)
        } else {
            ...
            calixDir = homeDir.appendingPathComponent(".calix", isDirectory: true)
        }

        try? FileManager.default.createDirectory(at: calixDir, withIntermediateDirectories: true, attributes: [
            .posixPermissions: 0o700
        ])

        self.savePath = calixDir.appendingPathComponent("sessions.json")
        self.backupPath = calixDir.appendingPathComponent("sessions.json.bak")
        self.recoveryMarkerPath = calixDir.appendingPathComponent(".recovery")
        self.recoverySnapshotPath = calixDir.appendingPathComponent("sessions.recovery.json")
    }
    ...
    /// Migrate session data from legacy Application Support path to ~/.calix/.
    /// Returns true if migration was performed.
    @discardableResult
    func migrateFromLegacyPath() -> Bool {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let legacyPath = appSupport.appendingPathComponent("Calix/session.json")
