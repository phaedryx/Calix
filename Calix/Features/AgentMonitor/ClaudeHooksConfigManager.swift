...
// calix-agent-hook lifecycle hook. Mirrors ClaudeConfigManager's API shape
...
struct ClaudeHooksConfigManager: Sendable {
    ...
    /// The 8 hook events Calix installs a command entry for, and the
    ...
    /// `calix-agent-hook`'s subscription so the hook actually fires.
    ...
    /// Merges Calix's 8 hook entries into `configPath`'s `"hooks"` section,
    /// preserving the user's own existing hook entries and unrelated
    /// top-level keys. Idempotent: re-running replaces Calix's own prior
    /// entries rather than duplicating them.
    ///
    /// `PreToolUse` alone gets a second Calix-owned command entry: the
    ...
    /// that blocks the tool call until Calix's own `/approval-request`
    ...
    static func installHooks(scriptPath: String, approvalScriptPath: String, configPath: String? = nil) throws {
        ...
        for target in targetEvents {
            ...
            guard let groups = existingValue as? [[String: Any]] else {
                // The existing value for this event has a shape Calix
                // doesn't recognize (hand-edited, or a future hooks
                // format) — skip installing Calix's own entry for this
    ...
    /// Removes only Calix's own command entries (identified by the
    /// `calix-agent-hook` command path) from `configPath`'s `"hooks"`
    ...
    /// Whether Calix's own command entry is present for at least one of the
    ...
    /// The Calix-owned command entries for one target event: `PreToolUse`
    ...
    /// Builds the matcher-group Calix installs for one target event:
    ...
    /// Removes Calix's own command entries from every group in `groups`,
