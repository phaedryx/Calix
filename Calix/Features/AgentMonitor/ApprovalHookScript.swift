...
// Calix
//
// The `calix-approval-hook` shell script installed under
// `~/Library/Application Support/Calix/bin/`, alongside
// `calix-agent-hook` (see AgentHookScript.swift). Unlike
// calix-agent-hook's fire-and-forget async POST to /agent-event, this
...
enum ApprovalHookScript {
    ...
    static let fileName = "calix-approval-hook"
    ...
    ///
    /// The `X-Calix-Surface-ID` header value is
    ...
    /// `CALIX_SURFACE_ID` otherwise -- see `AgentHookScript`'s doc
    ...
    /// documents -- Calix's own server answers by 570s, strictly before
    ...
    ///   crucially does NOT call `fail_safe()`. After Calix crashes,
    ...
    #!/bin/sh
    #
    # calix-approval-hook — synchronously forwards a PreToolUse hook's
    # stdin JSON to Calix's local Agent Monitor IPC endpoint and prints
    ...
    fail_safe() {
        if [ "$kind" = "claude-code" ]; then
            printf '%s' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"ask","permissionDecisionReason":"Calix approval inbox unavailable"}}'
        fi
    }
