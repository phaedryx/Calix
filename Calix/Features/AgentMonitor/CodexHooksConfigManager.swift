...
// calix-agent-hook script — Codex's equivalent of
// ClaudeHooksConfigManager's `"hooks"` section in settings.json. Unlike
// Codex's `[mcp_servers.calix-ipc]` section (a single upserted table,
...
struct CodexHooksConfigManager: Sendable {
    ...
    /// The 6 Codex hook events Calix installs a `[[hooks.X]]` entry for.
    ...
    /// Removes only Calix's managed block from `configPath`, leaving the
    ...
    /// Whether Calix's managed block (its BEGIN marker) is present.
    ...
    /// writes for it, POSTing through `calix-approval-hook` instead of
    /// `calix-agent-hook`, with a timeout of
    ...
    /// Removes Calix's managed block (BEGIN line through END line
    ...
    /// immediately-following line that still looks like part of Calix's own
    /// generated block body (`isCalixGeneratedBlockLine`), stopping at the
    ...
    private static func removingManagedBlock(from content: String) -> (result: String, hadBlock: Bool) {
        ...
        if let endIndex = lines[(beginIndex + 1)...].firstIndex(where: { recognizedEndLines.contains($0) }) {
            ...
            while scanIndex < lines.count, isCalixGeneratedBlockLine(lines[scanIndex]) {
