...
// Calix
//
// Coordinates installing/removing calix-agent-hook plus each agent CLI's
...
struct AgentHooksCoordinator: Sendable {
    ...
    /// Installs `calix-agent-hook` and `calix-approval-hook` once, then
    ...
    private static func removeClaudeCode() -> ConfigStatus {
        ...
        // tool present"), removal must check whether Calix's own hooks are
    ...
    private static func removeCodex() -> ConfigStatus {
        ...
        // Calix's managed block is actually installed, not just whether
        // ~/.codex exists.
    ...
    /// Status for Claude Code / Codex when the shared `calix-agent-hook`
