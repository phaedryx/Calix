...
extension AgentEntry {
    ...
    /// and `OpenCodePluginManager`'s `X-Calix-Agent-Kind` header for
    ...
    /// `OpenCodePluginManager.scriptBody`'s `X-Calix-Agent-Kind` header —
...
struct AgentEvent: Sendable, Equatable {
    ...
    let cwd: String?
    let message: String?
    /// For a `PreToolUse` event whose `tool_name` is one of Calix's own
    /// `mcp__calix-ipc__*` tools, this surface's own peer ID (extracted
    ...
    /// `mcp__calix-ipc__register_peer`, the peer ID the server just
    ...
    private static let fromKeyToolNames: Set<String> = [
        "mcp__calix-ipc__send_message", "mcp__calix-ipc__broadcast",
    ]
    ...
    private static let peerIDKeyToolNames: Set<String> = [
        "mcp__calix-ipc__receive_messages",
    ]
    ...
    /// `calix-ipc` tool call that doesn't already know its own peer ID
    /// going in; the server generates it, so it's only available once
    /// the call has returned.
    private static let registerPeerToolName = "mcp__calix-ipc__register_peer"
