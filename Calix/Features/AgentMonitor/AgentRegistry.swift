...
final class AgentRegistry {
    ...
    /// evidence of a genuine new session Calix missed the `SessionStart`
    ...
    /// `CalixWindowController.enableIPC`), cleared by `reset()`.
    ...
    /// Whether `CalixMCPServer` currently has its IPC listener running.
    /// `AgentStatusView` observes this (not `CalixMCPServer.isRunning`
    /// directly) so the sidebar's disabled/enabled placeholder redraws —
    /// `CalixMCPServer` is a plain `@MainActor` class, not `@Observable`,
    ...
    /// `CalixWindowController.pollScreenClassificationIfAgentsSidebarVisible`
    ...
    /// periodic staleness sweep. Called by `CalixMCPServer.finishStart()`.
    ...
    /// staleness sweep. Called by `CalixMCPServer.stop()` so a disabled IPC
    ...
    ///   — a new session already under way that Calix never saw a
    ...
    ///   mid-session, or Calix restarted) can leave a stale-but-idle entry
    ...
    ///   `CalixMCPServer.routeAgentEvent`'s `X-Calix-Agent-Kind` header.
    ...
    func handleHookEvent(
        ...
        // Learn the surface -> peer binding from a calix-ipc PreToolUse's
        ...
        // Round 4 review: `calix-agent-hook`'s command entries all run
    ...
    /// `calix-agent-hook` exiting 0 with no output already guarantees the
    ...
    ///   written: on Calix's 2-second screen poll, most ticks reclassify
    ...
    /// `CalixWindowController.enableIPC` with one formatted entry per
    ...
    /// always used, but `CalixMCPServer` also calls it directly now, to
    /// bind a surface the moment its MCP connection's `initialize` (or an
    /// explicit `register_peer` tool call) carries an `X-Calix-Surface-ID`
    /// header — covering a passive recipient that never calls a
    /// calix-ipc tool itself and so never fires the hook path at all.
    ...
    /// `bindSurface`. Read-only query (Round 4 review): `CalixMCPServer`'s
    ...
    /// `IPCStore`. Two call sites in `CalixMCPServer` rely on this:
    ...
    /// Every peer ID currently bound to a surface. `CalixMCPServer`
    ...
    /// lookup. `CalixMCPServer` calls this once at the end of every
...
extension Notification.Name {
    ...
    static let calixSurfaceDestroyed = Notification.Name("com.calix.agentMonitor.surfaceDestroyed")

    /// Posted when an Agents sidebar row is clicked. `userInfo["surfaceID"]`
    /// carries the `UUID` of the surface to focus.
    static let calixFocusSurface = Notification.Name("com.calix.agentMonitor.focusSurface")
}
