...
// (a session with no live ghostty surface attached in this Calix
...
struct SessionBrowserRow: Identifiable, Equatable, Sendable {
    ...
    /// no ghostty surface in this Calix process is attached to it
    /// (orphaned by a crash, a `kill -9`'d Calix process, or a session
    /// started by a different Calix launch). Every `SessionBrowserRow`
    ...
    /// this session in this Calix process — i.e. it's already visibly
...
final class SessionBrowserModel {
    ...
    /// what `CalixWindowController.remoteSessionSpawnContext(forHost:)`
    ...
    /// `CalixWindowController` concern, kept out of this pure logic
    ...
    /// `CalixWindowController` concern, deliberately kept out of this
