...
// shows the session browser — every calix-session the daemon knows
// about, across all Calix windows and launches, not just this
...
final class SessionBrowserWindowController: NSWindowController {
    ...
    private init() {
        ...
        // `.calixFocusSurface` notification every window controller
    ...
    private func attach(_ row: SessionBrowserRow) {
        if row.isAttachedHere, let surfaceID = SessionSurfaceMap.shared.surfaceID(for: row.id) {
            NotificationCenter.default.post(
                name: .calixFocusSurface, object: nil, userInfo: ["surfaceID": surfaceID]
            )
            return
        }
