...
//  Calix
//
//  Hard-coded data table mapping the 15 languages Calix ships with to
...
//  user overrides from ~/.config/calix/lsp.json.
//
...
/// Everything Calix needs to know about one language server.
///
...
///   destination to redirect Calix into spawning an attacker-controlled
...
/// Immutable in-memory table of every language server Calix knows
...
struct LSPServerRegistry: Sendable, Equatable, Codable {
    ...
    static func loaded(overridePath: URL?) throws -> LSPServerRegistry {
        ...
        // manager that consumes paths under ~/.config/calix.
    ...
    /// The 15 languages Calix ships with. Order here is the order
