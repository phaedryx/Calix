...
// Calix
//
// Reference to a calix-session (persistent PTY session, see
// `calix-session/`) attached to a terminal leaf surface. Stored per leaf
...
struct SessionRef: Codable, Equatable, Sendable {
    /// The calix-session ULID (`SessionSpec.id` / `SessionInfo.id` on the
    ...
    /// (e.g. `"claude-code"`), so a resumed calix-session can also offer
...
extension SessionRef {
    ...
    /// rather than handed to `calix-session attach`, which would
