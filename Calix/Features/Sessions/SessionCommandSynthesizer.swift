...
enum SessionCommandSynthesizer {
    ...
    /// Builds `<binaryPath> --runtime-dir <root>/.calix/run --state-dir
    /// <root>/.calix/state attach <sessionID> --create --cwd <cwd>
    /// [--name <name>]`. `sessionID` is positional, matching the P2
    /// CLI's `AttachArgs` (`calix-session/crates/cli/src/cli.rs`), which
    /// has no `--id` flag.
    ///
    ...
    /// `execCommand`, for the default `.shell`-variant command that Calix
    ...
    /// (`calix-session/crates/cli/src/cli.rs:15-20`, `global = true`),
    /// which `attach` both accepts and forwards verbatim to the daemon it
    /// auto-spawns (`calix-session/crates/cli/src/commands/attach.rs:193-224`).
    /// With no `rootResolver` override (real production use), the
    /// composed paths equal `<real $HOME>/.calix/{run,state}`, identical
    ...
    /// namely `calix-session` itself, directly, with no intermediate
    /// wrapper process left surviving, so the ghostty surface's child
    /// process *is* `calix-session attach`, not a shell wrapping it.
    ...
    static func attachCommand(
        ...
        var command = "\(shSafeToken(binaryPath)) --runtime-dir \(shSafeToken(root + "/.calix/run")) --state-dir \(shSafeToken(root + "/.calix/state")) attach \(shSafeToken(sessionID)) --create --cwd \(shSafeToken(cwd))"
        if let name {
    ...
    /// calix-session binary — as opposed to `SessionSpawnPlanner.plan`,
    ...
    /// command around a hardcoded `"calix-session"` literal that may
    ...
    /// `CalixWindowController.performReconnect`.
    ///
    ...
    /// `remoteCommand` is the single shell-safe word `$HOME/.calix/bin/
    /// calix-session attach <sessionID> --create --cwd <cwd> [--name
    ...
    static func remoteAttachCommand(
        ...
        var remoteCommand = "$HOME/.calix/bin/calix-session attach \(shSafeToken(sessionID)) --create --cwd \(shSafeToken(cwd))"
        if let name {
