...
// `calix-session attach --create` command that survives ghostty
...
enum SpawnPlan: Sendable, Equatable {
    ...
    case passthrough
    /// Launch `calix-session attach --create` for `sessionID`, running
    ...
    /// caller applying the plan (see `CalixWindowController
...
enum SessionSpawnPlanner {
    ...
    /// can't find the calix-session binary at all (a `nil` resolution
    /// must not fall back to a hardcoded `"calix-session"` literal that
    ...
    /// lives on the remote machine, so the LOCAL calix-session binary's
