...
// (`AgentEvent.sessionID`) into the calix-session daemon's per-session
// meta map, so a later reattach (`SessionResumePlanner`) can look it
// up and offer to resume the same CLI conversation.
// `CalixMCPServer.routeAgentEvent` is meant to route every hook event
...
final class AgentSessionMetaBridge {
    ...
    /// calix-session tracking `surfaceID`, under
    /// `SessionResumePlanner.encodeMetaKey(kind: agentKind)`. A no-op
    /// when `surfaceID` has no tracked calix-session (an ordinary,
