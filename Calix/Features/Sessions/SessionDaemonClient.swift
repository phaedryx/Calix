...
// Calix
//
// Queries the calix-session daemon's state for a given session ID.
...
/// spawning a real `calix-session` process.
protocol SessionDaemonClientProtocol: Sendable {
    ...
    /// (`calix-session meta set <id> <key>=<value>`). Added for P4's
    ...
    /// `calix-session`'s own `remote-install` subcommand. Returns `nil`
...
/// (`Calix/Helpers/AwaitBridge.swift`): this type races two independent,
...
extension SessionDaemonClientProtocol {
    ...
    /// hung `calix-session` daemon could stall
    /// `SessionReconnectCoordinator.childExited`'s reconnect decision
...
/// Production client: shells out to the bundled `calix-session` binary
/// via `LSPCommandRunner` (the same `Process`-driving abstraction
/// `LSPInstaller` uses), rather than a bespoke `Process` invocation —
/// so `calix-session` subprocess calls inherit `SystemCommandRunner`'s
...
final class SessionDaemonClient: SessionDaemonClientProtocol, Sendable {
    ...
    /// The `["--runtime-dir", "<root>/.calix/run"]` pair every
    ...
    /// global `--runtime-dir` flag (`calix-session/crates/cli/src/cli.rs:15-16`,
    ...
    /// `<real $HOME>/.calix/run`, identical to the Rust CLI's own
    ...
    init(
        ...
        self.runtimeDirArgument = ["--runtime-dir", rootResolver.resolve() + "/.calix/run"]
    ...
    /// `calix-session kill` subprocess, racing it against a timeout and
    ...
    /// `calix-session` binary's own `remote-install` subcommand, with
    ...
    /// Kills a REMOTE session by shelling `ssh -- <host>
    /// "$HOME/.calix/bin/calix-session kill '<sessionID>'"`, with `ssh`
    ...
    /// only ever needs the `ssh` binary -- the local calix-session
    ...
    func killRemote(host: String, sessionID: String) async {
        ...
        let remoteCommand = "$HOME/.calix/bin/calix-session kill \(SessionCommandSynthesizer.shSafeToken(sessionID))"
        await Task {
            _ = try? await commandRunner.run(
...
/// --json` output — see `calix-session/crates/proto/src/control.rs`).
