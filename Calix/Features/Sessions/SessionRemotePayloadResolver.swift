...
//   - Resources/session-remote/<x86_64|aarch64>/calix-session
...
protocol SessionRemotePayloadResolverProtocol: Sendable {
    /// The bundled cross-compiled `calix-session` payload's absolute
...
struct SessionRemotePayloadResolver: SessionRemotePayloadResolverProtocol {
    ...
    func payloadPath(forArch arch: String) -> String? {
        bundle.url(forResource: "calix-session", withExtension: nil, subdirectory: "session-remote/\(arch)")?.path
    }
