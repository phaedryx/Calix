...
// Calix
//
// Resolves the bundled `calix-session` binary's path, shared by both
...
protocol SessionBinaryResolverProtocol: Sendable {
    /// The bundled `calix-session` binary's absolute path, or `nil` if
...
/// Production resolver: `CALIX_SESSION_BIN` env override (dev workflow
/// / test injection), then the bundled `Resources/bin/calix-session`
/// resource (see `project.yml`'s "Bundle Session Daemon"
/// postBuildScript), then `nil`. Deliberately has no third fallback to
/// a bare `"calix-session"` literal resolved via `PATH` — a review
...
struct SessionBinaryResolver: SessionBinaryResolverProtocol {
    ...
    func resolve() -> String? {
        if let override = environment["CALIX_SESSION_BIN"], !override.isEmpty {
            return override
        }
        return bundle.url(forResource: "calix-session", withExtension: nil, subdirectory: "bin")?.path
    }
}
