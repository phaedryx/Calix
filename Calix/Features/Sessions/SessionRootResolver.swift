...
// Calix
//
// Resolves the on-disk session root ($HOME/.calix: run/sessiond.sock,
// state/) that the Rust calix-session daemon/CLI derives from the
// literal HOME env var at process start (see
// calix-session/crates/cli/src/commands/mod.rs:74's default_home_subdir).
...
// calix-session binary path -- so every consumer must receive its
