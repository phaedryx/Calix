...
/// kept as `CalixWindowControllerReconnectGraceOverrides` even after
/// this move -- several tests reference it by that name directly.
#if DEBUG
enum CalixWindowControllerReconnectGraceOverrides {
...
/// The coordinator's own view of `CalixWindowController.performReconnect`'s
...
/// give-up logic and `CalixWindowController`'s actual FFI surface swap --
...
enum SessionReconnectDecision: Sendable, Equatable {
    ...
    case closePane
    /// Re-run `calix-session attach --create` for `sessionID`. `attempt`
    ...
    /// — never confirmed exited — so the underlying `calix-session` may
...
final class SessionReconnectCoordinator {
    ...
    /// Resolves which calix-session a surface belongs to. Tests inject
    ...
    /// `CalixWindowController` -- the actual FFI surface swap. Never
    /// consulted for timing/dedup/give-up, which stay entirely inside
    /// this coordinator. Set once, alongside `onDecision`, in
    /// `CalixWindowController`'s `sessionReconnectCoordinator` lazy-var
    /// init.
    ...
    /// `CalixWindowController.windowWillClose` alongside its own
    ...
    /// .sessionStateBounded(id:)` queries the LOCAL calix-session daemon
    ...
    private func performReconnect(oldSurfaceID: UUID, sessionID: String) {
        ...
        // calix-session daemon, which can never have a matching
    ...
    /// from `CalixWindowController.windowWillClose` alongside its own
    ...
    private static var reconnectEstablishGraceMilliseconds: UInt64 {
        #if DEBUG
        if let override = CalixWindowControllerReconnectGraceOverrides.reconnectEstablishGraceMilliseconds {
            return override
        }
        #endif
        return 2000
    ...
    /// are insufficient. Mirrors `CalixWindowController
    ...
    /// `CalixWindowController._performReconnectSurfaceCreationHookForTesting`'s
