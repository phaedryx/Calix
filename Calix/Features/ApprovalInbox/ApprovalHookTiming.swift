...
// CLI-agents hook chain: Calix's own long-poll timeout must resolve
...
//   Calix is still legitimately waiting on a human decision, and the
...
// in-pane prompt -- Calix never silently allows anything by going
// missing.
//
// See CalixMCPServerApprovalRequestTests.test_timing_constants_orderingInvariant
...
enum ApprovalHookTiming {
    ...
    /// per-hook entry in both CLIs; Calix's own injected approval-hook
    ...
    /// `CalixMCPServer.approvalRequestTimeoutMs`'s default: the
    ...
    /// The `calix-approval-hook` script's own curl `-m` deadline, in
