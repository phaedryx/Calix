...
// instead of reaching for CalixWindowController/NSWindow directly, one
// instance per window.
//
...
// CalixTests/ApprovalInbox/ApprovalBannerModelTests.swift for the
// specced contract.
...
final class ApprovalBannerModel {
    ...
    /// CalixTests/ApprovalInbox/ApprovalBannerModelTests.swift asserts
    ...
    /// `refreshHostingView()` or posts `.calixApprovalInboxChanged`: both
    ...
    /// `.calixApprovalInboxChanged` doc comment) exists for mutations
    ...
    /// request, so this posts exactly ONE `.calixApprovalInboxChanged`
