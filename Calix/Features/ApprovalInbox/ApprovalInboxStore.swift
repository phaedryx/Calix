...
// (`Calix/Helpers/AwaitBridge.swift`). See
// CalixTests/ApprovalInbox/ApprovalInboxStoreTests.swift for the
// specced contract.
//
...
/// rendered view hierarchy (see `CalixWindowController.refreshRecoveryBar()`'s
...
extension Notification.Name {
    static let calixApprovalInboxChanged = Notification.Name("com.calix.approvalInbox.changed")
}
...
final class ApprovalInboxStore {
    ...
    /// `decision`, but posts `.calixApprovalInboxChanged` exactly ONCE
    ...
    /// `CalixMCPServer.routeApprovalRequest` call this instead of
    ...
    /// posted `.calixApprovalInboxChanged`, so a test can assert a
    ...
    /// `CalixWindowController._closingTabIDsForTesting`). DO NOT use
    ...
    private func notifyChanged() {
        ...
        _testNotifyCount += 1
        #endif
        NotificationCenter.default.post(name: .calixApprovalInboxChanged, object: self)
    }
