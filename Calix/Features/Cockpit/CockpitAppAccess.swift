...
// redeclared here: Calix/Models/SplitTree.swift already defines a
// Swift (not C) `SplitDirection` enum (.horizontal/.vertical) used by
// SplitTree.insert(at:direction:) and CalixWindowController.performSplit
...
// CalixTests/Cockpit/CockpitAppAccessSeamTests.swift, which instead
// drives the CalixWindowController seams (performSplit,
...
/// (`CalixWindowController.ownsSplitLeaf`/`findTab(bySplitLeaf:)`), the
...
/// `CalixWindowController` seams this delegates to are tested directly
...
final class LiveCockpitAppAccess: CockpitAppAccessing {
    ...
    private func keyWindowController(_ appDelegate: AppDelegate) -> CalixWindowController? {
    ...
    private func owningController(for surfaceID: UUID, in appDelegate: AppDelegate) -> CalixWindowController? {
                ...
                for tab in group.tabs {
                    ...
                    for leafID in leafIDs {
                        ...
                        panes.append(CockpitPaneInfo(
                            ...
                            calixSessionID: SessionSurfaceMap.shared.sessionID(for: leafID)
                        ))
                    }
                }
            }
