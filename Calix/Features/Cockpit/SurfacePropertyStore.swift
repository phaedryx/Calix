...
// .ghosttySetTitle/.ghosttySetPwd notifications CalixWindowController
...
// via .calixSurfaceDestroyed (SurfaceRegistry.destroySurface's own
...
// for this codebase.
//
// Surface-id resolution: unlike CalixWindowController's own handlers
...
// CalixTests/Cockpit/SurfacePropertyStoreTests.swift for the specced
// contract.
//
...
// CalixWindowController's own handleSetTitleNotification).
...
final class SurfacePropertyStore: NSObject {
    ...
    func startObserving() {
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(handleSurfaceDestroyed(_:)), name: .calixSurfaceDestroyed, object: nil
        )
    }
    ...
    /// `.ghosttySetTitle`/`.ghosttySetPwd`/`.calixSurfaceDestroyed` for
