...
actor IPCStore {
    ...
    /// current value unchanged (Round 6 review: `CalixMCPServer.handleRegisterPeer`
    ...
    /// when a later step fails after the fact — `CalixMCPServer.
    ...
    /// `CalixMCPServer` to refresh every bound peer's badge once per
