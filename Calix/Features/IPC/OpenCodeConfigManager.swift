...
// Calix
//
// Manages reading/writing OpenCode's config files for the Calix IPC MCP server.
//
// OpenCode uses two files under `~/.config/opencode/`:
// 1. `opencode.json`  — the MCP remote server entry (upsert `mcp.calix-ipc`)
...
struct OpenCodeConfigManager: Sendable {
    ...
    /// Common prefix shared by all BEGIN CALIX IPC markers (historical and current).
    /// Kept as a derived constant so `beginDelimiter` stays in sync with it.
    private static let beginAnchor = "<!-- BEGIN CALIX IPC"

    /// BEGIN delimiter for the Calix IPC managed block in AGENTS.md.
    /// Must match the literal the tests search for.
    private static let beginDelimiter = beginAnchor + " (managed by Calix, do not edit) -->"

    /// Regex that matches a full BEGIN line (`<!-- BEGIN CALIX IPC ... -->`), requiring
    /// the `-->` closing on the same line. This prevents `stripManagedBlocks` from
    /// anchoring on user prose that merely mentions `<!-- BEGIN CALIX IPC` mid-line.
    private static let beginLinePattern = #"<!--\s*BEGIN CALIX IPC[^\n]*?-->"#

    /// END delimiter for the Calix IPC managed block in AGENTS.md.
    private static let endDelimiter = "<!-- END CALIX IPC -->"
    ...
    /// Enables the Calix IPC MCP entry in OpenCode's `opencode.json` and appends
    ...
    /// Disables the Calix IPC entry by removing `mcp.calix-ipc` from `opencode.json`
    ...
    /// Returns whether the Calix IPC MCP entry is present in `opencode.json`.
    ...
    private static func upsertOpenCodeJSON(port: Int, token: String, path: String) throws {
        ...
        // Build the calix-ipc entry fresh — no merge, guarantees stale header keys
    ...
    private static func removeFromOpenCodeJSON(path: String) throws {
        ...
        mcp.removeValue(forKey: calixIPCKey)
    ...
    /// Removes all `BEGIN CALIX IPC` ... `END CALIX IPC` managed blocks from a string.
    ///
    ...
    /// comment terminator on the same line as `BEGIN CALIX IPC`, so user prose that
    /// merely mentions `<!-- BEGIN CALIX IPC` mid-line is NOT treated as a block
    ...
    private static func stripManagedBlocks(from content: String) -> String {
        ...
        // mid-line mention of `<!-- BEGIN CALIX IPC` in user prose cannot
