...
// Calix
//
// Manages reading/writing ~/.hermes/config.yaml for the Calix IPC MCP server.
//
...
//     `calix-ipc:` child entry is inserted under it, with comment markers
...
enum HermesConfigError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .malformedManagedBlock(let reason):
            return "Malformed Calix-managed block in Hermes config: \(reason)"
        case .invalidScalarValue:
...
struct HermesConfigManager: Sendable {
    ...
    private static let beginLine = "# BEGIN CALIX IPC (managed by Calix, do not edit)"
    private static let endLine = "# END CALIX IPC"

    /// Matches a Calix BEGIN marker line. The line must consist of (optional
    /// indent) + `#` + (optional whitespace) + `BEGIN CALIX IPC` + word boundary
    ...
    private static let beginLineRegex = try! NSRegularExpression(
        pattern: #"(?m)^[ \t]*#\s*BEGIN CALIX IPC\b[^\n]*$"#
    )

    /// Matches a Calix END marker line.
    private static let endLineRegex = try! NSRegularExpression(
        pattern: #"(?m)^[ \t]*#\s*END CALIX IPC\s*$"#
    )

    /// Matches a `calix-ipc:` key line at any indent (used to validate the
    /// inside of a BEGIN/END span before treating it as a real managed block).
    private static let calixIPCKeyRegex = try! NSRegularExpression(
        pattern: #"(?m)^[ \t]*calix-ipc:\s*$"#
    )
    ...
    /// Enables Calix IPC by upserting the managed block in the Hermes config.
    /// Self-heals over previously-malformed managed blocks (orphan BEGIN/END,
    /// or BEGIN/END pairs missing the `calix-ipc:` body) by stripping anything
    ...
    /// Disables Calix IPC by removing the managed block(s) from the config.
    /// Throws `.malformedManagedBlock` on orphan BEGIN, orphan END, or
    /// BEGIN/END pairs lacking the `calix-ipc:` body, since silent removal
    ...
    /// (BEGIN line, then `calix-ipc:` line, then END line — in that order,
    ...
    static func isIPCEnabled(configPath: String? = nil) -> Bool {
        ...
        guard normalized.contains("BEGIN CALIX IPC") else { return false }
    ...
    /// is BEGIN line, followed by a `calix-ipc:` line, followed by an END
    ...
    private static func findValidManagedBlocks(in content: String) -> [Range<String.Index>] {
        ...
        for begin in beginMatches {
            ...
            // Require a `calix-ipc:` line strictly between begin and end.
            let between = begin.upperBound..<end.lowerBound
            if firstMatch(of: calixIPCKeyRegex, in: content, range: between) == nil {
                continue
            }
    ...
    /// `calix-ipc:` exists in the content. Throws `.malformedManagedBlock`
    ...
    private static func validateNoMalformedBlocks(in content: String) throws {
        ...
        for (begin, end) in zip(beginMatches, endMatches) {
            ...
            let between = begin.upperBound..<end.lowerBound
            if firstMatch(of: calixIPCKeyRegex, in: content, range: between) == nil {
                throw HermesConfigError.malformedManagedBlock("missing calix-ipc")
            }
        }
    }
    ...
    /// `calix-ipc:` body exists between them. Includes the trailing newline.
