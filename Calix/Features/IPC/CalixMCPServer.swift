...
@MainActor
final class CalixMCPServer {

    static let shared = CalixMCPServer()
    ...
    /// `terminal_*` dispatch (not at `CalixMCPServer` init), by which
    ...
    /// Resolves a calix-session ID to the surface UUID currently
    /// attached to it, for `/mcp` and `/agent-event` requests whose
    /// `X-Calix-Surface-ID` header carries a session ID rather than a
    /// raw surface UUID (persistent-session panes, see
    /// `AgentHookScript`'s `CALIX_SESSION_ID` precedence). Defaults to
    ...
    /// calix-session daemon's per-session meta map (P4), so a later
    ...
    /// (`~/Library/Application Support/Calix`); tests inject a per-test
    ...
    private func routeMCP(request: HTTPRequest) async -> HTTPResponse {
        ...
        // required header below, `/mcp` predates `X-Calix-Surface-ID`,
        ...
        // also resolves a calix-session ID via `sessionSurfaceMap` (the
    ...
    /// Trims whitespace from `X-Calix-Surface-ID` and parses it as a
    ...
    /// `calix-agent-hook`'s own hook POST on the other — is parsed
    ...
    private func parseSurfaceID(from headers: [String: String]) -> UUID? {
        guard let trimmed = header(named: "X-Calix-Surface-ID", in: headers)?
    ...
    /// Two-stage resolution of `X-Calix-Surface-ID`, shared by both
    ...
    /// parse fails — as a calix-session ID looked up in
    /// `sessionSurfaceMap`. A persistent-session pane's hook sends its
    /// stable calix-session ID in this same header instead of a surface
    /// UUID whenever `CALIX_SESSION_ID` is set (see
    /// `AgentHookScript.scriptBody`'s
    /// `${CALIX_SESSION_ID:-$CALIX_SURFACE_ID}` fallback), and an MCP
    ...
    /// the calix-session ID does. `nil` if neither resolves; what a
    ...
    private func resolveSurfaceID(from headers: [String: String]) -> UUID? {
        ...
        guard let raw = header(named: "X-Calix-Surface-ID", in: headers)?
    ...
    /// Resolves `X-Calix-Agent-Kind`, trimming whitespace and falling
    /// back to `AgentEntry.claudeCodeKind` when the header is absent OR
    /// present-but-blank (e.g. a proxy/plugin bug that sends
    /// `X-Calix-Agent-Kind: ` with no value), rather than letting an
    ...
    private func agentKind(from headers: [String: String]) -> String {
        if let trimmed = header(named: "X-Calix-Agent-Kind", in: headers)?
    ...
    private func routeAgentEvent(request: HTTPRequest) async -> HTTPResponse {
        ...
        // present) into the calix-session daemon's per-session meta so
        // a later reattach can offer to resume this conversation. A
        // no-op inside the bridge itself when `surfaceID` has no
        // tracked calix-session (an ordinary, non-persistent pane).
    ...
    private func routeCommandEvent(request: HTTPRequest) async -> HTTPResponse {
        ...
        guard let rawSurfaceIDHeader = header(named: "X-Calix-Surface-ID", in: request.headers)?
        ...
        guard let surfaceID = resolveSurfaceID(from: request.headers) else {
            ...
            // unregistered calix-session ID) is normal steady-state for a
    ...
    /// `calix-approval-hook` script blocks on from a CLI agent's
    ...
    /// -> `resolveSurfaceID` (400) -> an unrecognized `X-Calix-Agent-Kind`
    ...
    private func routeApprovalRequest(request: HTTPRequest) async -> HTTPResponse {
        ...
        guard kind == AgentEntry.claudeCodeKind || kind == AgentEntry.codexKind else {
            // An agent CLI Calix doesn't have a hook-response contract
    ...
    func startLSP() async {
        ...
        // on-disk edits outside Calix's own writers feed back into LSP
        // synchronisation notifications.
        let fileSyncManager = FileSyncManager()
        // Default persistence store under
        // `~/Library/Application Support/Calix/lsp/sessions.json`. Snapshots
    ...
    func start(token: String, preferredPort: Int = 41830) throws {
        ...
        for portOffset in 0..<10 {
            ...
            if resolvedPort != tryPort {
                NSLog("[CalixMCPServer] canonical scan requested port \(tryPort) but the listener resolved to \(resolvedPort); recording the resolved port")
            }
    ...
    ///
    /// `internal` (not `private`) so `CalixMCPServerTests` can exercise
    ...
    func bindKernelAssignedListener() -> (NWListener, Int)? {
        ...
        for attempt in 0..<5 {
            guard let probedPort = askKernelForFreeLoopbackPort() else {
                NSLog("[CalixMCPServer] BSD fallback attempt \(attempt): kernel did not return a port")
                continue
            }
            ...
            if Int(boundPort) != probedPort {
                NSLog("[CalixMCPServer] BSD fallback probed port \(probedPort) but the listener resolved to \(boundPort); recording the resolved port")
            }
    ...
    ///
    /// `internal` (not `private`) so `CalixMCPServerTests` can call this
    ...
    private func startListenerAndWaitForReady(_ nl: NWListener) -> NWListener? {
        ...
        let listenerQueue = DispatchQueue(
            label: "CalixMCPServer.listenerProbe"
        )
        nl.stateUpdateHandler = { state in
            ...
            case .ready:
                ...
                sema.signal()
            case .failed(let err):
                NSLog("[CalixMCPServer] probe listener failed: \(err)")
    ...
    /// production wiring.
    ///
    /// Writes `agent-endpoint.json` (port/token for the calix-agent-hook
    ...
    private func finishStart(
        ...
        do {
            ...
            NSLog("[CalixMCPServer] failed to write agent-endpoint.json: \(error)")
        }
        ...
        self.peerRegistrationTask = Task {
            let peer = await self.store.registerPeer(name: "calix-app", role: "review-ui")
    ...
    func stop() -> Task<Void, Never> {
        ...
        // `CalixMCPServerCockpitToolsTests.test_serverStop_expiresPendingApprovals`.
        ...
        // existing call sites (`CalixWindowController.disableIPC`,
    ...
    /// (more likely under load — see `CalixMCPServerTests`'s
    ...
    ///
    /// - Parameter surfaceID: The pane's `X-Calix-Surface-ID` header, when
    ...
    func handleJSONRPC(data: Data, authToken: String?, surfaceID: UUID? = nil) async -> (statusCode: Int, body: Data?) {
        ...
        case "initialize":
            ...
            // connection (one carrying `X-Calix-Surface-ID`). A
            ...
            //   pane's identity and inbox carry over, by design (Calix's
    ...
    private func handleToolCall(
        ...
        // every calix-ipc messaging tools/call request, rather than each
    ...
    /// The calix-ipc tools whose effects can change a bound peer's unread
    ...
    /// `CALIX_SURFACE_ID` env var) resolves to the SAME bound surface, so
    ...
    private func handleRegisterPeer(
        ...
        // elsewhere in this file (`parseSurfaceID`, the `X-Calix-Agent-Kind`
