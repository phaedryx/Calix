...
// Calix
//
// Manages reading/writing ~/.claude.json for the Calix IPC MCP server.
...
struct ClaudeConfigManager: Sendable {
    ...
    static func enableIPC(port: Int, token: String, configPath: String? = nil) throws {
        ...
        // Add/update calix-ipc entry.
        //
        // `X-Calix-Surface-ID` (Round 4) carries the pane's surface ID via
        // Claude Code's own `${VAR}` header env-expansion, so
        // `CalixMCPServer` can bind surface -> peer at MCP `initialize`
        // time even for a passive recipient that never calls a calix-ipc
        // tool. Must be the `${CALIX_SURFACE_ID:-}` empty-default form: an
        // undefined variable with no default fails Claude Code's config
        // parse entirely, which would break every *other* terminal (one
        // with no `CALIX_SURFACE_ID` env, e.g. outside Calix) too. An
        ...
        let calixEntry: [String: Any] = [
            ...
            "headers": [
                ...
                "X-Calix-Surface-ID": "${CALIX_SURFACE_ID:-}"
            ]
        ]
    ...
    static func disableIPC(configPath: String? = nil) throws {
        ...
        // Remove calix-ipc from mcpServers
