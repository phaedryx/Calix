...
// Calix
//
// Installs Calix's own zsh/fish command-lifecycle shell integration
// scripts into a Calix-owned directory tree -- never a user rc file (the
// command-log plan's explicit "the user's own rc files are never edited"
// constraint). CalixShellIntegrationEnvironment points ZDOTDIR /
...
enum ShellIntegrationInstaller {
    ...
    /// Default install root: `~/Library/Application Support/Calix/shell-integration`.
    ...
    /// CALIX_ZSH_ZDOTDIR (existence-checked, not merely non-empty --
    /// unset means fall back to $HOME), (2) sourcing calix.zsh (hook
    ...
    /// resumes exactly as if Calix were never involved.
    ///
    ...
    /// for `CALIX_ZSH_ZDOTDIR` and `ghostty-integration` swapped for
    /// `calix.zsh`. Quoted throughout (`'builtin' 'export' ...`) for the
    ...
    /// integration` itself only in the `always` block; `calix.zsh` has
    /// no such dependency on the user's `.zshenv`, so it's the `try`
    /// step here, with sourcing the user's real `.zshenv` moved to
    /// `always` -- guaranteeing that step runs even if `calix.zsh`
    /// itself errors, so an error in the Calix script never breaks the
    /// user's own shell startup chain.
    static let zshenvBody = """
    if [[ -n "${CALIX_ZSH_ZDOTDIR+X}" ]]; then
        'builtin' 'export' ZDOTDIR="$CALIX_ZSH_ZDOTDIR"
        'builtin' 'unset' 'CALIX_ZSH_ZDOTDIR'
    else
...

    {
        if [[ -o 'interactive' ]]; then
            ...
            # calix.zsh is found regardless of how this file itself was
            # reached (mirrors ghostty's own %x:A:h idiom).
            'builtin' 'typeset' _calix_file="${${(%):-%x}:A:h}"/calix.zsh
            [[ ! -r "$_calix_file" ]] || 'builtin' 'source' '--' "$_calix_file"
            'builtin' 'unset' '_calix_file'
        fi
    } always {
        ...
        'builtin' 'typeset' _calix_user_zshenv=${ZDOTDIR-$HOME}"/.zshenv"
        [[ ! -r "$_calix_user_zshenv" ]] || 'builtin' 'source' '--' "$_calix_user_zshenv"
        'builtin' 'unset' '_calix_user_zshenv'
    }
    """

    /// Installed at `<root>/zsh/calix.zsh`. Registers preexec/precmd
    ...
    /// POST start/end command-lifecycle events to the local Calix IPC
    ...
    /// (`CalixMCPServer.routeCommandEvent`), so a completed foreground curl
    ...
    /// Fails open
    /// (CALIX_SURFACE_ID and CALIX_SESSION_ID both unset, or
    ...
    /// `_calix_cmd_active` is never set for precmd to find).
    ///
    /// `_calix_post`'s agent-endpoint.json sed-extraction is the same
    ...
    static let calixZshBody = """
    [[ -o interactive ]] || return 0
    [[ -z "${_CALIX_HOOKS_LOADED:-}" ]] || return 0
    'builtin' 'typeset' '-g' '_CALIX_HOOKS_LOADED=1'

    [[ -n "${CALIX_SURFACE_ID:-}" || -n "${CALIX_SESSION_ID:-}" ]] || return 0

    'builtin' 'zmodload' 'zsh/datetime' 2>/dev/null || return 0

    _calix_post() {
        local endpoint_file="$HOME/Library/Application Support/Calix/agent-endpoint.json"
    ...
    _calix_preexec() {
        'builtin' 'typeset' '-g' '_calix_cmd_active=1'
        'builtin' 'typeset' '-g' _calix_cmd_id="$$-${EPOCHREALTIME/./}-$RANDOM"
        ...
        typeset -i _calix_ts
        _calix_ts=$(( EPOCHREALTIME * 1000 ))
        _calix_post "{\\"phase\\":\\"start\\",\\"cmd_id\\":\\"$_calix_cmd_id\\",\\"command_b64\\":\\"$command_b64\\",\\"cwd_b64\\":\\"$cwd_b64\\",\\"ts\\":$_calix_ts}"
    }

    _calix_precmd() {
        local code=$?
        [[ -n "${_calix_cmd_active:-}" ]] || return 0
        unset _calix_cmd_active
        typeset -i _calix_ts
        _calix_ts=$(( EPOCHREALTIME * 1000 ))
        _calix_post "{\\"phase\\":\\"end\\",\\"cmd_id\\":\\"$_calix_cmd_id\\",\\"exit_code\\":$code,\\"ts\\":$_calix_ts}"
    }

    'builtin' 'typeset' '-ag' 'preexec_functions'
    preexec_functions+=('_calix_preexec')
    'builtin' 'typeset' '-ag' 'precmd_functions'
    precmd_functions+=('_calix_precmd')

    return 0
    """

    /// Installed at `<root>/fish/vendor_conf.d/calix-integration.fish`.
    ...
    if not set -q CALIX_SURFACE_ID; and not set -q CALIX_SESSION_ID
        return 0
    end

    function _calix_post --argument-names body
        set -l endpoint_file "$HOME/Library/Application Support/Calix/agent-endpoint.json"
        ...
        set -l surface_id "$CALIX_SESSION_ID"
        test -n "$surface_id"; or set surface_id "$CALIX_SURFACE_ID"
        command curl -s --connect-timeout 0.1 --max-time 0.3 \\
            -X POST \\
            -H "Authorization: Bearer $token" \\
            -H "X-Calix-Surface-ID: $surface_id" \\
            -H 'Content-Type: application/json' \\
            --data-binary "$body" \\
            "http://127.0.0.1:$port/command-event" > /dev/null 2>&1; or return 0
    end

    function _calix_preexec --on-event fish_preexec
        ...
        set -g _calix_cmd_active 1
        set -g _calix_cmd_id "$fish_pid-"(date +%s000)"-"(random)
        ...
        _calix_post "{\\"phase\\":\\"start\\",\\"cmd_id\\":\\"$_calix_cmd_id\\",\\"command_b64\\":\\"$command_b64\\",\\"cwd_b64\\":\\"$cwd_b64\\",\\"ts\\":$ts}"
    end

    function _calix_postexec --on-event fish_postexec
        set -l code $status
        set -q _calix_cmd_active; or return 0
        set -e _calix_cmd_active
        set -l ts (math (date +%s) "*" 1000)
        _calix_post "{\\"phase\\":\\"end\\",\\"cmd_id\\":\\"$_calix_cmd_id\\",\\"exit_code\\":$code,\\"ts\\":$ts}"
    end
    """
    ...
    /// `<directory>/zsh/calix.zsh`.
    static func calixZshPath(in directory: URL) -> URL {
        directory.appendingPathComponent("zsh").appendingPathComponent("calix.zsh")
    }

    /// `<directory>/fish/vendor_conf.d/calix-integration.fish`.
    static func fishIntegrationPath(in directory: URL) -> URL {
        directory.appendingPathComponent("fish")
            ...
            .appendingPathComponent("calix-integration.fish")
    }
    ...
    /// CalixShellIntegrationEnvironment.apply(rootDirectory:).
    static func install(toDirectory directory: URL) throws -> URL {
        ...
        try write(calixZshBody, to: calixZshPath(in: directory))
    ...
    static func remove(fromDirectory directory: URL) throws {
        for path in [zshenvPath(in: directory), calixZshPath(in: directory), fishIntegrationPath(in: directory)] {
    ...
    static func isInstalled(inDirectory directory: URL) -> Bool {
        ...
        return fm.fileExists(atPath: zshenvPath(in: directory).path)
            && fm.fileExists(atPath: calixZshPath(in: directory).path)
