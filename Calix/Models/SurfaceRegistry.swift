...
private let logger = Logger(subsystem: "com.calix.terminal", category: "SurfaceRegistry")
...
final class SurfaceRegistry {
    ...
    /// same rationale as `CalixMCPServer.agentRegistry`.
    ...
    /// (see `CalixWindowController`'s reconnect path, which
    ...
    /// `CalixWindowController.windowWillClose`, iterate `allIDs` to
    ...
    /// `pwd`) so the surface's child process is a `calix-session attach`
    ...
    func destroySurface(_ id: UUID) {
        guard var entry = entries[id] else {
            ...
            // -- unregisterView + the .calixSurfaceDestroyed post must
    ...
    /// own doc comment). `CalixWindowController.performReconnect` calls
