...
// Calix
//
// 11-color preset enum for tab group color identification.
...
enum TabGroupColor: String, CaseIterable, Codable, Sendable {
    ...
    /// Maps a persisted raw value to the current palette, falling back to
    /// `.blue` for anything unrecognized (e.g. a color removed in some
    /// future revision).
    static func decode(rawValue raw: String?) -> TabGroupColor {
        guard let raw else { return .blue }
        return TabGroupColor(rawValue: raw) ?? .blue
    }
